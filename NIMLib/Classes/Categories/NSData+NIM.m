...
{
    return [self nim_gzippedDataWithCompressLevel:-1.0f];
}
...
- (BOOL)nim_writeToFile:(NSString *)filepath
{
    ...
    {
        NSString *md5 = [self nim_md5];
        NSString *fileMD5 = [NIMNOSUtil fileMD5:filepath];
        if (fileMD5 && md5 && [fileMD5 isEqualToString:md5])
        {
            NIMLogApp(@"duplicated file %@ exist for %@",filepath,md5);
            return YES;
        }
    }

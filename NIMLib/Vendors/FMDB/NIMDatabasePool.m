...
@interface NIMDatabasePool()

- (void)pushDatabaseBackInPool:(NIMDatabase*)db;
- (NIMDatabase*)db;

@end


@implementation NIMDatabasePool
...
+ (instancetype)databasePoolWithPath:(NSString*)aPath flags:(int)openFlags {
    return NIMDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags {
    ...
    if (self != nil) {
        _path               = [aPath copy];
        _lockQueue          = dispatch_queue_create([[NSString stringWithFormat:@"nimdb.%@", self] UTF8String], NULL);
        _databaseInPool     = NIMDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = NIMDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    _delegate = 0x00;
    NIMDBRelease(_path);
    NIMDBRelease(_databaseInPool);
    NIMDBRelease(_databaseOutPool);
    
    if (_lockQueue) {
        NIMDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (NIMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            db = [NIMDatabase databaseWithPath:self->_path];

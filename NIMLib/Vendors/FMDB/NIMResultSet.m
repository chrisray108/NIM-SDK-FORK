...
@implementation NIMResultSet
@synthesize query=_query;
@synthesize statement=_statement;

+ (instancetype)resultSetWithStatement:(NIMStatement *)statement usingParentDatabase:(NIMDatabase*)aDB {
    
    NIMResultSet *rs = [[NIMResultSet alloc] init];
    ...
    return NIMDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    NIMDBRelease(_columnNameToIndexMap);
...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return NIMDBReturnAutoreleased([dict copy]);
    }
    else {
...
- (void)setParentDB:(NIMDatabase *)newDb {
    _parentDB = newDb;
}

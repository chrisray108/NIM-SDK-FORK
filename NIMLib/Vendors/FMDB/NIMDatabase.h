...
#import "NIMDatabasePool.h"
    ...
    #define NIMDBAutorelease(__v) ([__v autorelease]);
    #define NIMDBReturnAutoreleased NIMDBAutorelease

    #define NIMDBRetain(__v) ([__v retain]);
    #define NIMDBReturnRetained NIMDBRetain

    #define NIMDBRelease(__v) ([__v release]);

    #define NIMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define NIMDBAutorelease(__v)
    #define NIMDBReturnAutoreleased(__v) (__v)

    #define NIMDBRetain(__v)
    #define NIMDBReturnRetained(__v) (__v)

    #define NIMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define NIMDBDispatchQueueRelease(__v)
    #else
        #define NIMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^NIMDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);
...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 - `<NIMStatement>` - A wrapper for `sqlite_stmt`.
...
@interface NIMDatabase : NSObject  {
...
- (NIMResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using FMDB you will not need to interact directly with `NIMStatement`, but rather with `<FMDatabase>` and `<FMResultSet>` only.
...
@interface NIMStatement : NSObject {

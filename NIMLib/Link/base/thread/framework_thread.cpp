...
public:
	virtual void Run()
	{
		...
		FrameworkThread::SetThreadWasQuitProperly(true);
	}
};
...
void FrameworkThread::StopSoon()
{
	...
	stopping_ = true;
	message_loop_->PostNonNestableTask(new ThreadQuitTask());
}
...
void FrameworkThread::Run()
{
	...
	SetThreadWasQuitProperly(false);
	{
		...
		assert(GetThreadWasQuitProperly());
	...
	set_thread_id(kInvalidThreadId);
	{
		...
		if (tls != NULL)
			assert(tls->managed == 0); // you must call ThreadManager::UnregisterThread before come there
	}
	FreeTlsData();
}

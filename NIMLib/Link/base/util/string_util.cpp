...
void StringTrimT(std::basic_string<CharType> &output)
{
	...
	CharType *dst = const_cast<CharType *>(src);

	for (; check < length; check++)
		if (NOT_SPACE(src[check]))
			*dst++ = src[check];

	output.resize(dst - src);
}
...
void UTF8ToUTF16(const UTF8Char *utf8, size_t length, std::basic_string<UTF16Char> &utf16)
{
	...
	{
		NIMConversionResult result = NIMConvertUTF8toUTF16(&src_begin,
													 src_end,
													 &dst_begin,
		...
		if (result == NIMsourceIllegal || result == NIMsourceExhausted)
		{
			utf16.clear();
			break;
		}
...
void UTF16ToUTF8(const UTF16Char *utf16, size_t length, std::basic_string<UTF8Char> &utf8)
{
	...
	while (src_begin < src_end)
	{
		NIMConversionResult result = NIMConvertUTF16toUTF8(&src_begin,
													 src_end,
													 &dst_begin,
													 dst_begin + COUNT_OF(output),
													 NIMlenientConversion);
...
void UTF8ToUTF32(const UTF8Char *utf8, size_t length, std::basic_string<UTF32Char> &utf32)
{
	...
	while (src_begin < src_end)
	{
		NIMConversionResult result = NIMConvertUTF8toUTF32(&src_begin,
													 src_end,
													 &dst_begin,
													 dst_begin + COUNT_OF(output),
													 NIMlenientConversion);
...
void UTF32ToUTF8(const UTF32Char *utf32, size_t length, std::basic_string<UTF8Char> &utf8)
{
	...
	while (src_begin < src_end)
	{
		NIMConversionResult result = NIMConvertUTF32toUTF8(&src_begin,
													 src_end,
													 &dst_begin,
													 dst_begin + COUNT_OF(output),
													 NIMlenientConversion);
...
void UTF16ToUTF32(const UTF16Char *utf16, size_t length, std::basic_string<UTF32Char> &utf32)
{
	...
	while (src_begin < src_end)
	{
		NIMConversionResult result = NIMConvertUTF16toUTF32(&src_begin,
													  src_end,
													  &dst_begin,
													  dst_begin + COUNT_OF(output),
													  NIMlenientConversion);
...
void UTF32ToUTF16(const UTF32Char *utf32, size_t length, std::basic_string<UTF16Char> &utf16)
{
	...
	while (src_begin < src_end)
	{
		NIMConversionResult result = NIMConvertUTF32toUTF16(&src_begin,
													  src_end,
													  &dst_begin,
													  dst_begin + COUNT_OF(output),
													  NIMlenientConversion);

...
{
	AddToIncomingQueue(PendingTask(task, EvalDelayedRuntime(0), true));
}

void MessageLoop::PostDelayedTask(Task *task, int64_t delay_ms)
{
	AddToIncomingQueue(PendingTask(task, EvalDelayedRuntime(delay_ms), true));
}

void MessageLoop::PostNonNestableTask(Task *task)
{
	AddToIncomingQueue(PendingTask(task, EvalDelayedRuntime(0), false));
}

void MessageLoop::PostNonNestableDelayedTask(Task *task, int64_t delay_ms)
{
	AddToIncomingQueue(PendingTask(task, EvalDelayedRuntime(delay_ms), false));
}
...
// the PendingTask class
MessageLoop::PendingTask::PendingTask(Task *task, TimeTicks delayed_run_time, bool nestable) :
task(task), delayed_run_time(delayed_run_time), nestable(nestable)
{
...
{
	assert(this == current());
	GetPump()->AddObserver(observer);
}

void UIMessageLoop::RemoveUIObserver(UIObserver *observer)
{
	assert(this == current());
	GetPump()->RemoveObserver(observer);
}
...
{
	assert(this == current());
	GetPump()->RegisterIOHandler(file_handle, handler);
}

bool IOMessageLoop::WaitForIOCompletion(DWORD timeout, IOHandler *handler)
{
	assert(this == current());
	return GetPump()->WaitForIOCompletion(timeout, handler);
}
...
{
	assert(this == current());
	return GetPump()->WatchFileDescriptor(fd, 
							              persistent, 
							              static_cast<LibeventMessagePump::Mode>(mode), 
							              controller, delegate);
}
#endif
}

...
#include "boost/noncopyable.hpp"                    // for boost::nocopyable
...
class BASE_API MessageLoop : public MessagePump::Delegate, public nimboost::noncopyable
{
public:	
	enum Type
	{
	...
	// 这个任务用于异步关闭本线程或者其他线程的消息循环
	class BASE_API QuitTask: public Task
	{
	public:
		void Run()
		{
	...
	void PostDelayedTask(Task *task, int64_t delay_ms);
	void PostNonNestableTask(Task *task);
	void PostNonNestableDelayedTask(Task *task, int64_t delay_ms);
...
protected:
	...
	struct PendingTask
	{
		PendingTask(Task *task, nbase::TimeTicks delayed_run_time, bool nestable);
		~PendingTask();
...
class BASE_API UIMessageLoop : public MessageLoop
{
public:
...
#error Not support currently!
	...
	{
		return reinterpret_cast<UIMessagePump *>(pump_.get());
	}
};

#endif // OS_WIN

class BASE_API IOMessageLoop : public MessageLoop
{
public:
...
#error Not support currently!
	...
	IOMessagePump* GetPump()
	{
		return reinterpret_cast<IOMessagePump *>(pump_.get());
	}
};

 ...
 *
 */
int nim_modp_b64_encode(char* dest, const char* str, int len);
 ...
 * \endcode
 */
int nim_modp_b64_decode(char* dest, const char* src, int len);
...
inline std::string& nim_modp_b64_encode(std::string& s)
{
    std::string x(modp_b64_encode_len(s.size()), '\0');
    int d = nim_modp_b64_encode(&x[0], s.data(), (int)s.size());
 ...
 */
inline std::string& nim_modp_b64_decode(std::string& s)
{
    std::string x(modp_b64_decode_len(s.size()), '\0');
    int d = nim_modp_b64_decode(const_cast<char*>(x.data()), s.data(), (int)s.size());
    if (d < 0) {
        x.clear();
    } else {
        x.erase(d, std::string::npos);

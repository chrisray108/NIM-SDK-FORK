...
public:
      ...
      void resize(size_t n)
      {
		  if (bb.resize(n))
			  return;
		  throw NException("resize buffer overflow", NIMResultMemoryError);
	  }
      void append(const char *data, size_t size)
      {
		  if (bb.append(data, size))
			  return;
		  throw NException("append buffer overflow", NIMResultMemoryError);
      }
      ...
      void replace(size_t pos, const char *rep, size_t n)
      {
          if (bb.replace(pos, rep, n))
              return;
          throw NException("replace buffer overflow", NIMResultMemoryError);
      }
      void reserve(size_t n)
      {
		  if (bb.reserve(n))
			  return;
		  throw NException("reserve buffer overflow", NIMResultMemoryError);
      }
...
public:
      ...
      Pack & push_varstr_as_uint64(const std::string &s) { return s.empty() ? push_uint64(0) : push_uint64(atoll(s.c_str())); }
      ...
      {
		  if (len > 0x7FFFFFFF)
			  throw NException("push_varstr: varstr too big", NIMResultMemoryError);
		  return push_len(uint32_t(len)).push(s, len);
	  }
      Pack & push_len(uint32_t u32)
      {
          while ((u32 & 0xFFFFFF80) != 0L)
          {
              push_uint8((uint8_t)((u32 & 0x7F) | 0x80));
              u32 = u32 >> 7;
          }
          return push_uint8((uint8_t)(u32 & 0x7F));
      }
...
public:
      ...
      {
          if (!empty())
			  throw NException("finish: too much data", NIMResultMemoryError);
      }

	  bool pop_bool() const
	  {
		  if (size_ < 1u)
			  throw NException("pop_uint8: not enough data", NIMResultMemoryError);
      ...
      {
		  if (size_ < 2u)
			  throw NException("pop_uint16: not enough data", NIMResultMemoryError);
      ...
      {
		  if (size_ < 4u)
			  throw NException("pop_uint32: not enough data", NIMResultMemoryError);
      ...
      {
		  if (size_ < 8u)
			  throw NException("pop_uint64: not enough data", NIMResultMemoryError);
		  uint64_t i64 = 0;
      ...
      std::string pop_uint64_as_str() const
      {
          ...
          char buffer[32] = {0};
          snprintf(buffer, 32, UINT64_FORMAT,value);
          return buffer;
      }
    
      uint32_t pop_len() const
      {
          uint32_t value = 0;
          uint32_t i = 0;
          uint8_t b;
          while (((b = pop_uint8()) & 0x80) != 0)
          {
              ...
              i += 7;
              if (i > 28)
                  throw NException("pop len: not enough data", NIMResultMemoryError);
          }
          return value | (b << i);
      }
      ...
      const char * pop_fetch_ptr(size_t k) const
      {
          ...
          {
              //abort();
              throw NException("pop_fetch_ptr: not enough data", NIMResultMemoryError);
          }
...
#define YX_REQUEST_ONLY()        void unmarshal(const nbase::Unpack &up) {assert(0);}
#define YX_RESPONSE_ONLY()       void marshal(nbase::Pack &p) const{assert(0);}
...
struct BASE_API Voidmable : public nbase::Marshallable
{
	virtual void marshal(Pack &p) const {}
	virtual void unmarshal(const Unpack &up) {}
};

struct BASE_API Mulmable : public nbase::Marshallable
{
...
struct BASE_API Mulumable : public nbase::Marshallable
{
...
struct BASE_API Rawmable : public nbase::Marshallable
{

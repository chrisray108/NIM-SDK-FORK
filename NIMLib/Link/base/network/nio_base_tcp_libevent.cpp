	...
	: sock_(-1), event_base_(event_base), listener_(0), port_(0), connected_(false)
{
	...
	else
	{
        ...
        proxy_connected_    = true;
        proxy_can_change_   = true;
        proxy_layer_        = NULL;
	}
}
...
void TcpClient::OnEvent(int fd, short event)
{
	...
	if (event & EV_READ)
	{
		...
		char buf[1];
		int n = (int)::recv(sock_, buf, 1, MSG_PEEK);
		if (n == 0 || (n == SOCKET_ERROR && !would_block()))
			listener()->OnClose();
		else
        {
            if (!proxy_connected_ && proxy_layer_)
            {
                proxy_layer_->OnRead();
                proxy_connected_ = proxy_layer_->IsProxyConnected();
                if (proxy_connected_)
                    listener()->OnConnect();
            }
            else
...
{
    proxy_can_change_ = false;
    if (proxy_layer_) 
        return proxy_layer_->Connect(host, port);
    else
        return ConnectDirectly(host, port);
}

int TcpClient::Write(const void *data, size_t size)
{
	int n = (int)::send(sock_, (const char *)data, size, 0);
	if (n == -1 && !would_block())
	{
		DEFLOG(nbase::LogInterface::LV_ERR, 
			   __FILE__, 
...
{
	int n = (int)::recv(sock_, (char *)buf, size, 0);
...
int TcpClient::ConnectDirectly(const char *host, int port)
{
    host_ = host;
    port_ = port;
    ...
    int op = 1;
    int err =  setsockopt(sock_, IPPROTO_TCP, TCP_NODELAY, (char *)&op, sizeof(int));
    if (err != 0)
    {
        DEFLOG(nbase::LogInterface::LV_ERR,
               __FILE__,
               __LINE__,
               "nbase::TcpClient set nodelay failed %d",err);
    }
    ...
    if (r != 0 && !would_block())
    {
        DEFLOG(nbase::LogInterface::LV_ERR,
               __FILE__, 
               __LINE__, 
               "nbase::TcpClient: connect() error %d",r);
    }
    return r;
}
    
void TcpClient::SetProxy(const nbase::ProxySet *proxy)
{
    if (proxy_can_change_)
    {
        proxy_connected_ = false;
        if (proxy_layer_ != NULL)
        {
            delete proxy_layer_;
        }
        proxy_layer_ = new nbase::ProxySocketLayer(this, proxy);
    }
}

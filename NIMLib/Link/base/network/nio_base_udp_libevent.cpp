	...
	: event_base_(event_base), listener_(0)
{
	...
	else
	{
		...
		reuse_address(sock_);
        memset(&event_read_, 0, sizeof(event));
...
int UdpSock::Write(uint32_t ip, uint16_t port, const void *data, size_t size)
{
	...
	addr.sin_port = htons(port);
	int n = (int)::sendto(sock_, (const char *)data, size, 0, (const sockaddr *)&addr, sizeof(addr));
	if (n == -1 && !would_block())
...
int UdpSock::Read(uint32_t &ip, uint16_t &port, void *data, size_t size)
{
	sockaddr_in addr;
	int len = sizeof(addr);
	int n   = (int)::recvfrom(sock_, (char *)data, size, 0, (sockaddr *)&addr, (socklen_t*)&len);
	if (n == -1 && !would_block())

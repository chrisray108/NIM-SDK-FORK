...
// Date: 2011/6/30
//
// Condition variable implementation on windows
...
{
	NAutoLock auto_lock(&internal_lock_);
	...
	{   // Rare shutdown problem.
		...
		NAutoUnlock auto_unlock(&internal_lock_);
...
void ConditionVariable::TimedWait(const TimeDelta& max_time)
{
	...
	{
		NAutoUnlock unlock(&user_lock_);  // Release caller's lock
		WaitForSingleObject(handle, static_cast<DWORD>(max_time.ToMilliseconds()));

...
public:
  ...
  RET call(CALL_FORMAL_ARGS) const {
    if (detail::select(nimboost::tuples::get<0>(args), CALL_ACTUAL_ARGS)) 
      detail::select(nimboost::tuples::get<1>(args), CALL_ACTUAL_ARGS); 
  }
};
...
public:
  ...
  RET call(CALL_FORMAL_ARGS) const {
    if (detail::select(nimboost::tuples::get<0>(args), CALL_ACTUAL_ARGS)) 
      ...
      detail::select(nimboost::tuples::get<2>(args), CALL_ACTUAL_ARGS);
  }
};
...
struct return_type_2_ifthenelsereturn<1, false, false, false, A, B> {
  ...
  typedef const typename nimboost::remove_reference<A>::type plainA; 
  typedef const typename nimboost::remove_reference<B>::type plainB; 
  // TODO: Add support for volatile ?

  typedef typename
       return_type_2_ifthenelsereturn<
         2,
         nimboost::is_convertible<plainA,plainB>::value, 
         nimboost::is_convertible<plainB,plainA>::value,
         nimboost::is_same<plainA,plainB>::value,
         plainA, 
         plainB>::type type;
};
...
struct return_type_2<other_action<ifthenelsereturn_action>, A, B> { 
  ...
  typedef typename 
    nimboost::add_const<typename detail::same_or_not<A1, B1>::type>::type type;
};

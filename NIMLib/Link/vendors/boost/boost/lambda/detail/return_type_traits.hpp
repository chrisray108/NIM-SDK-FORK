...
using ::nimboost::type_traits::ice_and;
using ::nimboost::type_traits::ice_or;
using ::nimboost::type_traits::ice_not;
...
namespace detail {
  ...
  class protect_conversion {
      typedef typename nimboost::remove_reference<T>::type non_ref_T;
    public:

  // add const to rvalues, so that all rvalues are stored as const in 
  // the args tuple
    typedef typename detail::IF_type<
//      nimboost::is_reference<T>::value && !nimboost::is_const<non_ref_T>::value,
      ice_and<nimboost::is_reference<T>::value,
              ice_not<nimboost::is_const<non_ref_T>::value>::value>::value,
...
// experimental feature
  ...
  typedef typename nimboost::remove_reference<A>::type non_ref_A;
  typedef typename nimboost::remove_reference<B>::type non_ref_B;

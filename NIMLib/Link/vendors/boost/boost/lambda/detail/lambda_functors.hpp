...
template<> struct placeholder<FIRST> {
  ...
  RET call(CALL_FORMAL_ARGS) const { 
    BOOST_STATIC_ASSERT(nimboost::is_reference<RET>::value); 
...
public:
  ...
  // Support for nimboost::result_of.
...
template<class T>
struct result_of<nimboost::lambda::lambda_functor<T>()>
{
    typedef typename nimboost::lambda::lambda_functor<T>::nullary_return_type type;
};

template<class T>
struct result_of<const nimboost::lambda::lambda_functor<T>()>
{
    typedef typename nimboost::lambda::lambda_functor<T>::nullary_return_type type;
};
...
template<class T>
struct tr1_result_of<nimboost::lambda::lambda_functor<T>()>
{
    typedef typename nimboost::lambda::lambda_functor<T>::nullary_return_type type;
};

template<class T>
struct tr1_result_of<const nimboost::lambda::lambda_functor<T>()>
{
    typedef typename nimboost::lambda::lambda_functor<T>::nullary_return_type type;
};

...
namespace nimboost { namespace range_detail_microsoft {
    ...
    size_of(T const& x)
    {
        return std::distance(nimboost::begin(x), nimboost::end(x));
    }
...
} } // namespace nimboost::range_detail_microsoft
...
/**/
    ...
    #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TYPE_begin(Fullname) \
        inline \
        nimboost::range_detail_microsoft::mutable_iterator_of< Fullname >::type \
        BOOST_RANGE_DETAIL_MICROSOFT_range_begin(Fullname& x) \
        { \
            return nimboost::range_detail_microsoft::begin_of(x); \
        } \
    /**/
    ...
    #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TYPE_begin_const(Fullname) \
        inline \
        nimboost::range_detail_microsoft::const_iterator_of< Fullname >::type \
    ...
    #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TYPE_end(Fullname) \
        ...
        { \
            return nimboost::range_detail_microsoft::end_of(x); \
        } \
    /**/
    ...
    /**/
        ...
        #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TYPE_size(Fullname) \
            inline \
            nimboost::range_detail_microsoft::size_type_of< Fullname >::type \
            boost_range_size(Fullname const& x) \
            { \
                return nimboost::range_detail_microsoft::size_of(x); \
            } \
        /**/
...
/**/
    ...
    #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TEMPLATE_begin(Params, Fullname) \
        ...
        typename nimboost::range_detail_microsoft::mutable_iterator_of< Fullname >::type \
        BOOST_RANGE_DETAIL_MICROSOFT_range_begin(Fullname& x) \
        { \
            return nimboost::range_detail_microsoft::begin_of(x); \
        } \
    ...
    #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TEMPLATE_begin_const(Params, Fullname) \
        template< Params > inline \
        typename nimboost::range_detail_microsoft::const_iterator_of< Fullname >::type \
        BOOST_RANGE_DETAIL_MICROSOFT_range_begin(Fullname const& x) \
        { \
            return nimboost::range_detail_microsoft::begin_of(x); \
        } \
    ...
    /**/
        ...
        #define BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TEMPLATE_size(Params, Fullname) \
            template< Params > inline \
            typename nimboost::range_detail_microsoft::size_type_of< Fullname >::type \
            boost_range_size(Fullname const& x) \
            { \
                return nimboost::range_detail_microsoft::size_of(x); \
            } \
...
namespace nimboost { namespace range_detail_microsoft {
    ...
    public:
        ...
        explicit list_iterator(ListT& lst, POSITION pos) :
            m_plst(nimboost::addressof(lst)), m_pos(pos)
        { }
...
namespace nimboost { namespace range_detail_microsoft {
    ...
    {
        return
            nimboost::distance(rng1) == nimboost::distance(rng2) &&
            std::equal(nimboost::begin(rng1), nimboost::end(rng1), nimboost::begin(rng2))
        ;
    }
    ...
    {
        typedef typename nimboost::range_const_iterator<AssocContainer>::type iter_t;
        for (iter_t it = nimboost::const_begin(ac), last = nimboost::const_end(ac); it != last; ++it) {
    ...
    bool test_emptiness(Range& )
    {
        ...
        Range emptyRng;
        result = result && nimboost::empty(emptyRng);
    ...
    bool test_trivial(Range& rng)
    {
        ...
        citer_t cit = nimboost::begin(rng);
        (void)cit; // unused

        // mutability check
        typedef typename range_value<Range>::type val_t;
        val_t v = *nimboost::begin(rng);
        *nimboost::begin(rng) = v;
        result = result && *nimboost::begin(rng) == v;
    ...
    {
        nimboost::function_requires< ForwardRangeConcept<Range> >();
        ...
        std::copy(nimboost::begin(rng), nimboost::end(rng), std::back_inserter(saved));
        std::rotate(nimboost::begin(saved), nimboost::next(nimboost::begin(saved)), nimboost::end(saved));

        std::rotate(nimboost::begin(rng), nimboost::next(nimboost::begin(rng)), nimboost::end(rng));
    ...
    {
        nimboost::function_requires< BidirectionalRangeConcept<Range> >();
        ...
        result = result && (test_equals)(
            nimboost::make_iterator_range(nimboost::rbegin(saved), nimboost::rend(saved)),
            nimboost::make_iterator_range(nimboost::rbegin(rng), nimboost::rend(rng))
        );
    ...
    {
        nimboost::function_requires< RandomAccessRangeConcept<Range> >();
        ...
        std::vector<val_t> saved;
        std::copy(nimboost::begin(rng), nimboost::end(rng), std::back_inserter(saved));
        std::sort(nimboost::begin(saved), nimboost::end(saved));

        std::random_shuffle(nimboost::begin(rng), nimboost::end(rng));
        std::sort(nimboost::begin(rng), nimboost::end(rng));
        result = result && (test_equals)(rng, saved);

        std::random_shuffle(nimboost::begin(rng), nimboost::end(rng));
        std::stable_sort(nimboost::begin(rng), nimboost::end(rng));
        result = result && (test_equals)(rng, saved);

        std::random_shuffle(nimboost::begin(rng), nimboost::end(rng));
        std::partial_sort(nimboost::begin(rng), nimboost::end(rng), nimboost::end(rng));
        result = result && (test_equals)(rng, saved);
    ...
    bool test_init_array(ArrayT& arr, SampleRange const& sample)
    {
        ...
        for (iter_t it = nimboost::const_begin(sample), last = nimboost::const_end(sample); it != last; ++it) {
    ...
    bool test_init_map(MapT& map, SampleMap const& sample)
    {
        ...
        return nimboost::distance(map) == nimboost::distance(sample);
    }
    ...
    struct test_mutable_iter :
        nimboost::is_same< typename nimboost::BOOST_RANGE_DETAIL_MICROSOFT_range_mutable_iterator<Range>::type, Iter >
    { };


    template< class Range, class Iter >
    struct test_const_iter :
        nimboost::is_same< typename nimboost::range_const_iterator<Range>::type, Iter >
    { };

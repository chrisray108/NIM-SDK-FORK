...
namespace nimboost { namespace range_detail_microsoft {
    ...
    public:
        ...
        explicit mfc_ptr_array_iterator(ArrayT& arr, INT_PTR index) :
            super_t(index), m_parr(nimboost::addressof(arr))
        { }
    ...
    public:
        ...
        explicit mfc_map_iterator(MapT const& map, POSITION pos) :
            m_pmap(nimboost::addressof(map)), m_posNext(pos)
        {
            increment();
        }
    ...
    public:
        ...
        explicit mfc_cpair_map_iterator(MapT& map, PairT *pp) :
            m_pmap(nimboost::addressof(map)), m_pp(pp)
        { }
    ...
    struct customization< ::CTypedPtrArray<BaseClass, PtrType> >
    {
        ...
        Iterator begin(X& x)
        {
            return Iterator(nimboost::begin<BaseClass>(x), fun<X>());
        }

        template< class Iterator, class X >
        Iterator end(X& x)
        {
            return Iterator(nimboost::end<BaseClass>(x), fun<X>());
        }
    };

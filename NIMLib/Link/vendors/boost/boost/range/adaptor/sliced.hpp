...
namespace nimboost
{
    namespace adaptors
    {
        ...
        class sliced_range : public nimboost::iterator_range< BOOST_DEDUCED_TYPENAME range_iterator<RandomAccessRange>::type >
        {
            typedef nimboost::iterator_range< BOOST_DEDUCED_TYPENAME range_iterator<RandomAccessRange>::type > base_t;
        public:
            template<typename Rng, typename T, typename U>
            sliced_range(Rng& rng, T t, U u)
                : base_t(nimboost::make_iterator_range(rng, t, u - nimboost::size(rng)))
            {
            }
        };
        ...
        slice( RandomAccessRange& rng, std::size_t t, std::size_t u )
        {
            ...
            BOOST_ASSERT( static_cast<std::size_t>(nimboost::size(rng)) >= u &&

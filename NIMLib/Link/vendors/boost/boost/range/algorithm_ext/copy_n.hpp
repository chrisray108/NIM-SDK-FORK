...
inline OutputIterator copy_n(const SinglePassRange& rng, Size n, OutputIterator out)
{
    ...
    BOOST_ASSERT( n <= static_cast<Size>(::nimboost::distance(rng)) );
    BOOST_ASSERT( n >= static_cast<Size>(0) );

    BOOST_DEDUCED_TYPENAME range_iterator<const SinglePassRange>::type source = ::nimboost::begin(rng);

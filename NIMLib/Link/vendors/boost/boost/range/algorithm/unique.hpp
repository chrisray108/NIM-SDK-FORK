...
unique( ForwardRange& rng )
{
    ...
    return range_return<ForwardRange,re>::
        pack( std::unique( nimboost::begin(rng),
                           nimboost::end(rng)), rng );
}
...
unique( ForwardRange& rng, BinaryPredicate pred )
{
    BOOST_RANGE_CONCEPT_ASSERT(( ForwardRangeConcept<ForwardRange> ));
    return range_return<ForwardRange,re>::
        pack(std::unique(nimboost::begin(rng), nimboost::end(rng), pred),
             rng);
}
/// \overload
template< range_return_value re, class ForwardRange, class BinaryPredicate >
...
{
    BOOST_RANGE_CONCEPT_ASSERT(( ForwardRangeConcept<ForwardRange> ));
    return ::nimboost::range::unique<return_begin_found>(rng);
}
/// \overload
template< class ForwardRange >
...
{
    BOOST_RANGE_CONCEPT_ASSERT(( ForwardRangeConcept<const ForwardRange> ));
    return ::nimboost::range::unique<return_begin_found>(rng, pred);
}

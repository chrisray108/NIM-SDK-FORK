...
#include <boost/integer.hpp>  // for nimboost::uint_t
...
#define BOOST_CRC_PARM_TYPE  typename ::nimboost::uint_t<Bits>::fast
#else
...
namespace detail
{
    ...
    template < std::size_t Bits >
    struct high_uint_t
        : nimboost::uint_t< Bits >
    {
        typedef nimboost::uint_t<Bits>        base_type;
    ...
    struct reflector
    {
        typedef typename nimboost::uint_t<Bits>::fast  value_type;

        static  value_type  reflect( value_type x );

    };  // nimboost::detail::reflector

...
#define BOOST_TYPEOF_modifier_support(ID, Fun)\
    ...
    {\
        typedef\
            typename nimboost::type_of::encode_type<\
            typename nimboost::type_of::push_back<\
            V\
            , nimboost::mpl::size_t<ID> >::type\
            , T>::type\
            type;\
    };\
    template<class Iter> struct decode_type_impl<nimboost::mpl::size_t<ID>, Iter>\
    {\
        typedef nimboost::type_of::decode_type<Iter> d1;\
...
#define BOOST_TYPEOF_array_support(ID, Qualifier)\
    ...
    {\
        typedef\
            ...
            V\
            , nimboost::mpl::size_t<ID> >::type\
            , nimboost::mpl::size_t<N> >::type\
            , T>::type\
        type;\
    };\
    template<class Iter>\
    struct decode_type_impl<nimboost::mpl::size_t<ID>, Iter>\
    {\
        enum{n = Iter::type::value};\
        typedef nimboost::type_of::decode_type<typename Iter::next> d;\

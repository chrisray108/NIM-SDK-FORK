...
namespace nimboost { namespace proto
{
    ...
    /// assert( nimboost::addressof(i) == nimboost::addressof(j) );
    /// \endcode
    ...
    /// in a \c nimboost::reference_wrapper\<\>.
    ///
    /// Example:
    ///
    /// \code
    /// proto::terminal<int>::type i = {42};
    /// nimboost::reference_wrapper<proto::terminal<int>::type> j
    ///     = proto::when<_, proto::_byref(_)>()(i);
    /// assert( nimboost::addressof(i) == nimboost::addressof(j.get()) );
    /// \endcode
    struct _byref : callable
    {
        ...
        {
            typedef nimboost::reference_wrapper<T const> const type;
        };

        template<typename This, typename T>
        struct result<This(T &)>
        {
            typedef nimboost::reference_wrapper<T> const type;
        };

        /// Wrap the parameter \c t in a \c nimboost::reference_wrapper\<\>
        /// \param t The object to wrap
        /// \return <tt>nimboost::ref(t)</tt>
        /// \throw nothrow
        template<typename T>
        nimboost::reference_wrapper<T> const operator ()(T &t) const
        {
            return nimboost::reference_wrapper<T>(t);
        }

        /// \overload
        ///
        template<typename T>
        nimboost::reference_wrapper<T const> const operator ()(T const &t) const
        {
            return nimboost::reference_wrapper<T const>(t);
        }
    };

    /// \brief A unary CallableTransform that strips references
    /// and \c nimboost::reference_wrapper\<\> from its argument.
    ///
    /// Example:
    ///
    /// \code
    /// proto::terminal<int>::type i = {42};
    /// int j = 67;
    /// int k = proto::when<_, proto::_byval(proto::_state)>()(i, nimboost::ref(j));
    /// assert( 67 == k );
    /// \endcode
    struct _byval : callable
    {
        ...
        template<typename This, typename T>
        struct result<This(nimboost::reference_wrapper<T>)>
          : result<This(T)>
        {};
        ...
        ///
        template<typename T>
        T operator ()(nimboost::reference_wrapper<T> const &t) const
        {
            return t;
        }
    };

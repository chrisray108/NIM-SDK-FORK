...
struct same_size_impl
{
    ...
    {
        return ( nimboost::size( x1 ) == nimboost::size( x2 ) );
    }
};
...
struct same_size_fusion
{
    ...
    template< class S1 , class S2 >
    bool same_size_op( const S1 &x1 , const S2 &x2 , nimboost::true_type ) const
    {
        return same_size( x1 , x2 );
    }

    template< class S1 , class S2 >
    bool same_size_op( const S1 &x1 , const S2 &x2 , nimboost::false_type ) const
    {
        return true;
    }
};
...
struct same_size_impl< FusionSeq , FusionSeq , typename nimboost::enable_if< typename nimboost::fusion::traits::is_sequence< FusionSeq >::type >::type >
{
    static bool same_size( const FusionSeq &x1 , const FusionSeq &x2 )
    {
        typedef nimboost::fusion::vector< const FusionSeq& , const FusionSeq& > Sequences;
        Sequences sequences( x1 , x2 );
        return nimboost::fusion::all( nimboost::fusion::zip_view< Sequences >( sequences ) ,
                                   nimboost::fusion::make_fused( detail::same_size_fusion() ) );
    }
};

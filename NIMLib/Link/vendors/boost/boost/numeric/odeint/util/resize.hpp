...
namespace detail {
    ...
    struct resizer
    {
        ...
        template< class StateOut , class StateIn >
        void resize_op( StateOut &x1 , const StateIn &x2 , nimboost::true_type ) const
        {
            resize( x1 , x2 );
        }

        template< class StateOut , class StateIn >
        void resize_op( StateOut &x1 , const StateIn &x2 , nimboost::false_type ) const
        {
        }
...
struct resize_impl< FusionSeq , FusionSeq , typename nimboost::enable_if< typename nimboost::fusion::traits::is_sequence< FusionSeq >::type >::type >
{
    static void resize( FusionSeq &x1 , const FusionSeq &x2 )
    {
        typedef nimboost::fusion::vector< FusionSeq& , const FusionSeq& > Sequences;
        Sequences sequences( x1 , x2 );
        nimboost::fusion::for_each( nimboost::fusion::zip_view< Sequences >( sequences ) , nimboost::fusion::make_fused( detail::resizer() ) );
    }
};

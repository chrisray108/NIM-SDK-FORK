...
public:
    ...
    typename nimboost::disable_if< nimboost::is_same< DerivIn , time_type > , void >::type
 ...
 *
 * \note The system is always passed as value, which might result in poor performance if it contains data. In this case it can be used with `nimboost::ref`
 * or `std::ref`, for example `stepper.do_step( nimboost::ref( sys ) , x , t , dt );`
 *

...
{
   return nimboost::lexical_cast<T>(s);
}
#endif
template <class T>
...
#define BOOST_MATH_BIG_CONSTANT(T, D, x)\
   nimboost::math::tools::make_big_value<T>(\
      BOOST_JOIN(x, L), \
      BOOST_STRINGIZE(x), \
      ...
      nimboost::is_convertible<const char*, T>())
//
...
#define BOOST_MATH_HUGE_CONSTANT(T, D, x)\
   nimboost::math::tools::make_big_value<T>(0.0L, BOOST_STRINGIZE(x), \

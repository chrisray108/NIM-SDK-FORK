...
inline void unpack_and_append_tuple(Seq& s,
                                    const Item& data,
                                    const nimboost::integral_constant<int, N>&,
                                    ...
                                    const nimboost::true_type&)
{
   ...
   value_type val = nimboost::math::get<N>(data);
   s.push_back(val);

   typedef nimboost::integral_constant<int, N+1> next_value;
   typedef nimboost::integral_constant<bool, (nimboost::math::tuple_size<Item>::value > N+1)> terminate;

   unpack_and_append_tuple(s, data, next_value(), terminate());
}

template <class Seq, class Item>
inline void unpack_and_append(Seq& s, const Item& data, const nimboost::true_type&)
{
   s.push_back(data);
}

template <class Seq, class Item>
inline void unpack_and_append(Seq& s, const Item& data, const nimboost::false_type&)
{
   // Item had better be a tuple-like type or we've had it!!!!
   typedef nimboost::integral_constant<int, 0> next_value;
   typedef nimboost::integral_constant<bool, (nimboost::math::tuple_size<Item>::value > 0)> terminate;
...
{
   typedef typename Seq::value_type value_type;
   unpack_and_append(s, data, ::nimboost::is_convertible<Item, value_type>());
}
   ...
   test_data& insert(F func, const parameter_info<T>& arg1)
   {
      ...
      while(a != b)
      {
         ...
         try{
            ...
            nimboost::math::tools::detail::unpack_and_append(row, func(*a));
            m_data.insert(row);
         }
...
#  pragma warning(disable: 4127)
   ...
   case periodic_in_range:
      ...
      do{
         ...
         try{
            info.z1 = nimboost::lexical_cast<T>(line);
            break;
         }
         catch(const nimboost::bad_lexical_cast&)
         {
      ...
      do{
         ...
         else
         {
            try
            {
               info.z2 = nimboost::lexical_cast<T>(line);
            }
            catch(const nimboost::bad_lexical_cast&)
            {
               std::cout << "Sorry, that was not valid input, try again [y/n]?";
      ...
      do{
         ...
         nimboost::algorithm::trim(line);
         try{
            info.n1 = nimboost::lexical_cast<int>(line);
            info.n2 = 0;
            if(info.n1 <= 0)
            {
   ...
   case power_series:
      ...
      do{
         ...
         nimboost::algorithm::trim(line);
         try{
            info.n2 = nimboost::lexical_cast<int>(line);
            break;
         }
         catch(const nimboost::bad_lexical_cast&)
         {
                         ...
                         const char* name)
{
   ...
   "   static const nimboost::array<nimboost::array<T, "

...
T chebyshev_coefficient(unsigned n, unsigned m)
{
   ...
   if(r & 1)
      result = -result;
   result /= n - r;
   result *= nimboost::math::binomial_coefficient<T>(n - r, r);
   result *= ldexp(1.0f, m);
   return result;
}
...
public:
   ...
   polynomial(const polynomial<U>& p)
   {
      ...
      {
         m_data.push_back(nimboost::math::tools::real_cast<T>(p[i]));
      }
   }
   ...
   {
      return nimboost::math::tools::evaluate_polynomial(&m_data[0], z, m_data.size());;
   }

...
//
template <class T>
nimboost::numeric::ublas::vector<T> solve(
          const nimboost::numeric::ublas::matrix<T>& A_,
          const nimboost::numeric::ublas::vector<T>& b_)
{
   //BOOST_ASSERT(A_.size() == b_.size());

   nimboost::numeric::ublas::matrix<T> A(A_);
   nimboost::numeric::ublas::vector<T> b(b_);
   nimboost::numeric::ublas::permutation_matrix<> piv(b.size());
   lu_factorize(A, piv);
   lu_substitute(A, piv, b);
   //
   // iterate to reduce error:
   //
   nimboost::numeric::ublas::vector<T> delta(b.size());

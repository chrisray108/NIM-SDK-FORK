...
typename Dist::value_type 
   do_inverse_discrete_quantile(
      ...
      const Tolerance& tol,
      nimboost::uintmax_t& max_iter)
{
   typedef typename Dist::value_type value_type;
   typedef typename Dist::policy_type policy_type;

   static const char* function = "nimboost::math::do_inverse_discrete_quantile<%1%>";
   ...
   nimboost::math::tie(min_bound, max_bound) = support(dist);
   ...
   //
   if((nimboost::math::sign)(fb) == (nimboost::math::sign)(fa))
   {
      if(fa < 0)
      {
         //
         ...
         // until we find it:
         //
         while(((nimboost::math::sign)(fb) == (nimboost::math::sign)(fa)) && (a != b))
         {
            if(count == 0)

...
T float_next_imp(const T& val, const Policy& pol)
{
   ...
   int fpclass = (nimboost::math::fpclassify)(val);
...
inline double float_next(const double& val, const Policy& pol)
{
   ...
   if(!(nimboost::math::isfinite)(val) && (val > 0))
...
inline typename tools::promote_args<T, U>::type nextafter(const T& val, const U& direction, const Policy& pol)
{
   ...
   return val < direction ? nimboost::math::float_next<result_type>(val, pol) : val == direction ? val : nimboost::math::float_prior<result_type>(val, pol);
}
...
T float_distance_imp(const T& a, const T& b, const Policy& pol)
{
   ...
   if(nimboost::math::sign(a) != nimboost::math::sign(b))
   ...
   //
   frexp(((nimboost::math::fpclassify)(a) == FP_SUBNORMAL) ? tools::min_value<T>() : a, &expon);
   ...
   //
   T mb, x, y, z;
   if(((nimboost::math::fpclassify)(a) == FP_SUBNORMAL) || (b - a < tools::min_value<T>()))
   {
      //
...
{
   return nimboost::math::float_distance(a, b, policies::policy<>());
}
...
{
   return nimboost::math::float_advance(val, distance, policies::policy<>());
}

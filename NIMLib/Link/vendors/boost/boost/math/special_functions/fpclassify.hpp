...
//
// This must not be located in any namespace under nimboost::math
...
template <class T>
inline bool is_nan_helper(T t, const nimboost::true_type&)
{
#ifdef isnan
   return isnan(t);
...
template <class T>
inline bool is_nan_helper(T, const nimboost::false_type&)
{
   return false;
}
...
inline int fpclassify_imp BOOST_NO_MACRO_EXPAND(T t, const generic_tag<true>&)
{
   ...
   if(::nimboost::math_detail::is_nan_helper(t, ::nimboost::is_floating_point<T>()))
      return FP_NAN;
#elif defined(isnan)
   if(nimboost::math_detail::is_nan_helper(t, ::nimboost::is_floating_point<T>()))
      return FP_NAN;
#elif defined(_MSC_VER) || defined(__BORLANDC__)
   if(::_isnan(nimboost::math::tools::real_cast<double>(t)))
      return FP_NAN;
#endif
...
{
   return nimboost::math::detail::fpclassify_imp(t, generic_tag<true>());
}
#endif
...
{
   return nimboost::math::detail::isfinite_impl(t, generic_tag<true>());
}
#endif
...
{ //!< \brief return true if floating-point type t is finite.
   typedef typename detail::fp_traits<T>::type traits;
   typedef typename traits::method method;
   // typedef typename nimboost::is_floating_point<T>::type fp_tag;
   typedef typename tools::promote_args_permissive<T>::type value_type;
...
{ //!< \brief return true if floating-point type t is finite.
   typedef detail::fp_traits<long double>::type traits;
   typedef traits::method method;
   typedef nimboost::is_floating_point<long double>::type fp_tag;
   typedef long double value_type;
   return detail::isfinite_impl(static_cast<value_type>(x), method());
}
#endif
...
{
   return nimboost::math::detail::isnormal_impl(t, generic_tag<true>());
}
#endif
...
{
   return nimboost::math::detail::isinf_impl(t, generic_tag<true>());
}
#endif
...
#ifdef isnan
template <> inline bool isnan BOOST_NO_MACRO_EXPAND<float>(float t){ return ::nimboost::math_detail::is_nan_helper(t, nimboost::true_type()); }
template <> inline bool isnan BOOST_NO_MACRO_EXPAND<double>(double t){ return ::nimboost::math_detail::is_nan_helper(t, nimboost::true_type()); }
template <> inline bool isnan BOOST_NO_MACRO_EXPAND<long double>(long double t){ return ::nimboost::math_detail::is_nan_helper(t, nimboost::true_type()); }

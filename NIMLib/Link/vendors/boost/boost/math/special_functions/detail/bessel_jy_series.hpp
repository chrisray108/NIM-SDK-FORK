...
inline T bessel_j_small_z_series(T v, T x, const Policy& pol)
{
   ...
   {
      prefix = pow(x / 2, v) / nimboost::math::tgamma(v+1, pol);
   }
   else
   {
      prefix = v * log(x / 2) - nimboost::math::lgamma(v+1, pol);
      prefix = exp(prefix);
   }
   if(0 == prefix)
      return prefix;
...
#endif
   policies::check_series_iterations<T>("nimboost::math::bessel_j_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
...
inline T bessel_y_small_z_series(T v, T x, T* pscale, const Policy& pol)
{
   ...
   if(!need_logs)
   {
      gam = nimboost::math::tgamma(v, pol);
      p = pow(x / 2, v);
   ...
   {
      gam = nimboost::math::lgamma(v, pol);
      p = v * p;
   ...
   *pscale = scale;
   ...
   T result = nimboost::math::tools::sum_series(s, nimboost::math::policies::get_epsilon<T, Policy>(), max_iter);
#endif
   policies::check_series_iterations<T>("nimboost::math::bessel_y_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
   result *= prefix;

   if(!need_logs)
   {
      prefix = nimboost::math::tgamma(-v, pol) * nimboost::math::cos_pi(v) * p / constants::pi<T>();
   }
   else
   {
      int sgn;
      prefix = nimboost::math::lgamma(-v, &sgn, pol) + p;
...
#if BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x582))
   T b = nimboost::math::tools::sum_series(s2, nimboost::math::policies::get_epsilon<T, Policy>(), max_iter, zero);
#else
   T b = nimboost::math::tools::sum_series(s2, nimboost::math::policies::get_epsilon<T, Policy>(), max_iter);
#endif
...
T bessel_yn_small_z(int n, T z, T* scale, const Policy& pol)
{
   ...
   {
      T p = pow(z / 2, n);
      T result = -((nimboost::math::factorial<T>(n - 1) / constants::pi<T>()));

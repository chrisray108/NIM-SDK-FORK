...
{
   T c = 1;
   T p = (v / nimboost::math::sin_pi(v, pol)) * pow(x / 2, -v) / nimboost::math::tgamma(1 - v, pol);
   T q = (v / nimboost::math::sin_pi(v, pol)) * pow(x / 2, v) / nimboost::math::tgamma(1 + v, pol);
   T f = (p - q) / v;
   T g_prefix = nimboost::math::sin_pi(v / 2, pol);
...
T asymptotic_bessel_i_large_x(T v, T x, const Policy& pol)
{
   ...
   return (nimboost::math::isfinite)(s) ? 
      s : policies::raise_overflow_error<T>("nimboost::math::asymptotic_bessel_i_large_x<%1%>(%1%,%1%)", 0, pol);
}

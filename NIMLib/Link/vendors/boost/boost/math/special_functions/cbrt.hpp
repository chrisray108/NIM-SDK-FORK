...
struct big_int_type
{
   operator nimboost::uintmax_t()const;
};
...
struct largest_cbrt_int_type
{
   typedef typename mpl::if_<
      nimboost::is_convertible<big_int_type, T>,
      nimboost::uintmax_t,
      unsigned int
   >::type type;
};
...
T cbrt_imp(T z, const Policy& pol)
{
   ...
   {
      return policies::raise_domain_error("nimboost::math::cbrt<%1%>(%1%)", "Argument to function must be finite but got %1%.", z, pol);
   }

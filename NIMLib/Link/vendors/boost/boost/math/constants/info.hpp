...
namespace nimboost{ namespace math{ namespace constants{
...
#pragma warning(disable:4127)
   ...
   if(precision_type::value)
   {
      ...
      {
       os <<
       "nimboost::math::policies::precision<" << nameof<T>() << ", " << nameof<Policy>() << " reports that the compile time precision is \n" << precision_type::value << " binary digits.\n";
      }
      else if (std::numeric_limits<T>::radix == 10)
      {
         os <<
   ...
   {
      os <<
         "nimboost::math::policies::precision<" << nameof<T>() << ", Policy> \n"
         "reports that there is no compile type precision available.\n"
         "nimboost::math::tools::digits<" << nameof<T>() << ">() \n"
         "reports that the current runtime precision is \n" <<
         nimboost::math::tools::digits<T>() << " binary digits.\n";
   }
   ...
   case 0:
      ...
      if(nimboost::math::tools::digits<T>() > max_string_digits)
      {
...
{
   print_info_on_type<T, nimboost::math::policies::policy<> >(os);
}

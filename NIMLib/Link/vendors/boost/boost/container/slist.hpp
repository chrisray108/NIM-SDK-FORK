...
struct intrusive_slist_type
{
   ...
   typedef typename nimboost::intrusive::pointer_traits
      ...
      <Allocator, typename container_detail::intrusive_slist_type<Allocator>::type>
{
   ...
   slist(BOOST_RV_REF(slist) x)
      : AllocHolder(nimboost::move(static_cast<AllocHolder&>(x)))
   {}
   ...
   slist& operator= (BOOST_RV_REF(slist) x)
   {
      if (&x != this){
         ...
         if(this_alloc == x_alloc){
            ...
            this->clear();
            this->icont() = nimboost::move(x.icont());
   ...
   {  this->emplace_after(this->cbefore_begin(), nimboost::forward<Args>(args)...); }
   ...
   {
      NodePtr pnode(AllocHolder::create_node(nimboost::forward<Args>(args)...));
   ...
   {  return this->emplace_after(this->previous(p), nimboost::forward<Args>(args)...);  }
   ...
   {  this->insert(this->cbegin(), ::nimboost::move(x));  }
   ...
   {  return this->insert_after(previous(p), ::nimboost::forward<U>(x)); }

   template<class U>
   iterator priv_insert_after(const_iterator prev_pos, BOOST_FWD_REF(U) x)
   {  return iterator(this->icont().insert_after(prev_pos.get(), *this->create_node(::nimboost::forward<U>(x)))); }
...
template <class T, class Allocator>
struct has_trivial_destructor_after_move<nimboost::container::slist<T, Allocator> >
...
template <class T, class Allocator>
class insert_iterator<nimboost::container::slist<T, Allocator> >
{
 protected:
   typedef nimboost::container::slist<T, Allocator> Container;
   Container* container;

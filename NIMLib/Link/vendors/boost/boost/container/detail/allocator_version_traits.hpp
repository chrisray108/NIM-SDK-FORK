...
template<class Allocator, unsigned Version = nimboost::container::container_detail::version<Allocator>::value>
...
struct allocator_version_traits<Allocator, 1>
{
   ...
   typedef typename nimboost::container::allocator_traits<Allocator>::pointer    pointer;
   typedef typename nimboost::container::allocator_traits<Allocator>::size_type  size_type;
   typedef typename nimboost::container::allocator_traits<Allocator>::value_type value_type;
   ...
   static void deallocate_individual(Allocator &a, multiallocation_chain &holder)
   {
      ...
      while(n--){
         pointer p = nimboost::intrusive::pointer_traits<pointer>::pointer_to(*it);
         ++it;
         a.deallocate(p, 1);
      }
   }

   ...
   :  public rbtree_hook<VoidPointer>::type
{
   ...
   {
      const_cast<A&>(m_data.first) = ::nimboost::move(p.first);
      m_data.second = ::nimboost::move(p.second);
   }
...
{
   typedef typename nimboost::container::
      ...
      >
{
   ...
   rbtree(BOOST_RV_REF(rbtree) x)
      :  AllocHolder(::nimboost::move(static_cast<AllocHolder&>(x)), x.value_comp())
   {}
   ...
   rbtree& operator=(BOOST_COPY_ASSIGN_REF(rbtree) x)
   {
      if (&x != this){
         ...
         //automatically
         Icont other_tree(::nimboost::move(this->icont()));
   ...
   {
      NodePtr tmp = AllocHolder::create_node(nimboost::forward<MovableConvertible>(mv));
      scoped_destroy_deallocator<NodeAlloc> destroy_deallocator(tmp, this->node_alloc());
      iterator ret(this->icont().insert_unique_commit(*tmp, data));
      destroy_deallocator.release();
      return ret;
   ...
   std::pair<iterator,bool> insert_unique(BOOST_FWD_REF(MovableConvertible) mv)
   {
      ...
      if(ret.second){
         ret.first = this->insert_unique_commit(nimboost::forward<MovableConvertible>(mv), data);
      }
      return ret;
   }
   ...
   {  return this->emplace_unique_impl(AllocHolder::create_node(nimboost::forward<Args>(args)...));   }

   template <class... Args>
   iterator emplace_hint_unique(const_iterator hint, Args&&... args)
   {  return this->emplace_unique_hint_impl(hint, AllocHolder::create_node(nimboost::forward<Args>(args)...));   }

   template <class... Args>
   iterator emplace_equal(Args&&... args)
   {
      NodePtr tmp(AllocHolder::create_node(nimboost::forward<Args>(args)...));
      scoped_destroy_deallocator<NodeAlloc> destroy_deallocator(tmp, this->node_alloc());
   ...
   iterator insert_unique(const_iterator hint, BOOST_FWD_REF(MovableConvertible) mv)
   {
      ...
      if(!ret.second)
         return ret.first;
      return this->insert_unique_commit(nimboost::forward<MovableConvertible>(mv), data);
   }
...
struct has_trivial_destructor_after_move
   <nimboost::container::container_detail::rbtree<K, V, KOV, C, A> >
{

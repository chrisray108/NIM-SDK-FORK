...
template<class T, bool IsClass = nimboost::is_class<T>::value >
struct has_member_swap
{
   static const bool value = nimboost::container::container_detail::
...
template<class T> inline
typename container_detail::enable_if_c
      <!container_swap::has_member_swap<T>::value && nimboost::has_move_emulation_enabled<T>::value, void>::type
   swap_dispatch(T &left, T &right)
{
   T temp(nimboost::move(left)); // may throw
   left = nimboost::move(right); // may throw
   right = nimboost::move(temp); // may throw
}
...
{  return nimboost::container::container_detail::to_raw_pointer(p.operator->());  }
...
{  nimboost::container::swap_dispatch(l, r);   }
...
template<class Pointer>
struct are_elements_contiguous<nimboost::container::container_detail::vector_iterator<Pointer> >
{
   static const bool value = true;
};

template<class Pointer>
struct are_elements_contiguous<nimboost::container::container_detail::vector_const_iterator<Pointer> >
{
   static const bool value = true;
};
...
template <class PointedType, class DifferenceType, class OffsetType, std::size_t OffsetAlignment>
struct are_elements_contiguous< ::nimboost::interprocess::offset_ptr<PointedType, DifferenceType, OffsetType, OffsetAlignment> >
{
   static const bool value = true;
};
...
struct is_memcpy_copy_assignable
{
   static const bool value = are_contiguous_and_same<I, O>::value &&
      nimboost::has_trivial_assign< typename ::std::iterator_traits<I>::value_type >::value;
};
...
{
   static const bool value = are_contiguous_and_same<I, O>::value &&
      nimboost::has_trivial_copy< typename ::std::iterator_traits<I>::value_type >::value;
};
...
//!   \code
//!   for (; f != l; ++r, ++f)
//!      allocator_traits::construct(a, &*r, nimboost::move(*f));
//!   \endcode
//!
//! <b>Returns</b>: r
template
   ...
   uninitialized_move_alloc(A &a, I f, I l, F r)
{
   ...
   BOOST_TRY{
      while (f != l) {
         allocator_traits<A>::construct(a, container_detail::to_raw_pointer(&*r), nimboost::move(*f));
         ++f; ++r;
      }
   }
   BOOST_CATCH(...){
...
inline void destroy_alloc_n(A &a, I f, typename std::iterator_traits<I>::difference_type n
   ,typename nimboost::container::container_detail::enable_if_c
      < !nimboost::has_trivial_destructor<typename std::iterator_traits<I>::value_type>::value >::type* = 0)
{
   while(n--){
                    ...
                    , G large_range_f, typename allocator_traits<A>::size_type n_j)
{
   ...
   for (; n != n_i ; ++short_range_f, ++large_range_f, ++n){
      nimboost::container::swap_dispatch(*short_range_f, *large_range_f);
   }
   nimboost::container::uninitialized_move_alloc_n(a, large_range_f, n_j - n_i, short_range_f);  // may throw
   nimboost::container::destroy_alloc_n(a, large_range_f, n_j - n_i);
}
...
{
   if (n_o < n_i){
      inp_start = nimboost::container::copy_n_source_dest(inp_start, n_o, out_start);     // may throw
      nimboost::container::uninitialized_copy_alloc_n(a, inp_start, n_i - n_o, out_start);// may throw
   }
   else{
      out_start = nimboost::container::copy_n(inp_start, n_i, out_start);  // may throw
      nimboost::container::destroy_alloc_n(a, out_start, n_o - n_i);
   }
}
...
{
   if (n_o < n_i){
      inp_start = nimboost::container::move_n_source_dest(inp_start, n_o, out_start);  // may throw
      nimboost::container::uninitialized_move_alloc_n(a, inp_start, n_i - n_o, out_start);  // may throw
   }
   else{
      out_start = nimboost::container::move_n(inp_start, n_i, out_start);  // may throw
      nimboost::container::destroy_alloc_n(a, out_start, n_o - n_i);
   }
}

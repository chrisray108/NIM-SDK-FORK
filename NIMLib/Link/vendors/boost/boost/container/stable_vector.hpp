...
{
   private:
   typedef typename nimboost::intrusive::
...
//   node();
   ...
   public:
   typename ::nimboost::intrusive::pointer_traits<Pointer>::element_type value;
};

template<typename Pointer, bool IsConst>
class iterator
{
   typedef nimboost::intrusive::pointer_traits<Pointer>                                non_const_ptr_traits;
   public:
   ...
   typedef typename ::nimboost::container::container_detail::if_c
      < IsConst
   ...
   {
      typedef nimboost::intrusive::pointer_traits<pointer> ptr_traits;
...
struct index_traits
{
   ...
   typedef ::nimboost::container::vector
...
class stable_vector
{
   ...
   typedef ::nimboost::container::container_detail::
      integral_constant<unsigned, 1>                  allocator_v1;
   typedef ::nimboost::container::container_detail::
      integral_constant<unsigned, 2>                  allocator_v2;
   typedef ::nimboost::container::container_detail::integral_constant
      <unsigned, nimboost::container::container_detail::
   ...
   stable_vector(BOOST_RV_REF(stable_vector) x)
      : internal_data(nimboost::move(x.priv_node_alloc())), index(nimboost::move(x.index))
   {
   ...
   stable_vector& operator=(BOOST_RV_REF(stable_vector) x)
   {
      if (&x != this){
         ...
         if(this_alloc == x_alloc){
            ...
            this->clear();
            this->index = nimboost::move(x.index);
            this->priv_swap_members(x);
   ...
   void emplace_back(Args &&...args)
   {
      ...
      EmplaceFunctor &&ef = EmplaceFunctor(nimboost::forward<Args>(args)...);
   ...
   void priv_push_back(BOOST_MOVE_CATCH_FWD(U) x)
   {
      if(this->priv_capacity_bigger_than_size()){
         ...
         BOOST_ASSERT(!!p);
         {
            ...
            //This might throw
            this->priv_build_node_from_convertible(p, ::nimboost::forward<U>(x));
            rollback.release();
         }
         ...
         this->insert(this->cend(), ::nimboost::forward<U>(x));
      }
   }
   ...
   iterator priv_insert(const_iterator position, BOOST_RV_REF(T) x)
   {
      ...
      typedef nimboost::move_iterator<repeat_it>      repeat_move_it;
      //Just call more general insert(pos, size, value) and return iterator
   ...
   {
      //This can throw
      nimboost::container::construct_in_place
   ...
   {
      //This can throw
      nimboost::container::allocator_traits<node_allocator_type>::construct
         ( this->priv_node_alloc()
         , container_detail::addressof(p->value)
         , ::nimboost::forward<ValueConvertible>(value_convertible));
      //This does not throw
   ...
   {
      nimboost::container::swap_dispatch(this->internal_data.pool_size, x.internal_data.pool_size);
      ...
      : public node_allocator_type
   {
      ...
      explicit ebo_holder(BOOST_FWD_REF(AllocatorRLValue) a)
         : node_allocator_type(nimboost::forward<AllocatorRLValue>(a))
         , pool_size(0)
         , end_node()
      {}
...
template <class T, class Allocator>
struct has_trivial_destructor_after_move<nimboost::container::stable_vector<T, Allocator> >

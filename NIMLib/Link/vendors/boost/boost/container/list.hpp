      ...
      <Allocator, typename container_detail::intrusive_list_type<Allocator>::type>
{
   ...
   {  this->emplace(this->cend(), nimboost::forward<Args>(args)...); }
   ...
   {  this->emplace(this->cbegin(), nimboost::forward<Args>(args)...);  }
   ...
   {
      NodePtr tmp = AllocHolder::create_node(nimboost::move(x));
      return iterator(this->icont().insert(p.get(), *tmp));
   }
   ...
   {  this->insert(this->cend(), nimboost::move(x));    }
...
template <class T, class Allocator>
struct has_trivial_destructor_after_move<nimboost::container::list<T, Allocator> >

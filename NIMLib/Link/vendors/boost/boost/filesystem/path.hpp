...
# endif
    ...
    path(Source const& source,
      typename nimboost::enable_if<path_traits::is_pathable<
        typename nimboost::decay<Source>::type> >::type* =0)
    {
    ...
    template <class Source>
      typename nimboost::enable_if<path_traits::is_pathable<
        typename nimboost::decay<Source>::type>, path&>::type
    ...
    template <class CharT>
      typename nimboost::enable_if<is_integral<CharT>, path&>::type
    operator+=(CharT c)
    {
      CharT tmp[2];
      tmp[0] = c;
...
# endif
  ...
  private:
    ...
    friend class nimboost::filesystem::path;
...
# endif
  ...
  //    use nimboost::io::quoted() to handle spaces in paths
  ...
  {
    return os
      << nimboost::io::quoted(p.template string<std::basic_string<Char> >(), static_cast<Char>('&'));
  }
  ...
  operator>>(std::basic_istream<Char, Traits>& is, path& p)
  {
    ...
    is >> nimboost::io::quoted(str, static_cast<Char>('&'));
    p = str;
    return is;
  }

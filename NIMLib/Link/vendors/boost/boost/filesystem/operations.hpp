...
# endif
  ...
  struct space_info
  {
    ...
    nimboost::uintmax_t capacity;
    nimboost::uintmax_t free;      // <= capacity
    nimboost::uintmax_t available; // <= free
  };
...
//--------------------------------------------------------------------------------------//
  ...
  namespace detail
  {
    ...
    BOOST_FILESYSTEM_DECL
    nimboost::uintmax_t file_size(const path& p, system::error_code* ec=0);
    BOOST_FILESYSTEM_DECL
    nimboost::uintmax_t hard_link_count(const path& p, system::error_code* ec=0);
    BOOST_FILESYSTEM_DECL
    ...
    BOOST_FILESYSTEM_DECL
    nimboost::uintmax_t remove_all(const path& p, system::error_code* ec=0);
    BOOST_FILESYSTEM_DECL
...
# endif
  ...
  inline
  nimboost::uintmax_t file_size(const path& p) {return detail::file_size(p);}

  inline
  nimboost::uintmax_t file_size(const path& p, system::error_code& ec)
                                       {return detail::file_size(p, &ec);}
  inline
  nimboost::uintmax_t hard_link_count(const path& p) {return detail::hard_link_count(p);}

  inline
  nimboost::uintmax_t hard_link_count(const path& p, system::error_code& ec)
  ...
  inline
  nimboost::uintmax_t remove_all(const path& p) {return detail::remove_all(p);}
    
  inline
  nimboost::uintmax_t remove_all(const path& p, system::error_code& ec)
...
public:
  ...
  directory_entry() {}
  explicit directory_entry(const nimboost::filesystem::path& p,
    file_status st = file_status(), file_status symlink_st=file_status())
    : m_path(p), m_status(st), m_symlink_status(symlink_st)
    {}

  void assign(const nimboost::filesystem::path& p,
    file_status st = file_status(), file_status symlink_st = file_status())
    { m_path = p; m_status = st; m_symlink_status = symlink_st; }

  void replace_filename(const nimboost::filesystem::path& p,
    file_status st = file_status(), file_status symlink_st = file_status())
  {
...
# ifndef BOOST_FILESYSTEM_NO_DEPRECATED
  void replace_leaf(const nimboost::filesystem::path& p,
    file_status st, file_status symlink_st)
      { replace_filename(p, st, symlink_st); }
# endif

  const nimboost::filesystem::path&  path() const               {return m_path;}
...
private:
  nimboost::filesystem::path   m_path;
...
//--------------------------------------------------------------------------------------//
  ...
  class directory_iterator
    : public nimboost::iterator_facade< directory_iterator,
                                     directory_entry,
  ...
  private:
    ...
    nimboost::shared_ptr< detail::dir_itr_imp >  m_imp;

    friend class nimboost::iterator_core_access;

    nimboost::iterator_facade<
      directory_iterator,
      directory_entry,
      nimboost::single_pass_traversal_tag >::reference dereference() const 
    {
...
//--------------------------------------------------------------------------------------//
  ...
  namespace detail
  {
    ...
    //  clients of struct 'nimboost::filesystem::detail::recur_dir_itr_imp'
...
//--------------------------------------------------------------------------------------//
  ...
  class recursive_directory_iterator
    : public nimboost::iterator_facade<
        recursive_directory_iterator,
        directory_entry,
        nimboost::single_pass_traversal_tag >
  {
...
#   endif
  ...
  private:
    ...
    // shared_ptr provides shallow-copy semantics required for InputIterators.
    // m_imp.get()==0 indicates the end iterator.
    nimboost::shared_ptr< detail::recur_dir_itr_imp >  m_imp;
    ...
    nimboost::iterator_facade< 
      recursive_directory_iterator,
      directory_entry,
      nimboost::single_pass_traversal_tag >::reference
    dereference() const 
    {
      BOOST_ASSERT_MSG(m_imp.get(),
...
//--------------------------------------------------------------------------------------//
  ...
  private:
    ...
    nimboost::shared_ptr<m_imp> m_imp_ptr;
  };

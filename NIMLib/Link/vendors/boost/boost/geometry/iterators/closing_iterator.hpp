...
struct closing_iterator
    ...
    <
        ...
        typename nimboost::range_value<Range>::type const,
    ...
    >
{
    ...
    explicit inline closing_iterator(Range& range)
        : m_range(&range)
        , m_iterator(nimboost::begin(range))
        , m_end(nimboost::end(range))
        , m_size(nimboost::size(range))
        , m_index(0)
    {}
...
private:
    ...
    inline typename nimboost::range_value<Range>::type const& dereference() const
    {
        return *m_iterator;
    }
    ...
    inline void update_iterator()
    {
        this->m_iterator = m_index <= m_size
            ? nimboost::begin(*m_range) + (m_index % m_size)
            : nimboost::end(*m_range)
            ;
    }

    Range* m_range;
    typename nimboost::range_iterator<Range>::type m_iterator;
    typename nimboost::range_iterator<Range>::type m_end;

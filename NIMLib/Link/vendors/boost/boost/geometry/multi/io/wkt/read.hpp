...
struct multi_parser
{
    static inline void apply(std::string const& wkt, MultiGeometry& geometry)
    {
        ...
        if (initialize<MultiGeometry>(tokens, PrefixPolicy::apply(), wkt, it))
        {
            ...
            {
                traits::resize<MultiGeometry>::apply(geometry, nimboost::size(geometry) + 1);
                Parser
                    <
                        typename nimboost::range_value<MultiGeometry>::type
                    >::apply(it, tokens.end(), wkt, *(nimboost::end(geometry) - 1));
                if (it != tokens.end() && *it == ",")
                {

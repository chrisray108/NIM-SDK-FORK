...
namespace dispatch
{
    ...
    struct select_rings<multi_polygon_tag, Multi>
    {
                    ...
                    ring_identifier id, Map& map, bool midpoint)
        {
            ...
            typedef select_rings<polygon_tag, typename nimboost::range_value<Multi>::type> per_polygon;

            id.multi_index = 0;
            for (iterator_type it = nimboost::begin(multi); it != nimboost::end(multi); ++it)
            {
                id.ring_index = -1;

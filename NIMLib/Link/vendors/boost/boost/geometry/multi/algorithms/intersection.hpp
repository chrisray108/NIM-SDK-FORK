...
struct intersection_multi_linestring_multi_linestring_point
{
            ...
            Strategy const& strategy)
    {
        ...
        for (typename nimboost::range_iterator
                ...
                >::type it1 = nimboost::begin(ml1);
            it1 != nimboost::end(ml1);
            ++it1)
        {
            for (typename nimboost::range_iterator
                    <
                        MultiLinestring2 const
                    >::type it2 = nimboost::begin(ml2);
                it2 != nimboost::end(ml2);
                ++it2)
            {
...
struct clip_multi_linestring
{
            ...
            Box const& box, OutputIterator out, Strategy const& )
    {
        ...
        for (typename nimboost::range_iterator<MultiLinestring const>::type it
            = nimboost::begin(multi_linestring);
            it != nimboost::end(multi_linestring); ++it)
        {

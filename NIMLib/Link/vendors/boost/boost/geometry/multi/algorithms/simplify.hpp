...
struct simplify_multi
{
    ...
    {
        traits::resize<MultiGeometry>::apply(out, nimboost::size(multi));

        typename nimboost::range_iterator<MultiGeometry>::type it_out
                = nimboost::begin(out);
        for (typename nimboost::range_iterator<MultiGeometry const>::type it_in
                    = nimboost::begin(multi);
            it_in != nimboost::end(multi);
            ++it_in, ++it_out)
        {

...
private :
    ...
    typedef typename
        nimboost::mpl::if_c
        <
            nimboost::is_void<CalculationType>::type::value,
...
public :
            ...
            PointOfSegment const& p2, sums& state)
    {
        ...
        */
        ...
        calculation_type const x1 = nimboost::numeric_cast<calculation_type>(get<0>(p1));
        calculation_type const y1 = nimboost::numeric_cast<calculation_type>(get<1>(p1));
        calculation_type const x2 = nimboost::numeric_cast<calculation_type>(get<0>(p2));
        calculation_type const y2 = nimboost::numeric_cast<calculation_type>(get<1>(p2));
    ...
    static inline bool result(sums const& state, Point& centroid)
    {
        ...
        if (state.count > 0 && ! math::equals(state.sum_a2, zero))
        {
            ...
            set<0>(centroid,
                nimboost::numeric_cast<coordinate_type>(state.sum_x / a3));
            set<1>(centroid,
                nimboost::numeric_cast<coordinate_type>(state.sum_y / a3));
            return true;
        }

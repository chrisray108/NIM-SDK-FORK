...
public :
                    ...
                    OutputIterator out, double max_distance)
    {
        ...
        std::vector<dp_point_type> ref_candidates(nimboost::begin(range),
                        nimboost::end(range));
        ...
        consider(nimboost::begin(ref_candidates), nimboost::end(ref_candidates), max_distance, n, strategy);

        // Copy included elements to the output
        for(typename std::vector<dp_point_type>::const_iterator it
                        = nimboost::begin(ref_candidates);
            it != nimboost::end(ref_candidates);
            ++it)
        {
            if (it->included)
            {

...
struct get_extremes
{
    ...
    inline void apply(InputRange const& range)
    {
        ...
        RangeIterator left_it = nimboost::begin(range);
        RangeIterator right_it = nimboost::begin(range);

        for (RangeIterator it = nimboost::begin(range) + 1;
...
struct assign_range
{
    ...
    inline void apply(InputRange const& range)
    {
        ...
        for (RangeIterator it = nimboost::begin(range);
            it != nimboost::end(range);
            ++it)
        {
...
{
    typedef typename nimboost::range_value<Range>::type point_type;
    typedef geometry::less<point_type> comparator;

    std::sort(nimboost::begin(range), nimboost::end(range), comparator());
}
...
public:
    ...
    inline void apply(InputGeometry const& geometry, partitions& state) const
    {
        ...
        //std::cout << nimboost::size(assigner.lower_points) << std::endl;
        //std::cout << nimboost::size(assigner.upper_points) << std::endl;
...
private:
        ...
        OutputIterator out, bool skip_first)
    {
        ...
        bool first = true;
        for (typename nimboost::range_iterator<view_type const>::type it = nimboost::begin(view);
            it != nimboost::end(view); ++it)
        {
            if (first && skip_first)
            {
                first = false;

...
struct disjoint_linestring_box
{
    static inline bool apply(Linestring const& linestring, Box const& box)
    {
        ...
        const size_type count = ::nimboost::size(linestring);

        if ( count == 0 )
            return false;
        else if ( count == 1 )
            return detail::disjoint::point_box<point_type, Box, 0, dimension<point_type>::value>
                   ::apply(*::nimboost::begin(linestring), box);
        else
        {
            const_iterator it0 = ::nimboost::begin(linestring);
            const_iterator it1 = ::nimboost::begin(linestring) + 1;
            const_iterator last = ::nimboost::end(linestring);

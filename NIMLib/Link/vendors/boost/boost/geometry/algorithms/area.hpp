...
struct ring_area
{
    ...
    apply(Ring const& ring, Strategy const& strategy)
    {
        ...
        nimboost::ignore_unused_variable_warning(strategy);

        // An open ring has at least three points,
        // A closed ring has at least four points,
        // if not, there is no (zero) area
        if (int(nimboost::size(ring))
        ...
        typedef typename closeable_view
            ...
            >::type view_type;
        typedef typename nimboost::range_iterator<view_type const>::type iterator_type;

        rview_type rview(ring);
        view_type view(rview);
        typename Strategy::state_type state;
        iterator_type it = nimboost::begin(view);
        iterator_type end = nimboost::end(view);
...
struct devarianted_area<nimboost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> >
{
    template <typename Strategy>
    struct visitor: nimboost::static_visitor<typename Strategy::return_type>
    {
    ...
    template <typename Strategy>
    static inline typename Strategy::return_type
    apply(nimboost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> const& geometry,
          Strategy const& strategy)
    {
        return nimboost::apply_visitor(visitor<Strategy>(strategy), geometry);
    }
};

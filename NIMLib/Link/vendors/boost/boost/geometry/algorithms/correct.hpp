...
struct correct_ring
{
    ...
    {
        // Check close-ness
        if (nimboost::size(r) > 2)
        {
            // check if closed, if not, close it
            bool const disjoint = geometry::disjoint(*nimboost::begin(r), *(nimboost::end(r) - 1));
            closure_selector const s = geometry::closure<Ring>::value;

            if (disjoint && (s == closed))
            {
                geometry::append(r, *nimboost::begin(r));
            }
            if (! disjoint && s != closed)
            {
                // Open it by removing last point
                geometry::traits::resize<Ring>::apply(r, nimboost::size(r) - 1);
            }
        }
        // Check area
        Predicate predicate;
        ...
        {
            std::reverse(nimboost::begin(r), nimboost::end(r));
        }
    }
};

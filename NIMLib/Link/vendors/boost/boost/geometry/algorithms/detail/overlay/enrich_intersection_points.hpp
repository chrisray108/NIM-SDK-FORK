...
inline void update_discarded(Turns& turn_points, Operations& operations)
{
    ...
    for (typename nimboost::range_iterator<Operations>::type it = nimboost::begin(operations);
         it != nimboost::end(operations);
         ++it)
    {
            ...
            Strategy const& strategy)
{
    ...
    std::sort(nimboost::begin(operations),
                nimboost::end(operations),
    ...
    if (clustered)
    {
        typedef typename nimboost::range_iterator<Container>::type nc_iterator;
        nc_iterator it = nimboost::begin(operations);
        nc_iterator begin_cluster = nimboost::end(operations);
            ...
            prev = it++)
        {
            ...
            {
                if (begin_cluster == nimboost::end(operations))
                {
                    begin_cluster = prev;
                }
            }
            else if (begin_cluster != nimboost::end(operations))
            {
                handle_cluster<IndexType, Reverse1, Reverse2>(begin_cluster, it, turn_points,
                        for_operation, geometry1, geometry2, strategy);
                begin_cluster = nimboost::end(operations);
            }
        }
        if (begin_cluster != nimboost::end(operations))
        {
...
inline void enrich_discard(Container& operations, TurnPoints& turn_points)
{
    ...
    operations.erase(
            std::remove_if(nimboost::begin(operations),
                    nimboost::end(operations),
                    predicate),
            nimboost::end(operations));
}
...
{
    typedef typename IndexType::type operations_type;
    typedef typename nimboost::range_iterator<Container const>::type iterator_type;
    ...
    if (operations.size() > 0)
    {
        ...
        bool first = true;
        iterator_type it = nimboost::begin(operations);
        for (iterator_type prev = it + (nimboost::size(operations) - 1);
             it != nimboost::end(operations);
             prev = it++)
        {
...
#ifdef BOOST_GEOMETRY_DEBUG_ENRICH
    {
        for (iterator_type it = nimboost::begin(operations);
             it != nimboost::end(operations);
             ++it)
        {
            operations_type& op = turn_points[it->index]
            ...
            std::cout << it->index
                ...
                << " seg: " << op.seg_id
                << " dst: " << nimboost::numeric_cast<double>(op.enriched.distance)
...
{
    typedef typename nimboost::range_value<TurnPoints>::type turn_point_type;
    typedef typename turn_point_type::container_type container_type;

    int index = 0;
    for (typename nimboost::range_iterator<TurnPoints const>::type
            it = nimboost::begin(turn_points);
         it != nimboost::end(turn_points);
         ++it, ++index)
    {
        ...
        {
            int op_index = 0;
            for (typename nimboost::range_iterator<container_type const>::type
                    op_it = nimboost::begin(it->operations);
                op_it != nimboost::end(it->operations);
                ++op_it, ++op_index)
            {
    ...
    Strategy const& strategy)
{
    ...
    for (typename nimboost::range_iterator<TurnPoints>::type
            it = nimboost::begin(turn_points);
         it != nimboost::end(turn_points);
         ++it)
    {

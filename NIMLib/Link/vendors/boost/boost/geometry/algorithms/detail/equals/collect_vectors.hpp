...
{
    typedef typename nimboost::range_value<Collection>::type item_type;
    typedef typename item_type::type calculation_type;

    static inline void apply(Collection& collection, Range const& range)
    {
        if (nimboost::size(range) < 2)
        {
            return;
        }

        typedef typename nimboost::range_iterator<Range const>::type iterator;

        bool first = true;
        iterator it = nimboost::begin(range);

        for (iterator prev = it++;
            it != nimboost::end(range);
            prev = it++)
        {
            typename nimboost::range_value<Collection>::type v;
            ...
            calculation_type magnitude = sqrt(
                nimboost::numeric_cast<calculation_type>(v.dx * v.dx + v.dy * v.dy));
...
struct box_collect_vectors
{
    ...
    static inline void apply(Collection& collection, Box const& box)
    {
        ...
        typedef typename nimboost::range_value<Collection>::type item;
...
struct polygon_collect_vectors
{
    static inline void apply(Collection& collection, Polygon const& polygon)
    {
        ...
        for (BOOST_AUTO_TPL(it, nimboost::begin(rings)); it != nimboost::end(rings); ++it)
        {
...
struct multi_collect_vectors
{
    ...
    {
        for (typename nimboost::range_iterator<MultiGeometry const>::type
                it = nimboost::begin(multi);
            it != nimboost::end(multi);
            ++it)
        {
...
struct collect_vectors<multi_polygon_tag, Collection, MultiPolygon>
    : detail::collect_vectors::multi_collect_vectors
        <
            ...
            <
                typename nimboost::range_value<MultiPolygon>::type,
                Collection
            >
        >
{};

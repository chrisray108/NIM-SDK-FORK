...
{
    typedef nimboost::range_iterator
        <
            index_vector_type const
        >::type index_iterator_type;

    for(index_iterator_type it = nimboost::begin(input);
        it != nimboost::end(input);
        ++it)
    {
...
static inline void handle_one(InputCollection const& collection,
        ...
        Policy& policy)
{
    typedef nimboost::range_iterator<index_vector_type const>::type
                index_iterator_type;
    // Quadratic behaviour at lowest level (lowest quad, or all exceeding)
    for(index_iterator_type it1 = nimboost::begin(input);
        it1 != nimboost::end(input);
        ++it1)
    {
        index_iterator_type it2 = it1;
        for(++it2; it2 != nimboost::end(input); ++it2)
        {
        ...
        Policy& policy)
{
    ...
    for(index_iterator_type it1 = nimboost::begin(input1);
        it1 != nimboost::end(input1);
        ++it1)
    {
        for(index_iterator_type it2 = nimboost::begin(input2);
            it2 != nimboost::end(input2);
            ++it2)
        {
...
class partition_one_collection
{
    ...
    {
        if (nimboost::size(input) > 0)
        {
            if (std::size_t(nimboost::size(input)) > min_elements && level < 100)
            {
...
class partition_two_collections
{
    ...
    static inline void next_level(Box const& box,
            ...
            Policy& policy, VisitBoxPolicy& box_policy)
    {
        if (nimboost::size(input1) > 0 && nimboost::size(input2) > 0)
        {
            if (std::size_t(nimboost::size(input1)) > min_elements
                && std::size_t(nimboost::size(input2)) > min_elements
                && level < 100)
            {
...
class partition
{
    ...
    {
        std::size_t index = 0;
        for(typename nimboost::range_iterator<InputCollection const>::type it
            = nimboost::begin(collection);
            it != nimboost::end(collection);
            ++it, ++index)
        {
...
public :
    ...
    {
        if (std::size_t(nimboost::size(collection)) > min_elements)
        {
        ...
        {
            typedef typename nimboost::range_iterator
                <
                    InputCollection const
                >::type iterator_type;
            for(iterator_type it1 = nimboost::begin(collection);
                it1 != nimboost::end(collection);
                ++it1)
            {
                iterator_type it2 = it1;
                for(++it2; it2 != nimboost::end(collection); ++it2)
                {
    ...
    {
        if (std::size_t(nimboost::size(collection1)) > min_elements
            && std::size_t(nimboost::size(collection2)) > min_elements)
        {
        ...
        else
        {
            typedef typename nimboost::range_iterator
                <
                    InputCollection const
                >::type iterator_type;
            for(iterator_type it1 = nimboost::begin(collection1);
                it1 != nimboost::end(collection1);
                ++it1)
            {
                for(iterator_type it2 = nimboost::begin(collection2);
                    it2 != nimboost::end(collection2);
                    ++it2)
                {
                    visitor.apply(*it1, *it2);
                }

...
inline Iterator advance_circular(Iterator it, Vector const& vector, segment_identifier& seg_id, bool forward = true)
{
    ...
    if (it == nimboost::begin(vector) && increment < 0)
    {
        it = nimboost::end(vector);
        seg_id.segment_index = nimboost::size(vector);
    }
    it += increment;
    seg_id.segment_index += increment;
    if (it == nimboost::end(vector))
    {
        seg_id.segment_index = 0;
        it = nimboost::begin(vector);
    }
    return it;
}
                ...
                Info& info)
{
    ...
    int const n = nimboost::size(ring);
    ...
    iterator_type it = nimboost::begin(ring) + seg_id.segment_index;

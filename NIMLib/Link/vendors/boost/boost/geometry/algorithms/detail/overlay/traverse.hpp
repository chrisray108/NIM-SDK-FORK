                ...
                std::string const& header)
{
    ...
    if (nimboost::contains(header, "Finished"))
    {
        std::cout << std::endl;
    }
}
...
inline void set_visited_for_continue(Info& info, Turn const& turn)
{
    ...
    {
        for (typename nimboost::range_iterator
            <
                typename Info::container_type
            >::type it = nimboost::begin(info.operations);
            it != nimboost::end(info.operations);
            ++it)
        {
            if (it->visited.none())
            {
...
inline bool assign_next_ip(G1 const& g1, G2 const& g2,
            Turns& turns,
            typename nimboost::range_iterator<Turns>::type& ip,
            ...
            segment_identifier& seg_id)
{
    ...
    if (info.enriched.next_ip_index < 0)
    {
        ...
        seg_id = info.seg_id;
        ip = nimboost::begin(turns) + info.enriched.travels_to_ip_index;
    }
    else
    {
        ip = nimboost::begin(turns) + info.enriched.next_ip_index;
        seg_id = info.seg_id;
    }
            ...
            Iterator& selected)
{
    ...
    bool has_tp = false;
    selected = nimboost::end(turn.operations);
    for (Iterator it = nimboost::begin(turn.operations);
        it != nimboost::end(turn.operations);
        ++it)
    {
...
public :
    ...
    {
        typedef typename nimboost::range_value<Rings>::type ring_type;
        typedef typename nimboost::range_iterator<Turns>::type turn_iterator;
        typedef typename nimboost::range_value<Turns>::type turn_type;
        ...
        std::size_t size_at_start = nimboost::size(rings);
        ...
        do
        {
            ...
            for (turn_iterator it = nimboost::begin(turns);
                state.good() && it != nimboost::end(turns);
                ++it)
            {
                // Skip discarded ones
                if (! (it->is_discarded() || it->blocked()))
                {
                    for (turn_operation_iterator_type iit = nimboost::begin(it->operations);
                        state.good() && iit != nimboost::end(it->operations);
                        ++iit)
                    {
                        if (iit->visited.none()

...
struct intersection_linestring_linestring_point
{
            ...
            Strategy const& )
    {
        ...
        for (typename nimboost::range_iterator<std::deque<turn_info> const>::type
            it = nimboost::begin(turns); it != nimboost::end(turns); ++it)
        {
            PointOut p;

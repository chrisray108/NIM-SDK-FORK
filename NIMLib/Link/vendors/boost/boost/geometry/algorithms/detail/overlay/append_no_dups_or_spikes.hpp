...
inline void append_no_dups_or_spikes(Range& range, Point const& point)
{
    ...
    while(nimboost::size(range) >= 3 
            && point_is_spike_or_equal(point, *(nimboost::end(range) - 3), *(nimboost::end(range) - 2)))
    {
        // Use the Concept/traits, so resize and append again
        traits::resize<Range>::apply(range, nimboost::size(range) - 2);
        traits::push_back<Range>::apply(range, point);
    }

    // There might still be one duplicate not catched by the condition above
    if (nimboost::size(range) == 2
        && geometry::detail::equals::equals_point_point(*nimboost::begin(range), point))
    {

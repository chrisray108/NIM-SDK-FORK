...
public :
                ...
                Turns& turns, OutputIterator out)
    {
        ...
        std::sort(nimboost::begin(turns), nimboost::end(turns), sort_on_segment<turn_type>());
        ...
        bool entered = false;
        bool first = true;
        for (turn_iterator it = nimboost::begin(turns); it != nimboost::end(turns); ++it)
        {
            turn_operation_iterator_type iit = nimboost::begin(it->operations);

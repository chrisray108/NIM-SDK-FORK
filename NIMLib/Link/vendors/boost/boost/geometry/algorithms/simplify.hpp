...
struct simplify_range_insert
{
    ...
    {
        if (nimboost::size(range) <= 2 || max_distance < 0)
        {
            std::copy(nimboost::begin(range), nimboost::end(range), out);
        }
        else
        {
...
struct simplify_copy
{
                             ...
                             Distance const& , Strategy const& )
    {
        std::copy
            (
                nimboost::begin(range), nimboost::end(range), std::back_inserter(out)
            );
    }
};
...
struct simplify_range
{
                    ...
                    Distance const& max_distance, Strategy const& strategy)
    {
        ...
        */
        ...
        if (nimboost::size(range) <= int(Minimum) || max_distance < 0.0)
        {
...
struct simplify_polygon
{
                    ...
                    Distance const& max_distance, Strategy const& strategy)
    {
        ...
        BOOST_AUTO_TPL(it_out, nimboost::begin(rings_out));
        for (BOOST_AUTO_TPL(it_in,  nimboost::begin(rings_in));
            it_in != nimboost::end(rings_in);
            ++it_in, ++it_out)
        {

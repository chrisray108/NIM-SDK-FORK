...
struct point_to_box
{
    static inline void apply(Point const& point, Box& box)
    {
        ...
        set<Index, Dimension>(box,
                nimboost::numeric_cast<coordinate_type>(get<Dimension>(point)));
        point_to_box
            <
                Point, Box,
...
struct segment_to_range
{
    static inline void apply(Segment const& segment, Range& range)
    {
        ...
        typename nimboost::range_iterator<Range>::type it = nimboost::begin(range);
...
struct range_to_range
{
    ...
    static inline void apply(Range1 const& source, Range2& destination)
    {
        ...
        int i = 0;
        for (typename nimboost::range_iterator<view_type const>::type it
            = nimboost::begin(view);
            it != nimboost::end(view) && i < n;
            ++it, ++i)
        {
...
struct polygon_to_polygon
{
    ...
    static inline void apply(Polygon1 const& source, Polygon2& destination)
    {
        ...
        BOOST_AUTO_TPL(it_source, nimboost::begin(rings_source));
        BOOST_AUTO_TPL(it_dest, nimboost::begin(rings_dest));

        for ( ; it_source != nimboost::end(rings_source); ++it_source, ++it_dest)
        {
...
<
    ...
    bool UseAssignment = nimboost::is_same<Geometry1, Geometry2>::value
                         && !nimboost::is_array<Geometry1>::value
>
...
struct devarianted_convert<nimboost::variant<BOOST_VARIANT_ENUM_PARAMS(T)>, Geometry2>
{
    ...
    static inline void apply(
        nimboost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> const& geometry1,
        Geometry2& geometry2
    )
    {

...
struct range_count
{
    ...
    {
        std::size_t n = nimboost::size(range);
        if (add_for_open && n > 0)
        {
            if (geometry::closure<Range>::value == open)
            {
                if (geometry::disjoint(*nimboost::begin(range), *(nimboost::begin(range) + n - 1)))
                {
                    return n + 1;
                }
            }
...
struct devarianted_num_points<nimboost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> >
{
    struct visitor: nimboost::static_visitor<std::size_t>
    {
        bool m_add_for_open;
    ...
    apply(nimboost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> const& geometry,
          bool add_for_open)
    {
        return nimboost::apply_visitor(visitor(add_for_open), geometry);
    }
};

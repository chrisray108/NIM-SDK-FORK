...
inline bool has_only_turns(Turns const& turns)
{
    ...
    typedef typename nimboost::range_iterator<Turns const>::type iterator_type;
    for (iterator_type it = nimboost::begin(turns); it != nimboost::end(turns); ++it)
    {

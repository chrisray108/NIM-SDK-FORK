...
struct fe_range_per_point
{
    ...
    static inline void apply(Range& range, Functor& f)
    {
        ...
        // return (std::for_each(nimboost::begin(range), nimboost::end(range), f));
        ...
        for (typename nimboost::range_iterator<Range>::type it = nimboost::begin(range); it != nimboost::end(range); ++it)
        {
            f(*it);
        }
    }
...
struct fe_range_per_segment
{
    ...
    static inline void apply(Range& range, Functor& f)
    {
        ...
        BOOST_AUTO_TPL(it, nimboost::begin(range));
        BOOST_AUTO_TPL(previous, it++);
        while(it != nimboost::end(range))
        {

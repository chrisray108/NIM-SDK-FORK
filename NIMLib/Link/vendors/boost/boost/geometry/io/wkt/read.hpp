...
typedef nimboost::tokenizer<nimboost::char_separator<char> > tokenizer;
...
struct parsing_assigner
{
            ...
            Point& point, std::string const& wkt)
    {
        ...
        catch(nimboost::bad_lexical_cast const& blc)
        {
...
struct container_appender
{
    typedef typename geometry::point_type
        <
            typename nimboost::remove_reference<Geometry>::type
        >::type point_type;
...
{
    it = tokens.begin();
    if (it != tokens.end() && nimboost::iequals(*it++, geometry_name))
    {
...
struct geometry_parser
{
    static inline void apply(std::string const& wkt, Geometry& geometry)
    {
        ...
        tokenizer tokens(wkt, nimboost::char_separator<char>(" ", ",()"));
        tokenizer::iterator it;
...
struct box_parser
{
    static inline void apply(std::string const& wkt, Box& box)
    {
        ...
        if (it != end && nimboost::iequals(*it, "POLYGON"))
        {
            ++it;
            bool has_empty, has_z, has_m;
        ...
        else if (it != end && nimboost::iequals(*it, "BOX"))
        {
            ++it;
        }
        else
...
struct segment_parser
{
    static inline void apply(std::string const& wkt, Segment& segment)
    {
        ...
        tokenizer::iterator end = tokens.end();
        if (it != end &&
            (nimboost::iequals(*it, "SEGMENT")
            || nimboost::iequals(*it, "LINESTRING") ))
        {
            ++it;
        }
        else

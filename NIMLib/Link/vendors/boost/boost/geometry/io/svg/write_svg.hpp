...
struct svg_range
{
        ...
        Range const& range, std::string const& style, int )
    {
        ...
        for (iterator it = nimboost::begin(range);
...
struct svg_poly
{
        ...
        Polygon const& polygon, std::string const& style, int )
    {
        ...
        typedef typename nimboost::range_iterator<ring_type const>::type iterator_type;

        bool first = true;
        os << "<g fill-rule=\"evenodd\"><path d=\"";

        ring_type const& ring = geometry::exterior_ring(polygon);
        for (iterator_type it = nimboost::begin(ring);
            it != nimboost::end(ring);
            ++it, first = false)
        {
            os << (first ? "M" : " L") << " "
                << geometry::get<0>(*it)
        ...
        // Inner rings:
        {
            ...
            for (BOOST_AUTO_TPL(rit, nimboost::begin(rings));
                rit != nimboost::end(rings); ++rit)
            {
                first = true;
                for (BOOST_AUTO_TPL(it, nimboost::begin(*rit)); it != nimboost::end(*rit);
                    ++it, first = false)
                {
                    os << (first ? "M" : " L") << " "
                        << geometry::get<0>(*it)

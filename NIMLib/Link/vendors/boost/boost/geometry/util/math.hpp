...
struct equals<Type, true>
{
    ...
    static inline bool apply(Type const& a, Type const& b)
    {
        ...
        // FUTURE: replace by some boost tool or nimboost::test::close_at_tolerance
...
struct define_pi
{
    static inline T apply()
    {
        ...
        return nimboost::math::constants::pi<T>();
    }
};
...
inline bool equals(T1 const& a, T2 const& b)
{
    ...
    return detail::equals
        <
            select_type,
            nimboost::is_floating_point<select_type>::type::value
        >::apply(a, b);
}

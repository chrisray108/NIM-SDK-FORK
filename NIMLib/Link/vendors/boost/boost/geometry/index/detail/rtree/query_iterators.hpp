...
namespace nimboost { namespace geometry { namespace index { namespace detail { namespace rtree { namespace iterators {
...
public:
    ...
    const value_type * operator->() const
    {
        return nimboost::addressof(m_visitor.dereference());
    }
...
}}}}}} // namespace nimboost::geometry::index::detail::rtree::iterators
...
public:
    ...
    {
        const query_iterator_wrapper * p = dynamic_cast<const query_iterator_wrapper *>(nimboost::addressof(r));
...
public:
    ...
    nimboost::function<query_iterator_base*()> clone;
    nimboost::function<bool()> is_end;
    nimboost::function<reference()> dereference;
    nimboost::function<void()> increment;
    nimboost::function<bool(query_iterator_base const&)> equals;
};
...
public:
    ...
    explicit query_iterator_wrapper(Iterator const& it)
        : m_iterator(it)
    {
        base_t::clone = nimboost::bind(&query_iterator_wrapper::clone_, this);
        base_t::is_end = nimboost::bind(&query_iterator_wrapper::is_end_, this);
        base_t::dereference = nimboost::bind(&query_iterator_wrapper::dereference_, this);
        base_t::increment = nimboost::bind(&query_iterator_wrapper::increment_, this);
        base_t::equals = nimboost::bind(&query_iterator_wrapper::equals_, this, _1);
    }
...
public:
    ...
    const value_type * operator->() const
    {
        return nimboost::addressof(m_ptr->dereference());
    }
...
struct single_pass_iterator_concept :
    ::nimboost::mpl::vector<
        ::nimboost::type_erasure::copy_constructible<T>,
        ::nimboost::type_erasure::equality_comparable<T>,
        ::nimboost::type_erasure::dereferenceable<typename Allocators::const_reference, T>,
        ::nimboost::type_erasure::assignable<T>,
        ::nimboost::type_erasure::incrementable<T>,
        ::nimboost::type_erasure::equality_comparable<T, end_query_iterator<Value, Allocators> >,
        ::nimboost::type_erasure::relaxed // default ctor
    >
{};
...
{
    typedef ::nimboost::type_erasure::any<
        single_pass_iterator_concept<
            ::nimboost::type_erasure::_self, Value, Allocators
        >
    > type;
};
...
namespace nimboost { namespace type_erasure {

template<typename T, typename Value, typename Allocators, typename Base>
struct concept_interface<
    ::nimboost::geometry::index::detail::rtree::single_pass_iterator_concept<
        T, Value, Allocators
    >, Base, T>
    : Base
{
...
public:
    ...
    query_iterator(BOOST_RV_REF(query_iterator) o)
        : m_iterator(nimboost::move(o.m_iterator))
    {}
    query_iterator & operator=(BOOST_RV_REF(query_iterator) o)
    {
        if ( this != nimboost::addressof(o) )
    ...
    const value_type * operator->() const
    {
        return nimboost::addressof(*m_iterator);
    }
    ...
    friend bool operator==(query_iterator const& l, query_iterator const& r)
    {
        if ( !::nimboost::type_erasure::is_empty(l.m_iterator) )
        {
            if ( !::nimboost::type_erasure::is_empty(r.m_iterator) )

...
struct varray_traits
{
    ...
    typedef nimboost::false_type use_memop_in_swap_and_move;
    typedef nimboost::false_type use_optimized_swap;
    typedef nimboost::false_type disable_trivial_init;
};
...
struct checker
{
    ...
    static inline void check_capacity(Varray const& v, size_type s)
    {
        ...
        ::nimboost::ignore_unused_variable_warning(v);
        ::nimboost::ignore_unused_variable_warning(s);
    }
...
//#endif // BOOST_NO_EXCEPTIONS
        ...
        ::nimboost::ignore_unused_variable_warning(v);
        ::nimboost::ignore_unused_variable_warning(i);
    }
    ...
    static inline void check_iterator_end_neq(Varray const& v, const_iterator position)
    {
        ...
        ::nimboost::ignore_unused_variable_warning(v);
        ::nimboost::ignore_unused_variable_warning(position);
    }
...
varray is a sequence container like nimboost::container::vector with contiguous storage that can
change in size, along with the static allocation, low overhead, and fixed capacity of nimboost::array.
...
class varray
{
    ...
    BOOST_MPL_ASSERT_MSG(
        ( nimboost::is_unsigned<typename vt::size_type>::value &&
          sizeof(typename nimboost::uint_value_t<Capacity>::least) <= sizeof(typename vt::size_type) ),
        SIZE_TYPE_IS_TOO_SMALL_FOR_SPECIFIED_CAPACITY,
        (varray)
    );

    typedef nimboost::aligned_storage<
        sizeof(Value[Capacity]),
        nimboost::alignment_of<Value[Capacity]>::value
...
public:
    ...
    varray & operator=(varray<Value, C> & sv)
    {
        ...
        this->operator=(static_cast<const ::nimboost::rv<other> &>(const_cast<const other &>(sv)));
        return *this;
    }
#endif
...
public:
    ...
    typedef nimboost::reverse_iterator<iterator> reverse_iterator;
    //! @brief The const reverse iterator.
    typedef nimboost::reverse_iterator<const_iterator> const_reverse_iterator;
    ...
    varray(Iterator first, Iterator last)
        : m_size(0)
    {
        BOOST_CONCEPT_ASSERT((nimboost_concepts::ForwardTraversal<Iterator>)); // Make sure you passed a ForwardIterator
...
// TEMPORARY WORKAROUND
    ...
    varray & operator=(::nimboost::rv< varray<value_type, C> > const& other)
#else
    ...
    //!
    //! @par Throws
    //!   @li If \c nimboost::has_nothrow_move<Value>::value is \c true and Value's move constructor throws.
    //!   @li If \c nimboost::has_nothrow_move<Value>::value is \c false and Value's copy constructor throws.
    //! @internal
    ...
    //!
    //! @par Throws
    //!   @li If \c nimboost::has_nothrow_move<Value>::value is \c true and Value's move constructor or move assignment throws.
    //!   @li If \c nimboost::has_nothrow_move<Value>::value is \c false and Value's copy constructor or copy assignment throws.
    //! @internal
    //!   @li It throws only if \c use_memop_in_swap_and_move is \c false_type - default.
    //! @endinternal
    //!
    ...
    void push_back(BOOST_RV_REF(value_type) value)
    {
        ...
        namespace sv = varray_detail;
        sv::construct(dti(), this->end(), ::nimboost::move(value));                           // may throw
        ++m_size; // update end
    }
    ...
    iterator insert(iterator position, value_type const& value)
    {
        ...
        else
        {
            ...
            sv::construct(dti(), this->end(), nimboost::move(r));                      // may throw
            ++m_size; // update end
    ...
    iterator insert(iterator position, BOOST_RV_REF(value_type) value)
    {
        ...
        if ( position == this->end() )
        {
            sv::construct(dti(), position, nimboost::move(value));                     // may throw
            ++m_size; // update end
        }
        else
        {
            ...
            ++m_size; // update end
            sv::move_backward(position, this->end() - 2, this->end() - 1);          // may throw
            sv::assign(position, nimboost::move(value));                               // may throw
        }
    ...
    iterator insert(iterator position, Iterator first, Iterator last)
    {
        ...
        typedef typename nimboost::iterator_traversal<Iterator>::type traversal;
    ...
    void emplace_back(BOOST_FWD_REF(Args) ...args)
    {
        ...
        namespace sv = varray_detail;
        sv::construct(dti(), this->end(), ::nimboost::forward<Args>(args)...);                // may throw
        ++m_size; // update end
    }
    ...
    iterator emplace(iterator position, BOOST_FWD_REF(Args) ...args)
    {
        ...
        if ( position == this->end() )
        {
            sv::construct(dti(), position, ::nimboost::forward<Args>(args)...);               // may throw
            ++m_size; // update end
        }
        else
        {
            ...
            sv::construct(dti(), val_p, ::nimboost::forward<Args>(args)...);                  // may throw
            sv::scoped_destructor<value_type> d(val_p);
            sv::assign(position, ::nimboost::move(*val_p));                            // may throw
        }
    ...
    Value * data()
    {
        return nimboost::addressof(*(this->ptr()));
    }
...
private:
    ...
    //   Linear O(N).
    template <std::size_t C>
    void move_ctor_dispatch(varray<value_type, C> & other, nimboost::true_type /*use_memop*/)
    {
    ...
    // @par Throws
    //   @li If nimboost::has_nothrow_move<Value>::value is true and Value's move constructor throws
    //   @li If nimboost::has_nothrow_move<Value>::value is false and Value's copy constructor throws.
    // @par Complexity
    //   Linear O(N).
    template <std::size_t C>
    void move_ctor_dispatch(varray<value_type, C> & other, nimboost::false_type /*use_memop*/)
    {
        namespace sv = varray_detail;
    ...
    //   Linear O(N).
    template <std::size_t C>
    void move_assign_dispatch(varray<value_type, C> & other, nimboost::true_type /*use_memop*/)
    {
        this->clear();
    ...
    // @par Throws
    //   @li If nimboost::has_nothrow_move<Value>::value is true and Value's move constructor or move assignment throws
    //   @li If nimboost::has_nothrow_move<Value>::value is false and Value's copy constructor or move assignment throws.
    // @par Complexity
    //   Linear O(N).
    template <std::size_t C>
    void move_assign_dispatch(varray<value_type, C> & other, nimboost::false_type /*use_memop*/)
    {
        namespace sv = varray_detail;
    ...
    //   Linear O(N).
    template <std::size_t C>
    void swap_dispatch(varray<value_type, C> & other, nimboost::true_type const& /*use_optimized_swap*/)
    {
        typedef typename
        nimboost::mpl::if_c<
            Capacity < C,
    ...
    //   Linear O(N).
    template <std::size_t C>
    void swap_dispatch(varray<value_type, C> & other, nimboost::false_type const& /*use_optimized_swap*/)
    {
        namespace sv = varray_detail;
    ...
    // @par Complexity
    //   Linear O(N).
    void swap_dispatch_impl(iterator first_sm, iterator last_sm, iterator first_la, iterator last_la, nimboost::true_type const& /*use_memop*/)
    {
        ...
        {
            nimboost::aligned_storage<
                sizeof(value_type),
                nimboost::alignment_of<value_type>::value
            > temp_storage;
            value_type * temp_ptr = reinterpret_cast<value_type*>(temp_storage.address());

            ::memcpy(temp_ptr, nimboost::addressof(*first_sm), sizeof(value_type));
            ::memcpy(nimboost::addressof(*first_sm), nimboost::addressof(*first_la), sizeof(value_type));
            ::memcpy(nimboost::addressof(*first_la), temp_ptr, sizeof(value_type));
        }
    ...
    // @par Complexity
    //   Linear O(N).
    void swap_dispatch_impl(iterator first_sm, iterator last_sm, iterator first_la, iterator last_la, nimboost::false_type const& /*use_memop*/)
    {
        //BOOST_ASSERT_MSG(std::distance(first_sm, last_sm) <= std::distance(first_la, last_la));
        ...
        for (; first_sm != last_sm ; ++first_sm, ++first_la)
        {
            //nimboost::swap(*first_sm, *first_la);                                    // may throw
            value_type temp(nimboost::move(*first_sm));                                // may throw
            *first_sm = nimboost::move(*first_la);                                     // may throw
            *first_la = nimboost::move(temp);                                          // may throw
        }
    ...
    //   Linear O(N).
    template <typename Iterator>
    void insert_dispatch(iterator position, Iterator first, Iterator last, nimboost::random_access_traversal_tag const&)
    {
        BOOST_CONCEPT_ASSERT((nimboost_concepts::RandomAccessTraversal<Iterator>)); // Make sure you passed a RandomAccessIterator
        
        errh::check_iterator_end_eq(*this, position);
        
        typename nimboost::iterator_difference<Iterator>::type
    ...
    //   Linear O(N).
    template <typename Iterator>
    void assign_dispatch(Iterator first, Iterator last, nimboost::random_access_traversal_tag const& /*not_random_access*/)
    {
        namespace sv = varray_detail;
...
public:
    ...
    // nothrow
    Value * data() { return nimboost::addressof(*(this->ptr())); }
    const Value * data() const { return nimboost::addressof(*(this->ptr())); }

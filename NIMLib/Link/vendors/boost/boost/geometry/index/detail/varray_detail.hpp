...
namespace nimboost { namespace geometry { namespace index { namespace detail { namespace varray_detail {

template <typename I>
struct are_elements_contiguous : nimboost::is_pointer<I>
{};
...
struct are_elements_contiguous<
    nimboost::container::container_detail::vector_const_iterator<Pointer>
> : nimboost::true_type
{};

template <typename Pointer>
struct are_elements_contiguous<
    nimboost::container::container_detail::vector_iterator<Pointer>
> : nimboost::true_type
{};
...
struct are_corresponding :
    ::nimboost::mpl::and_<
        ::nimboost::is_same<
            ::nimboost::remove_const<
                typename ::nimboost::iterator_value<I>::type
            >,
            ::nimboost::remove_const<
                typename ::nimboost::iterator_value<O>::type
            >
        >,
...
void destroy_dispatch(I /*first*/, I /*last*/,
                      nimboost::true_type const& /*has_trivial_destructor*/)
{}

template <typename I>
void destroy_dispatch(I first, I last,
                      nimboost::false_type const& /*has_trivial_destructor*/)
{
    typedef typename nimboost::iterator_value<I>::type value_type;
...
inline O copy_dispatch(I first, I last, O dst,
                       nimboost::mpl::bool_<true> const& /*use_memmove*/)
{
    typedef typename nimboost::iterator_value<I>::type value_type;
    typename nimboost::iterator_difference<I>::type d = std::distance(first, last);

    ::memmove(nimboost::addressof(*dst), nimboost::addressof(*first), sizeof(value_type) * d);
    return dst + d;
}

template <typename I, typename O>
inline O copy_dispatch(I first, I last, O dst,
                       nimboost::mpl::bool_<false> const& /*use_memmove*/)
{
...
O uninitialized_copy_dispatch(I first, I last, O dst,
                              nimboost::mpl::bool_<true> const& /*use_memcpy*/)
{
    typedef typename nimboost::iterator_value<I>::type value_type;
    typename nimboost::iterator_difference<I>::type d = std::distance(first, last);

    ::memcpy(nimboost::addressof(*dst), nimboost::addressof(*first), sizeof(value_type) * d);
    return dst + d;
}
...
F uninitialized_copy_dispatch(I first, I last, F dst,
                              nimboost::mpl::bool_<false> const& /*use_memcpy*/)
{
...
F uninitialized_copy(I first, I last, F dst)
{
    ...
    ::nimboost::mpl::and_<
        ...
        ::nimboost::has_trivial_copy<
            typename ::nimboost::iterator_value<F>::type
        >
    >::type
    use_memcpy;
...
O uninitialized_move_dispatch(I first, I last, O dst,
                              nimboost::mpl::bool_<false> const& /*use_memcpy*/)
{
    //return nimboost::uninitialized_move(first, last, dst);                         // may throw
    ...
    BOOST_TRY
    {
        ...
        for (; first != last; ++first, ++o )
            new (nimboost::addressof(*o)) value_type(nimboost::move(*first));
    }
    BOOST_CATCH(...)
    {
        destroy(dst, o);
...
O move_dispatch(I first, I last, O dst,
                nimboost::mpl::bool_<false> const& /*use_memmove*/)
{
    return nimboost::move(first, last, dst);                                         // may throw
}
...
BDO move_backward_dispatch(BDI first, BDI last, BDO dst,
                           nimboost::mpl::bool_<true> const& /*use_memmove*/)
{
    typedef typename nimboost::iterator_value<BDI>::type value_type;
    typename nimboost::iterator_difference<BDI>::type d = std::distance(first, last);

    BDO foo(dst - d);
    ::memmove(nimboost::addressof(*foo), nimboost::addressof(*first), sizeof(value_type) * d);
    return foo;
}
...
BDO move_backward_dispatch(BDI first, BDI last, BDO dst,
                           nimboost::mpl::bool_<false> const& /*use_memmove*/)
{
    return nimboost::move_backward(first, last, dst);                                // may throw
}
...
BDO move_backward(BDI first, BDI last, BDO dst)
{
    ...
    ::nimboost::mpl::and_<
        ...
        ::nimboost::has_trivial_assign<
            typename ::nimboost::iterator_value<BDO>::type
        >
    >::type
    use_memmove;
...
struct has_nothrow_move : public
    ::nimboost::mpl::or_<
        nimboost::mpl::bool_<
            ::nimboost::has_nothrow_move<
                typename ::nimboost::remove_const<T>::type
            >::value
        >,
        nimboost::mpl::bool_<
            ::nimboost::has_nothrow_move<T>::value
        >
    >
{};
...
template <typename I, typename O>
inline
O uninitialized_move_if_noexcept_dispatch(I first, I last, O dst, nimboost::mpl::bool_<true> const& /*use_move*/)
{ return varray_detail::uninitialized_move(first, last, dst); }

template <typename I, typename O>
inline
O uninitialized_move_if_noexcept_dispatch(I first, I last, O dst, nimboost::mpl::bool_<false> const& /*use_move*/)
...
template <typename I, typename O>
inline
O move_if_noexcept_dispatch(I first, I last, O dst, nimboost::mpl::bool_<true> const& /*use_move*/)
{ return move(first, last, dst); }

template <typename I, typename O>
inline
O move_if_noexcept_dispatch(I first, I last, O dst, nimboost::mpl::bool_<false> const& /*use_move*/)
...
void uninitialized_fill_dispatch(I /*first*/, I /*last*/,
                                 nimboost::true_type const& /*has_trivial_constructor*/,
                                 nimboost::true_type const& /*disable_trivial_init*/)
{}
...
void uninitialized_fill_dispatch(I first, I last,
                                 nimboost::true_type const& /*has_trivial_constructor*/,
                                 nimboost::false_type const& /*disable_trivial_init*/)
{
    typedef typename nimboost::iterator_value<I>::type value_type;
    for ( ; first != last ; ++first )
        new (nimboost::addressof(*first)) value_type();
}

template <typename I, typename DisableTrivialInit>
inline
void uninitialized_fill_dispatch(I first, I last,
                                 nimboost::false_type const& /*has_trivial_constructor*/,
                                 DisableTrivialInit const& /*not_used*/)
{
    ...
    {
        for ( ; it != last ; ++it )
            new (nimboost::addressof(*it)) value_type();                           // may throw
    }
    BOOST_CATCH(...)
    {
        destroy(first, it);
...
{
    typedef typename nimboost::iterator_value<I>::type value_type;
    uninitialized_fill_dispatch(first, last, nimboost::has_trivial_constructor<value_type>(), disable_trivial_init);     // may throw
}
...
template <typename I>
inline
void construct_dispatch(nimboost::mpl::bool_<true> const& /*dont_init*/, I /*pos*/)
{}

template <typename I>
inline
void construct_dispatch(nimboost::mpl::bool_<false> const& /*dont_init*/, I pos)
{
    typedef typename ::nimboost::iterator_value<I>::type value_type;
    new (static_cast<void*>(::nimboost::addressof(*pos))) value_type();                      // may throw
}
...
void construct(DisableTrivialInit const&, I pos)
{
    ...
    typedef typename ::nimboost::mpl::and_<
        nimboost::has_trivial_constructor<value_type>,
        DisableTrivialInit
    >::type dont_init;
...
void construct_copy_dispatch(I pos, V const& v,
                             nimboost::mpl::bool_<true> const& /*use_memcpy*/)
{
    ::memcpy(nimboost::addressof(*pos), nimboost::addressof(v), sizeof(V));
}
...
void construct_copy_dispatch(I pos, P const& p,
                             nimboost::mpl::bool_<false> const& /*use_memcpy*/)
{
    typedef typename nimboost::iterator_value<I>::type V;
    new (static_cast<void*>(nimboost::addressof(*pos))) V(p);                      // may throw
}
...
{
    typedef typename nimboost::iterator_value<I>::type V;
    new (static_cast<void*>(nimboost::addressof(*pos))) V(::nimboost::move(p));       // may throw
}
...
void construct(DisableTrivialInit const&, I pos, BOOST_RV_REF(P) p)
{
    ...
    ::nimboost::mpl::and_<
        is_corresponding_value<I, P>,
        ::nimboost::has_trivial_move_constructor<P>
    >::type
    use_memcpy;

    construct_move_dispatch(pos, ::nimboost::move(p), use_memcpy());               // may throw
}
...
{
    typedef typename nimboost::iterator_value<I>::type V;
    new (static_cast<void*>(nimboost::addressof(*pos))) V(::nimboost::forward<Args>(args)...);    // may throw
}
...
{                                                                                                   \
    typedef typename nimboost::iterator_value<I>::type V;                                              \
    new                                                                                             \
    (static_cast<void*>(nimboost::addressof(*pos)))                                                    \
...
void assign(I pos, BOOST_RV_REF(V) v)
{
    ...
    ::nimboost::mpl::and_<
        is_corresponding_value<I, V>,
        ::nimboost::has_trivial_move_assign<V>
    >::type
    use_memcpy;

    assign_move_dispatch(pos, ::nimboost::move(v), use_memcpy());
}
...
}}}}} // namespace nimboost::geometry::index::detail::varray_detail

...
inline R difference_dispatch(T const& from, T const& to, ::nimboost::mpl::bool_<false> const& /*is_unsigned*/)
{
    return to - from;
}

template <typename R, typename T>
inline R difference_dispatch(T const& from, T const& to, ::nimboost::mpl::bool_<true> const& /*is_unsigned*/)
{
...
{
    BOOST_MPL_ASSERT_MSG(!nimboost::is_unsigned<R>::value, RESULT_CANT_BE_UNSIGNED, (R));
...
struct find_greatest_normalized_separation<Elements, Parameters, Translator, box_tag, DimensionIndex>
{
    ...
    typedef typename nimboost::mpl::if_c<
        nimboost::is_integral<coordinate_type>::value,
        double,
        coordinate_type

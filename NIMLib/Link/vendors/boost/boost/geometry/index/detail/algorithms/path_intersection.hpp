...
struct path_intersection<Indexable, Linestring, box_tag, linestring_tag>
{
    ...
    {
        typedef typename ::nimboost::range_value<Linestring>::type point_type;
        typedef typename ::nimboost::range_const_iterator<Linestring>::type const_iterator;        
        typedef typename ::nimboost::range_size<Linestring>::type size_type;
        
        const size_type count = ::nimboost::size(path);

        if ( count == 2 )
        {
            return index::detail::segment_intersection(b, *::nimboost::begin(path), *(::nimboost::begin(path)+1), comparable_distance);
        }
        else if ( 2 < count )
        {
            const_iterator it0 = ::nimboost::begin(path);
            const_iterator it1 = ::nimboost::begin(path) + 1;
            const_iterator last = ::nimboost::end(path);

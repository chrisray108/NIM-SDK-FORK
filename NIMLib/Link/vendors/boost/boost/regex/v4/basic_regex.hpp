...
{
   std::size_t r = nimboost::hash_range(i, j);
...
struct regex_data : public named_subexpressions
{
   ...
   regex_data(const ::nimboost::shared_ptr<
      ::nimboost::regex_traits_wrapper<traits> >& t) 
      : m_ptraits(t), m_expression(0), m_expression_len(0) {}
   regex_data() 
      : m_ptraits(new ::nimboost::regex_traits_wrapper<traits>()), m_expression(0), m_expression_len(0) {}

   ::nimboost::shared_ptr<
      ::nimboost::regex_traits_wrapper<traits>
...
public:
   ...
   basic_regex_implementation(const ::nimboost::shared_ptr<
      ::nimboost::regex_traits_wrapper<traits> >& t)
   ...
   {
      if(n == 0)
         nimboost::throw_exception(std::out_of_range("0 is not a valid subexpression index."));
   ...
   const ::nimboost::regex_traits_wrapper<traits>& get_traits()const
   {
...
public:
   ...
   {
      if(!m_pimpl.get())
         nimboost::throw_exception(std::logic_error("Can't access subexpressions in an invalid regex."));
   ...
   nimboost::shared_ptr<re_detail::named_subexpressions > get_named_subs()const
   {
      return m_pimpl;
   }

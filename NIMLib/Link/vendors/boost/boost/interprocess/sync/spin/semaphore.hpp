...
class spin_semaphore
{
   ...
   void wait();
   bool try_wait();
   bool timed_wait(const nimboost::posix_time::ptime &abs_time);

//   int get_count() const;
   private:
   volatile nimboost::uint32_t m_count;
};
...
{  ipcdetail::atomic_write32(&this->m_count, nimboost::uint32_t(initialCount));  }
...
{
   spin_wait swait;
   while(!ipcdetail::atomic_add_unless32(&m_count, nimboost::uint32_t(-1), nimboost::uint32_t(0))){
      swait.yield();
   }
}
...
{
   return ipcdetail::atomic_add_unless32(&m_count, nimboost::uint32_t(-1), nimboost::uint32_t(0));
}

inline bool spin_semaphore::timed_wait(const nimboost::posix_time::ptime &abs_time)
{

   ...
   :  public intersegment_base
{
   ...
   typedef nimboost::interprocess::flat_map
      <const void *
      ,segment_info_t
   ...
   {
      //------------------------------------------------------------------
      nimboost::interprocess::scoped_lock<typename mappings_t::mutex_type> lock(s_map);
      //------------------------------------------------------------------
      base = 0;
...
//! swap specialization
template<class T> inline
void swap (nimboost::interprocess::intersegment_ptr<T> &pt,
           nimboost::interprocess::intersegment_ptr<T> &pt2)
{  pt.swap(pt2);  }

//!to_raw_pointer() enables nimboost::mem_fn to recognize intersegment_ptr.
//!Never throws.
template<class T> inline
T * to_raw_pointer(nimboost::interprocess::intersegment_ptr<T> const & p)
{  return p.get();   }

//!Simulation of static_cast between pointers.
//!Never throws.
template<class T, class U> inline
nimboost::interprocess::intersegment_ptr<T> static_pointer_cast(const nimboost::interprocess::intersegment_ptr<U> &r)
{  return nimboost::interprocess::intersegment_ptr<T>(r, nimboost::interprocess::ipcdetail::static_cast_tag());  }

//!Simulation of const_cast between pointers.
//!Never throws.
template<class T, class U> inline
nimboost::interprocess::intersegment_ptr<T> const_pointer_cast(const nimboost::interprocess::intersegment_ptr<U> &r)
{  return nimboost::interprocess::intersegment_ptr<T>(r, nimboost::interprocess::ipcdetail::const_cast_tag());  }

//!Simulation of dynamic_cast between pointers.
//!Never throws.
template<class T, class U> inline
nimboost::interprocess::intersegment_ptr<T> dynamic_pointer_cast(const nimboost::interprocess::intersegment_ptr<U> &r)
{  return nimboost::interprocess::intersegment_ptr<T>(r, nimboost::interprocess::ipcdetail::dynamic_cast_tag());  }

//!Simulation of reinterpret_cast between pointers.
//!Never throws.
template<class T, class U> inline
nimboost::interprocess::intersegment_ptr<T> reinterpret_pointer_cast(const nimboost::interprocess::intersegment_ptr<U> &r)
{  return nimboost::interprocess::intersegment_ptr<T>(r, nimboost::interprocess::ipcdetail::reinterpret_cast_tag());  }

//!Trait class to detect if an smart pointer has
//!multi-segment addressing capabilities.
template <class T>
struct is_multisegment_ptr
   <nimboost::interprocess::intersegment_ptr<T> >
{

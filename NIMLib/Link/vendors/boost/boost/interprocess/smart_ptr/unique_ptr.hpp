...
class unique_ptr
{
   ...
   unique_ptr(BOOST_RV_REF(unique_ptr) u)
      : ptr_(u.release(), nimboost::forward<D>(u.get_deleter()))
   {}
   ...
   unique_ptr(BOOST_RV_REF_BEG unique_ptr<U, E> BOOST_RV_REF_END u,
      typename ipcdetail::enable_if_c<
            ...
            nat
            >::type = nat())
      : ptr_(const_cast<unique_ptr<U,E>&>(u).release(), nimboost::move<D>(u.get_deleter()))
   {}
   ...
   {
      reset(u.release());
      ptr_.second() = nimboost::move(u.get_deleter());
      return *this;
   }
   ...
   /// @cond
   private:
   nimboost::compressed_pair<pointer, D> ptr_;
...
private:
    ...
    template <class U, class E> unique_ptr(U p, E,
        typename nimboost::enable_if<nimboost::is_convertible<U, pointer> >::type* = 0);
...
//!of type T with nimboost::interprocess::deleter deleter
...
//!with nimboost::interproces::deleter from a pointer

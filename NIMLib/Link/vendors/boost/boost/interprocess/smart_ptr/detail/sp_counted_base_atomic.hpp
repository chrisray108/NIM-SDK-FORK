...
private:
    ...
    nimboost::uint32_t use_count_;        // #shared
    nimboost::uint32_t weak_count_;       // #weak + (#shared != 0)
...
public:
    ...
    bool add_ref_lock() // true on success
    {
        for( ;; )
        {
            nimboost::uint32_t tmp = static_cast< nimboost::uint32_t const volatile& >( use_count_ );
   ...
   { return (long)static_cast<nimboost::uint32_t const volatile &>( use_count_ ); }
};

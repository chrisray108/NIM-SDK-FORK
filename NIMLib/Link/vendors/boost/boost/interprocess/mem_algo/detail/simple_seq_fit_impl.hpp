...
class simple_seq_fit_impl
{
   ...
   typedef typename nimboost::intrusive::pointer_traits<char_ptr>::difference_type difference_type;
   typedef typename nimboost::make_unsigned<difference_type>::type size_type;
   ...
   {
      //-----------------------
      nimboost::interprocess::scoped_lock<interprocess_mutex> guard(m_header);
      //-----------------------
   ...
   template<class T>
   std::pair<T *, bool>
      allocation_command  (nimboost::interprocess::allocation_type command,   size_type limit_size,
                           size_type preferred_size,size_type &received_size,
                           T *reuse_ptr = 0);

   std::pair<void *, bool>
      raw_allocation_command  (nimboost::interprocess::allocation_type command,   size_type limit_size,
                               size_type preferred_size,size_type &received_size,
   ...
   std::pair<void *, bool> priv_allocate(nimboost::interprocess::allocation_type command
                                        ,size_type min_size
                                        ,size_type preferred_size
                                        ,size_type &received_size
                                        ,void *reuse_ptr = 0);

   std::pair<void *, bool> priv_allocation_command(nimboost::interprocess::allocation_type command
                                        ,size_type min_size
                                        ,size_type preferred_size
                                        ,size_type &received_size
                                        ,void *reuse_ptr
   ...
   void* priv_expand_both_sides(nimboost::interprocess::allocation_type command
                               ,size_type min_size
                               ,size_type preferred_size
                               ,size_type &received_size
                               ,void *reuse_ptr
   ...
   public:
	static const size_type Alignment      = ::nimboost::alignment_of< ::nimboost::detail::max_align>::value;
   private:
...
void simple_seq_fit_impl<MutexFamily, VoidPointer>::shrink_to_fit()
{
   ...
   if(!m_header.m_allocated){
      ...
      size_type ignore;
      unique_block = priv_allocate(nimboost::interprocess::allocate_new, 0, 0, ignore).first;
      if(!unique_block)
         return;
   ...
   allocate(size_type nbytes)
{
   ...
   //-----------------------
   size_type ignore;
   return priv_allocate(nimboost::interprocess::allocate_new, nbytes, nbytes, ignore).first;
}
                        ...
                        T *reuse_ptr)
{
   ...
   BOOST_ASSERT(0 == ((std::size_t)ret.first % ::nimboost::alignment_of<T>::value));
...
inline std::pair<void*, bool> simple_seq_fit_impl<MutexFamily, VoidPointer>::
   raw_allocation_command  (nimboost::interprocess::allocation_type command,   size_type limit_objects,
                        ...
                        void *reuse_ptr, size_type sizeof_object)
{
   ...
   if(command & nimboost::interprocess::try_shrink_in_place){
...
template<class MutexFamily, class VoidPointer>
inline std::pair<void*, bool> simple_seq_fit_impl<MutexFamily, VoidPointer>::
   priv_allocation_command (nimboost::interprocess::allocation_type command,   size_type limit_size,
                       size_type preferred_size, size_type &received_size,
                       void *reuse_ptr, size_type sizeof_object)
{
   command &= ~nimboost::interprocess::expand_bwd;
...
template<class MutexFamily, class VoidPointer>
void* simple_seq_fit_impl<MutexFamily, VoidPointer>::
   priv_expand_both_sides(nimboost::interprocess::allocation_type command
                         ,size_type min_size
                         ,size_type preferred_size
                         ,size_type &received_size
                         ,void *reuse_ptr
                         ,bool only_preferred_backwards)
{
   ...
   if(command & nimboost::interprocess::expand_fwd){
   ...
   if(command & nimboost::interprocess::expand_bwd){
...
std::pair<void *, bool> simple_seq_fit_impl<MutexFamily, VoidPointer>::
   priv_allocate(nimboost::interprocess::allocation_type command
                ,size_type limit_size
                ,size_type preferred_size
                ,size_type &received_size
                ,void *reuse_ptr)
{
   if(command & nimboost::interprocess::shrink_in_place){
      bool success =
   ...
   //
   if(reuse_ptr && (command & (nimboost::interprocess::expand_fwd | nimboost::interprocess::expand_bwd))){
   ...
   if(command & nimboost::interprocess::allocate_new){
      received_size = 0;
      while(block != root){

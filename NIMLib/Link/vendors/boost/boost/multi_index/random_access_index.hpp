...
public:
  ...
  typedef typename
    nimboost::reverse_iterator<iterator>              reverse_iterator;
  typedef typename
    nimboost::reverse_iterator<const_iterator>        const_reverse_iterator;
...
public:
  ...
  std::pair<iterator,bool> push_front(BOOST_RV_REF(value_type) x)
                             {return insert(begin(),nimboost::move(x));}
  ...
  std::pair<iterator,bool> push_back(BOOST_RV_REF(value_type) x)
                             {return insert(end(),nimboost::move(x));}
...
BOOST_MULTI_INDEX_PROTECTED_IF_MEMBER_TEMPLATE_FRIENDS:
  ...
  {
    {
      ...
      lm.load(::nimboost::bind(&loader::rearrange,&ld,_1,_2),ar,version);
  ...
  /* This forwarding function eases things for the nimboost::mem_fn construct
...
template<typename SuperMeta,typename TagList>
inline nimboost::mpl::true_* boost_foreach_is_noncopyable(
  nimboost::multi_index::detail::random_access_index<SuperMeta,TagList>*&,
  nimboost::foreach::tag)
{
  return 0;
}

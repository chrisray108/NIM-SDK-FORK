...
namespace nimboost { namespace scope_exit { namespace detail {
...
BOOST_TYPEOF_REGISTER_TEMPLATE(nimboost::scope_exit::detail::wrapper, 1)

#define BOOST_SCOPE_EXIT_AUX_ARGS nimboost_scope_exit_aux_args
extern nimboost::scope_exit::detail::undeclared BOOST_SCOPE_EXIT_AUX_ARGS;
...
namespace nimboost { namespace scope_exit { namespace aux {
...
struct msvc_typeid_wrapper {
    typedef typename msvc_extract_type<nimboost::mpl::int_<Id>
            >::id2type id2type;
...
struct encode_type {
    ...
    typedef typename msvc_register_type<T,
            nimboost::mpl::int_<value> >::id2type type;
...
typename nimboost::disable_if<
      typename nimboost::is_function<T>::type
...
#define BOOST_SCOPE_EXIT_DETAIL_TYPEDEF_TYPEOF_THIS(id, ty, new_type) \
    ...
    enum { \
        BOOST_SCOPE_EXIT_AUX_TYPEDEF_TYPEOF_THIS_INDEX_(id) = sizeof( \
            *::nimboost::scope_exit::aux::msvc_typeof_this::encode_start(this)) \
    }; \
    typedef \
        ty ::nimboost::scope_exit::aux::msvc_typeof_this::msvc_typeid_wrapper< \
...
#define BOOST_SCOPE_EXIT_DETAIL_CAPTURE_DECL(r, id_ty, i, var) \
    struct BOOST_SCOPE_EXIT_AUX_WRAPPED(BOOST_PP_TUPLE_ELEM(2, 0, id_ty), i) \
        ...
        : BOOST_TYPEOF(::nimboost::scope_exit::detail::wrap( \
...
private:
    This this_;
    nimboost::function<void (This)> f_;
};
...
struct guard<void> { // Without object `this_` (could capture `this` directly).
    ...
    void operator=(Lambda f) { f_ = f; }
private:
    nimboost::function<void (void)> f_;
};
...
#define BOOST_SCOPE_EXIT_AUX_IMPL(id, ty, traits) \
    ...
    struct BOOST_SCOPE_EXIT_DETAIL_PARAMS_T(id) { \
        ...
        BOOST_SCOPE_EXIT_AUX_PARAMS_INIT(id, \
                ...
                BOOST_SCOPE_EXIT_AUX_TRAITS_HAS_THIS(traits)) \
    ; \
    ::nimboost::scope_exit::detail::declared< \
        ::nimboost::scope_exit::detail::resolve< \

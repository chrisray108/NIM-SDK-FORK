...
namespace nimboost{ namespace multiprecision{ namespace default_ops{
...
inline void eval_convert_to(terminal<R>* result, const B& backend)
{
   ...
   //
   result->value = nimboost::lexical_cast<R>(backend.str(0, std::ios_base::fmtflags(0)));
}
...
{
   typedef typename nimboost::multiprecision::detail::canonical<A, T>::type canonical_type;
...
inline void eval_trunc(T& result, const T& a)
{
   ...
   {
      result = nimboost::math::policies::raise_rounding_error("nimboost::multiprecision::trunc<%1%>(%1%)", 0, number<T>(a), number<T>(a), nimboost::math::policies::policy<>()).backend();
      return;
   }
   if(eval_get_sign(a) < 0)
      eval_ceil(result, a);
...
inline void eval_round(T& result, const T& a)
{
   ...
   typedef typename nimboost::multiprecision::detail::canonical<float, T>::type fp_type;
   int c = eval_fpclassify(a);
   if(c == FP_NAN || c == FP_INFINITE)
   {
      result = nimboost::math::policies::raise_rounding_error("nimboost::multiprecision::round<%1%>(%1%)", 0, number<T>(a), number<T>(a), nimboost::math::policies::policy<>()).backend();
      return;
   }
   if(eval_get_sign(a) < 0)
   {
...
{
   typedef typename nimboost::multiprecision::detail::canonical<Arithmetic, T>::type si_type;
...
{
   typedef typename nimboost::multiprecision::detail::canonical<unsigned, T>::type ui_type;
...
void eval_integer_sqrt(B& s, B& r, const B& x)
{
   ...
   // at some point.
   //
   typedef typename nimboost::multiprecision::detail::canonical<unsigned char, B>::type ui_type;
...
inline int itrunc(const detail::expression<tag, A1, A2, A3, A4>& v, const Policy& pol)
{
   ...
   if((r > (std::numeric_limits<int>::max)()) || r < (std::numeric_limits<int>::min)() || !(nimboost::math::isfinite)(v))
      return nimboost::math::policies::raise_rounding_error("nimboost::multiprecision::itrunc<%1%>(%1%)", 0, number_type(v), 0, pol);
   return r.template convert_to<int>();
}
template <class tag, class A1, class A2, class A3, class A4>
inline int itrunc(const detail::expression<tag, A1, A2, A3, A4>& v)
{
   return itrunc(v, nimboost::math::policies::policy<>());
}
template <class Backend, expression_template_option ExpressionTemplates, class Policy>
inline int itrunc(const number<Backend, ExpressionTemplates>& v, const Policy& pol)
{
   ...
   if((r > (std::numeric_limits<int>::max)()) || r < (std::numeric_limits<int>::min)() || !(nimboost::math::isfinite)(v))
      return nimboost::math::policies::raise_rounding_error("nimboost::multiprecision::itrunc<%1%>(%1%)", 0, v, 0, pol);
   return r.template convert_to<int>();
}
...
inline long ltrunc(const detail::expression<tag, A1, A2, A3, A4>& v, const Policy& pol)
{
   typedef typename detail::expression<tag, A1, A2, A3, A4>::result_type number_type;
   number_type r = trunc(v, pol);
   if((r > (std::numeric_limits<long>::max)()) || r < (std::numeric_limits<long>::min)() || !(nimboost::math::isfinite)(v))
      return nimboost::math::policies::raise_rounding_error("nimboost::multiprecision::ltrunc<%1%>(%1%)", 0, number_type(v), 0L, pol);
   return r.template convert_to<long>();
}
template <class tag, class A1, class A2, class A3, class A4>
inline long ltrunc(const detail::expression<tag, A1, A2, A3, A4>& v)
{
   return ltrunc(v, nimboost::math::policies::policy<>());
}
...
inline long ltrunc(const number<T, ExpressionTemplates>& v, const Policy& pol)
{
   ...
   if((r > (std::numeric_limits<long>::max)()) || r < (std::numeric_limits<long>::min)() || !(nimboost::math::isfinite)(v))
      return nimboost::math::policies::raise_rounding_error("nimboost::multiprecision::ltrunc<%1%>(%1%)", 0, v, 0L, pol);
   return r.template convert_to<long>();
}
...
inline long long lltrunc(const detail::expression<tag, A1, A2, A3, A4>& v, const Policy& pol)
{
   typedef typename detail::expression<tag, A1, A2, A3, A4>::result_type number_type;
   number_type r = trunc(v, pol);
   if((r > (std::numeric_limits<long long>::max)()) || r < (std::numeric_limits<long long>::min)() || !(nimboost::math::isfinite)(v))
      return nimboost::math::policies::raise_rounding_error("nimboost::multiprecision::lltrunc<%1%>(%1%)", 0, number_type(v), 0LL, pol);
   return r.template convert_to<long long>();
}
template <class tag, class A1, class A2, class A3, class A4>
inline long long lltrunc(const detail::expression<tag, A1, A2, A3, A4>& v)
{
   return lltrunc(v, nimboost::math::policies::policy<>());
}
template <class T, expression_template_option ExpressionTemplates, class Policy>
inline long long lltrunc(const number<T, ExpressionTemplates>& v, const Policy& pol)
{
   number<T, ExpressionTemplates> r = trunc(v, pol);
   if((r > (std::numeric_limits<long long>::max)()) || r < (std::numeric_limits<long long>::min)() || !(nimboost::math::isfinite)(v))
      return nimboost::math::policies::raise_rounding_error("nimboost::multiprecision::lltrunc<%1%>(%1%)", 0, v, 0LL, pol);
   return r.template convert_to<long long>();
}
template <class T, expression_template_option ExpressionTemplates>
...
inline int iround(const detail::expression<tag, A1, A2, A3, A4>& v, const Policy& pol)
{
   ...
   if((r > (std::numeric_limits<int>::max)()) || r < (std::numeric_limits<int>::min)() || !(nimboost::math::isfinite)(v))
      return nimboost::math::policies::raise_rounding_error("nimboost::multiprecision::iround<%1%>(%1%)", 0, number_type(v), 0, pol);
   return r.template convert_to<int>();
}
template <class tag, class A1, class A2, class A3, class A4>
inline int iround(const detail::expression<tag, A1, A2, A3, A4>& v)
{
   return iround(v, nimboost::math::policies::policy<>());
}
template <class T, expression_template_option ExpressionTemplates, class Policy>
inline int iround(const number<T, ExpressionTemplates>& v, const Policy& pol)
{
   ...
   if((r > (std::numeric_limits<int>::max)()) || r < (std::numeric_limits<int>::min)() || !(nimboost::math::isfinite)(v))
      return nimboost::math::policies::raise_rounding_error("nimboost::multiprecision::iround<%1%>(%1%)", 0, v, 0, pol);
   return r.template convert_to<int>();
}
template <class T, expression_template_option ExpressionTemplates>
...
inline long lround(const detail::expression<tag, A1, A2, A3, A4>& v, const Policy& pol)
{
   ...
   number_type r = round(v, pol);
   if((r > (std::numeric_limits<long>::max)()) || r < (std::numeric_limits<long>::min)() || !(nimboost::math::isfinite)(v))
      return nimboost::math::policies::raise_rounding_error("nimboost::multiprecision::lround<%1%>(%1%)", 0, number_type(v), 0L, pol);
   return r.template convert_to<long>();
}
template <class tag, class A1, class A2, class A3, class A4>
inline long lround(const detail::expression<tag, A1, A2, A3, A4>& v)
{
   return lround(v, nimboost::math::policies::policy<>());
}
template <class T, expression_template_option ExpressionTemplates, class Policy>
inline long lround(const number<T, ExpressionTemplates>& v, const Policy& pol)
{
   number<T, ExpressionTemplates> r = round(v, pol);
   if((r > (std::numeric_limits<long>::max)()) || r < (std::numeric_limits<long>::min)() || !(nimboost::math::isfinite)(v))
      return nimboost::math::policies::raise_rounding_error("nimboost::multiprecision::lround<%1%>(%1%)", 0, v, 0L, pol);
   return r.template convert_to<long>();
}
template <class T, expression_template_option ExpressionTemplates>
...
inline long long llround(const detail::expression<tag, A1, A2, A3, A4>& v, const Policy& pol)
{
   ...
   number_type r = round(v, pol);
   if((r > (std::numeric_limits<long long>::max)()) || r < (std::numeric_limits<long long>::min)() || !(nimboost::math::isfinite)(v))
      return nimboost::math::policies::raise_rounding_error("nimboost::multiprecision::iround<%1%>(%1%)", 0, number_type(v), 0LL, pol);
   return r.template convert_to<long long>();
}
template <class tag, class A1, class A2, class A3, class A4>
inline long long llround(const detail::expression<tag, A1, A2, A3, A4>& v)
{
   return llround(v, nimboost::math::policies::policy<>());
}
template <class T, expression_template_option ExpressionTemplates, class Policy>
inline long long llround(const number<T, ExpressionTemplates>& v, const Policy& pol)
{
   number<T, ExpressionTemplates> r = round(v, pol);
   if((r > (std::numeric_limits<long long>::max)()) || r < (std::numeric_limits<long long>::min)() || !(nimboost::math::isfinite)(v))
      return nimboost::math::policies::raise_rounding_error("nimboost::multiprecision::iround<%1%>(%1%)", 0, v, 0LL, pol);
   return r.template convert_to<long long>();
}
template <class T, expression_template_option ExpressionTemplates>
...
template <class Backend> \
inline typename nimboost::enable_if_c<\
   nimboost::multiprecision::number_category<Backend>::value == category,\
...
{
   return BOOST_MP_MOVE(nimboost::math::sinhc_pi(x));
}

...
/*! \endcond */
{
  ...
  // Requirements
    //BOOST_CLASS_REQUIRE(T, nimboost, SGIAssignableConcept);
...
private:
    ...
    /*! \cond */
    template <class ValT> 
    static inline typename nimboost::conditional<
        ((nimboost::is_nothrow_move_constructible<ValT>::value && nimboost::is_nothrow_move_assignable<ValT>::value) || !nimboost::is_copy_constructible<ValT>::value)
            ...
            for (pointer first = m_first; dest < src; src = first) {
                for (size_type ii = 0; src < m_end; ++src, ++dest, ++moved, ++ii) {
                    ...
                    if (is_uninitialized(dest)) {
                        ...
                        replace(dest, nimboost::move(tmp));
                    }
                }
            }
        } BOOST_CATCH(...) {
...
public:
    ...
    */
    void push_back(rvalue_type item) {
        push_back_impl<rvalue_type>(nimboost::move(item));
    }
    ...
    */
    void push_front(rvalue_type item) {
        push_front_impl<rvalue_type>(nimboost::move(item));
    }
...
// Insert
    ...
    */
    iterator insert(iterator pos, rvalue_type item) {
        return insert_impl<rvalue_type>(pos, nimboost::move(item));
    }
    ...
    */
    iterator insert(iterator pos) {
        value_type temp;
        return insert(pos, nimboost::move(temp));
    }
...
public:
    ...
    */
    iterator rinsert(iterator pos, rvalue_type item) {
        return rinsert_impl<rvalue_type>(pos, nimboost::move(item));
    }
    ...
    */
    iterator rinsert(iterator pos) {
        value_type temp;
        return rinsert(pos, nimboost::move(temp));
    }
...
// Helper methods
    ...
    */
    void construct_or_replace(bool construct, pointer pos, rvalue_type item) {
        if (construct)
            ::new (pos) value_type(nimboost::move(item));
        else
            replace(pos, nimboost::move(item));
    }
    ...
    void initialize(InputIterator first, InputIterator last, const std::input_iterator_tag&) {
        ...
        initialize(distance, nimboost::make_move_iterator(tmp.begin()), nimboost::make_move_iterator(tmp.end()), distance);
    }
    ...
    void assign(InputIterator first, InputIterator last, const std::input_iterator_tag&) {
        ...
        assign_n(distance, distance,
            cb_details::make_assign_range<value_type>
                (nimboost::make_move_iterator(tmp.begin()), nimboost::make_move_iterator(tmp.end())));
    }

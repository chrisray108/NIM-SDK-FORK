...
using nimboost::graph::parallel::trigger_receive_context;
 ...
 public:
  ...
  { 
    send(data->process_group, p, property_map_put, 
         nimboost::parallel::detail::make_untracked_pair(k, v)); 
  }
 ...
 private:
  ...
  struct handle_message
  {
    ...
    handle_put(int source, int tag, 
               const nimboost::parallel::detail::untracked_pair<key_type, value_type>& data, 
...
namespace nimboost {
  ...
  struct hash<nimboost::detail::edge_desc_impl<D, V> >
  {
    std::size_t operator()(const nimboost::detail::edge_desc_impl<D, V> & x) const

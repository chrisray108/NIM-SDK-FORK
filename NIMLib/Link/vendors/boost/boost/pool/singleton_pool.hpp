...
namespace nimboost {
 ...
 Can be any Boost.Thread Mutex type or <tt>nimboost::details::pool::null_mutex</tt>.
 It is exposed so that users may declare some singleton pools normally (i.e., with synchronization), but 
 some singleton pools without synchronization (by specifying <tt>nimboost::details::pool::null_mutex</tt>) for efficiency reasons.
 The member typedef <tt>mutex</tt> exposes the value of this template parameter.  The default for this
 parameter is nimboost::details::pool::default_mutex which is a synonym for either <tt>nimboost::details::pool::null_mutex</tt>
 ...
 or for <tt>nimboost::mutex</tt> (when threading support is enabled in the compiler).
...
private:
   typedef nimboost::aligned_storage<sizeof(pool_type), nimboost::alignment_of<pool_type>::value> storage_type;

...
// static nimboost::type_traits::yes_type test(T1&, decltype(T1(nimboost::declval<T1&>()))* = 0);
//                                                        ^ 
#if !defined(BOOST_NO_CXX11_DELETED_FUNCTIONS) && !defined(BOOST_INTEL_CXX_VERSION)

#ifdef BOOST_NO_CXX11_DECLTYPE
    template <class T1>
    static nimboost::type_traits::yes_type test(T1&, nimboost::mpl::int_<sizeof(T1(nimboost::declval<T1&>()))>* = 0);
#else
    template <class T1>
    static nimboost::type_traits::yes_type test(T1&, decltype(T1(nimboost::declval<T1&>()))* = 0);
#endif

    static nimboost::type_traits::no_type test(...);
#else
    template <class T1>
    static nimboost::type_traits::no_type test(T1&, typename T1::boost_move_no_copy_constructor_or_assign* = 0);
    static nimboost::type_traits::yes_type test(...);
#endif
    ...
    //      struct T: private nimboost::noncopyable {
    //          ...
    //      private:
    //          T(const T &);
    //          ...
    ...
    BOOST_STATIC_CONSTANT(bool, value = (
            sizeof(test(
                nimboost::declval<BOOST_DEDUCED_TYPENAME nimboost::add_reference<T>::type>()
            )) == sizeof(nimboost::type_traits::yes_type)
        ||
            nimboost::is_rvalue_reference<T>::value
    ));
};
...
struct is_copy_constructible_impl {
    ...
    BOOST_STATIC_CONSTANT(bool, value = (
        nimboost::detail::is_copy_constructible_impl2<
            nimboost::is_base_and_derived<nimboost::noncopyable, T>::value,
            T
        >::value
    ));
};
...
BOOST_TT_AUX_BOOL_TRAIT_DEF1(is_copy_constructible,T,::nimboost::detail::is_copy_constructible_impl<T>::value)

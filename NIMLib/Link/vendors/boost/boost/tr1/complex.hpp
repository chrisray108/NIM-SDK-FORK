...
using nimboost::math::acos;
using nimboost::math::asin;
using nimboost::math::atan;
using nimboost::math::acosh;
using nimboost::math::asinh;
using nimboost::math::atanh;
using nimboost::math::fabs;
...
inline complex<typename nimboost::tr1_detail::promote_to_real<T, U>::type> 
   polar(const T& rho, const U& theta)
{
   typedef typename nimboost::tr1_detail::promote_to_real<T, U>::type real_type;
...
template<class T, class U>
inline complex<typename nimboost::tr1_detail::largest_real<T, U>::type>
   pow(const complex<T>& x, const complex<U>& y)
{
   typedef complex<typename nimboost::tr1_detail::largest_real<T, U>::type> result_type;
   typedef typename nimboost::mpl::if_<nimboost::is_same<result_type, complex<T> >, result_type const&, result_type>::type cast1_type;
   typedef typename nimboost::mpl::if_<nimboost::is_same<result_type, complex<U> >, result_type const&, result_type>::type cast2_type;
   cast1_type x1(x);
   cast2_type y1(y);
...
{
   typedef typename nimboost::tr1_detail::promote_to_real<T, U>::type real_type;
   typedef complex<typename nimboost::tr1_detail::promote_to_real<T, U>::type> result_type;
   typedef typename nimboost::mpl::if_<nimboost::is_same<result_type, complex<T> >, result_type const&, result_type>::type cast1_type;
   real_type r = y;
   cast1_type x1(x);
   ...
   pow (const T& x, const complex<U>& y)
{
   typedef typename nimboost::tr1_detail::promote_to_real<T, U>::type real_type;
   typedef complex<typename nimboost::tr1_detail::promote_to_real<T, U>::type> result_type;
   typedef typename nimboost::mpl::if_<nimboost::is_same<result_type, complex<U> >, result_type const&, result_type>::type cast_type;
   real_type r = x;

    ...
    struct let_eval
    {
        ...
        operator()(Vars const & vars, Map, Expr const & expr, Context const & ctx) const
        {
            ...
            scoped_environment<
              ...
              , map_type
            >
            env(nimboostphoenix::env(ctx), nimboostphoenix::env(ctx), locals);

            return eval(expr, nimboostphoenix::context(env, nimboostphoenix::actions(ctx)));
        }
    };

...
BOOST_PHOENIX_DEFINE_EXPRESSION_VARARG(
    (nimboost)(nimboostphoenix)(this_)
    ...
    struct this_eval
    {
        ...
        operator()(A0 const & a0, Context const & ctx) const
        {
            ...
            //std::cout << typeid(checker).name() << "\n";
				...
				inner_env_type inner_env = {fusion::at_c<0>(nimboostphoenix::env(ctx)), nimboostphoenix::eval(a0, ctx)};
				vector0<> locals;
				env_type env(inner_env, nimboostphoenix::env(ctx), locals);

				return nimboostphoenix::eval(*fusion::at_c<0>(nimboostphoenix::env(ctx)), nimboostphoenix::context(inner_env, nimboostphoenix::actions(ctx)));
            //return (*fusion::at_c<0>(nimboostphoenix::env(ctx)))(eval(a0, ctx));
        }
    };

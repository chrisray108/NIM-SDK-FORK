...
private:
    ...
    public:
        ...
        template< typename T >
        BOOST_LOG_TYPEOF(property_tree_type().get(typename property_tree_type::path_type(), nimboost::declval< T >())) or_default(T const& def_value) const
        {
            if (m_section.m_ptree)
                return m_section.m_ptree->get(m_path, def_value);
            else
        ...
        template< typename T >
        typename enable_if< nimboost::property_tree::detail::is_character< T >, std::basic_string< T > >::type

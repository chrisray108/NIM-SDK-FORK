...
struct filter_factory
{
    ...
    {
        BOOST_LOG_THROW_DESCR_PARAMS(parse_error, "The custom attribute value relation \"" + nimboost::log::aux::to_narrow(arg) + "\" is not supported", (name));
        BOOST_LOG_UNREACHABLE();
    }
...
public:
    ...
    {
        return nimboost::lexical_cast< value_type >(arg);
    }
};
...
>::type register_filter_factory(attribute_name const& name, shared_ptr< FactoryT > const& factory)
{
    ...
    register_filter_factory(name, nimboost::static_pointer_cast< factory_base >(factory));
}
...
inline void register_simple_filter_factory(attribute_name const& name)
{
    shared_ptr< filter_factory< CharT > > factory =
        nimboost::make_shared< basic_filter_factory< CharT, AttributeValueT > >();

...
{
    const char* str = nimboost::log::trivial::to_string(lvl);
    if (str)
        strm << str;
    else
...
#define BOOST_LOG_TRIVIAL(lvl)\
    BOOST_LOG_STREAM_WITH_PARAMS(::nimboost::log::trivial::logger::get(),\
        (::nimboost::log::keywords::severity = ::nimboost::log::trivial::lvl))

...
BOOST_FORCEINLINE nimboostphoenix::actor< aux::unary_function_terminal< attribute_matches< T, RegexT > > >
matches(attribute_name const& name, RegexT const& rex)
{
    ...
    nimboostphoenix::actor< terminal_type > act = {{ terminal_type(name, rex) }};
    return act;
}

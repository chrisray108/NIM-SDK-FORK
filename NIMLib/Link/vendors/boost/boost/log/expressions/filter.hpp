...
private:
    ...
    typedef nimboost::log::aux::light_function< bool (attribute_value_set const&) > filter_type;
...
public:
     ...
     */
    filter(BOOST_RV_REF(filter) that) BOOST_NOEXCEPT : m_Filter(nimboost::move(that.m_Filter))
    {
    }

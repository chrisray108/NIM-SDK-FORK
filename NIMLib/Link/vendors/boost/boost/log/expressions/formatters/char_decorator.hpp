...
public:
    ...
    struct result< this_type(ContextT) >
    {
        ...
        typedef typename nimboostphoenix::evaluator::impl<
            typename LeftT::proto_base_expr&,
            context_type,
            nimboostphoenix::unused
        >::result_type type;
    };
...
public:
    ...
    result_type operator() (ContextT const& ctx)
    {
        ...
        // Invoke the adopted formatter
        typedef nimboostphoenix::vector3<
            subactor_type*,
            typename fusion::result_of::at_c<
                typename remove_cv<
                    typename remove_reference<
                        typename nimboostphoenix::result_of::env< ContextT const& >::type
                    >::type
                >::type::args_type,
                0
            >::type,
            stream_type&
        > env_type;
        env_type env = { nimboost::addressof(m_subactor), fusion::at_c< 0 >(nimboostphoenix::env(ctx).args()), strm };
        nimboostphoenix::eval(m_subactor, nimboostphoenix::make_context(env, nimboostphoenix::actions(ctx)));
 ...
 */
template< typename SubactorT, typename ImplT, template< typename > class ActorT = nimboostphoenix::actor >
...
#define BOOST_LOG_AUX_OVERLOAD(left_ref, right_ref)\
    ...
    BOOST_FORCEINLINE nimboostphoenix::actor< aux::char_decorator_output_terminal< nimboostphoenix::actor< LeftExprT >, SubactorT, ImplT > >\
    operator<< (nimboostphoenix::actor< LeftExprT > left_ref left, char_decorator_actor< SubactorT, ImplT, ActorT > right_ref right)\
    {\
        typedef aux::char_decorator_output_terminal< nimboostphoenix::actor< LeftExprT >, SubactorT, ImplT > terminal_type;\
        nimboostphoenix::actor< terminal_type > actor = {{ terminal_type(left, right.get_terminal().get_subactor(), right.get_terminal().get_impl()) }};\
        return actor;\
    }
...
class char_decorator_gen1
{
    ...
    typedef typename nimboost::log::aux::deduce_char_type< typename range_value< RangeT >::type::first_type >::type char_type;
...
class char_decorator_gen2
{
    ...
    typedef typename nimboost::log::aux::deduce_char_type< typename range_value< FromRangeT >::type >::type from_char_type;
    typedef typename nimboost::log::aux::deduce_char_type< typename range_value< ToRangeT >::type >::type to_char_type;
...
template< typename SubactorT, typename ImplT >
struct is_nullary< custom_terminal< nimboost::log::expressions::char_decorator_terminal< SubactorT, ImplT > > > :
    public mpl::false_
{
};

template< typename LeftT, typename SubactorT, typename ImplT >
struct is_nullary< custom_terminal< nimboost::log::expressions::aux::char_decorator_output_terminal< LeftT, SubactorT, ImplT > > > :
    public mpl::false_
{
};

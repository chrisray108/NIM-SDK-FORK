...
public:
    ...
    typename result< this_type(ContextT const&) >::type operator() (ContextT const& ctx)
    {
        ...
        m_fun(fusion::at_c< 0 >(nimboostphoenix::env(ctx).args()), strm);
        return strm;
    }
...
template<
    ...
    bool HasSecondArgumentV = nimboost::log::aux::has_second_argument_type< FunT >::value,
    bool HasArg2V = nimboost::log::aux::has_arg2_type< FunT >::value
>
 ...
 */
template< typename FunT, typename CharT, template< typename > class ActorT = nimboostphoenix::actor >
...
#define BOOST_LOG_AUX_OVERLOAD(left_ref, right_ref)\
    ...
    BOOST_FORCEINLINE nimboostphoenix::actor< aux::wrapped_formatter_output_terminal< nimboostphoenix::actor< LeftExprT >, FunT > >\
    operator<< (nimboostphoenix::actor< LeftExprT > left_ref left, wrapped_formatter_actor< FunT, CharT > right_ref right)\
    {\
        typedef aux::wrapped_formatter_output_terminal< nimboostphoenix::actor< LeftExprT >, FunT > terminal_type;\
        nimboostphoenix::actor< terminal_type > actor = {{ terminal_type(left, right.get_function()) }};\
        return actor;\
    }
...
template< typename LeftT, typename FunT >
struct is_nullary< custom_terminal< nimboost::log::expressions::aux::wrapped_formatter_output_terminal< LeftT, FunT > > > :
    public mpl::false_
{
};

template< typename FunT, typename CharT >
struct is_nullary< custom_terminal< nimboost::log::expressions::wrapped_formatter_terminal< FunT, CharT > > > :
    public mpl::false_
{
};

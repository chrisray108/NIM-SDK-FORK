...
BOOST_LOG_API nimboost::log::aux::light_function< void (basic_formatting_ostream< char >&, attributes::named_scope::value_type::value_type const&) >
parse_named_scope_format(const char* begin, const char* end);
#endif

#ifdef BOOST_LOG_USE_WCHAR_T
//! Parses the named scope format string and constructs the formatter function
BOOST_LOG_API nimboost::log::aux::light_function< void (basic_formatting_ostream< wchar_t >&, attributes::named_scope::value_type::value_type const&) >
parse_named_scope_format(const wchar_t* begin, const wchar_t* end);
#endif

//! Parses the named scope format string and constructs the formatter function
template< typename CharT >
inline nimboost::log::aux::light_function< void (basic_formatting_ostream< CharT >&, attributes::named_scope::value_type::value_type const&) >
...
public:
    ...
    typedef nimboost::log::aux::light_function< void (stream_type&, value_type::value_type const&) > element_formatter_type;
...
public:
    ...
    result_type operator() (ContextT const& ctx)
    {
        ...
        m_visitor_invoker(m_name, fusion::at_c< 0 >(nimboostphoenix::env(ctx).args()), binder1st< formatter_function_type&, stream_type& >(m_formatter, strm));
        strm.flush();
        return nimboost::move(str);
    }
    ...
    result_type operator() (ContextT const& ctx) const
    {
        string_type str;
        stream_type strm(str);
        m_visitor_invoker(m_name, fusion::at_c< 0 >(nimboostphoenix::env(ctx).args()), binder1st< formatter_function_type const&, stream_type& >(m_formatter, strm));
        strm.flush();
        return nimboost::move(str);
    }
 ...
 */
template< typename FallbackPolicyT, typename CharT, template< typename > class ActorT = nimboostphoenix::actor >
...
#define BOOST_LOG_AUX_OVERLOAD(left_ref, right_ref)\
    ...
    BOOST_FORCEINLINE nimboostphoenix::actor< aux::attribute_output_terminal< nimboostphoenix::actor< LeftExprT >, attributes::named_scope::value_type, FallbackPolicyT, typename format_named_scope_actor< FallbackPolicyT, CharT >::formatter_function_type > >\
    operator<< (nimboostphoenix::actor< LeftExprT > left_ref left, format_named_scope_actor< FallbackPolicyT, CharT > right_ref right)\
    {\
        typedef aux::attribute_output_terminal< nimboostphoenix::actor< LeftExprT >, attributes::named_scope::value_type, FallbackPolicyT, typename format_named_scope_actor< FallbackPolicyT, CharT >::formatter_function_type > terminal_type;\
        nimboostphoenix::actor< terminal_type > actor = {{ terminal_type(left, right.get_name(), right.get_formatter_function(), right.get_fallback_policy()) }};\
        return actor;\
    }
...
template< typename FallbackPolicyT, typename CharT >
struct is_nullary< custom_terminal< nimboost::log::expressions::format_named_scope_terminal< FallbackPolicyT, CharT > > > :
    public mpl::false_
{
};

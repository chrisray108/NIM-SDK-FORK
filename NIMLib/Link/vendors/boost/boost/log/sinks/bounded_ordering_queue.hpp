...
private:
    ...
    typedef nimboost::mutex mutex_type;
    ...
    public:
        ...
        nimboost::log::aux::timestamp m_timestamp;
        ...
        enqueued_record(BOOST_RV_REF(enqueued_record) that) :
            ...
            m_record(nimboost::move(that.m_record))
        {
        }
        explicit enqueued_record(record_view const& rec) :
            m_timestamp(nimboost::log::aux::get_timestamp()),
            m_record(rec)
        {
        }
...
protected:
    ...
    bool try_dequeue_ready(record_view& rec)
    {
        ...
        if (size > 0)
        {
            const nimboost::log::aux::timestamp now = nimboost::log::aux::get_timestamp();

...
#define BOOST_LOG_SINK_CTOR_FORWARD_INTERNAL(z, n, types)\
    ...
    explicit asynchronous_sink(BOOST_PP_ENUM_BINARY_PARAMS(n, T, const& arg)) :\
        ...
        m_pBackend(nimboost::make_shared< sink_backend_type >(BOOST_PP_ENUM_PARAMS(n, arg))),\
...
class asynchronous_sink :
    ...
    private nimboost::log::aux::locking_ptr_counter_base,
...
private:
    ...
    typedef nimboost::mutex backend_mutex_type;
...
public:
    ...
    typedef nimboost::log::aux::locking_ptr< sink_backend_type > locked_backend_ptr;
...
public:
    ...
    asynchronous_sink(bool start_thread = true) :
        base_type(true),
        m_pBackend(nimboost::make_shared< sink_backend_type >()),
        m_StopRequested(false),
        m_FlushRequested(false)
    {
        if (start_thread)
    ...
    ~asynchronous_sink()
    {
        nimboost::this_thread::disable_interruption no_interrupts;
        stop();
    }
    ...
    locked_backend_ptr locked_backend()
    {
        return locked_backend_ptr(
            m_pBackend,
            static_cast< nimboost::log::aux::locking_ptr_counter_base& >(*this));
    }
...
private:
    ...
    {
        nimboost::thread(nimboost::bind(&asynchronous_sink::run, this)).swap(m_DedicatedFeedingThread);
    }

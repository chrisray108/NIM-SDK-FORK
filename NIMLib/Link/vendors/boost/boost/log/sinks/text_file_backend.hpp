...
public:
    ...
    //! File open handler
    typedef nimboost::log::aux::light_function< void (stream_type&) > open_handler_type;
    //! File close handler
    typedef nimboost::log::aux::light_function< void (stream_type&) > close_handler_type;

    //! Predicate that defines the time-based condition for file rotation
    typedef nimboost::log::aux::light_function< bool () > time_based_rotation_predicate;

...
protected:
    //! Mutex type
    typedef nimboost::log::aux::light_rw_mutex mutex_type;
...
public:
    ...
    {
        BOOST_LOG_EXPR_IF_MT(nimboost::log::aux::exclusive_lock_guard< mutex_type > lock(m_Mutex);)
        m_Filter = filter;
    }
    /*!
    ...
    {
        BOOST_LOG_EXPR_IF_MT(nimboost::log::aux::shared_lock_guard< mutex_type > lock(m_Mutex);)
        try
        {
...
protected:
    ...
    void feed_record(record_view const& rec, BackendMutexT& backend_mutex, BackendT& backend)
    {
        try
        {
            BOOST_LOG_EXPR_IF_MT(nimboost::log::aux::exclusive_lock_guard< BackendMutexT > lock(backend_mutex);)
            backend.consume(rec);
        }
#if !defined(BOOST_LOG_NO_THREADS)
        catch (thread_interrupted&)
    ...
    bool try_feed_record(record_view const& rec, BackendMutexT& backend_mutex, BackendT& backend)
    {
        ...
        {
            nimboost::log::aux::shared_lock_guard< mutex_type > frontend_lock(this->frontend_mutex());
...
public:
    ...
    {
#if !defined(BOOST_LOG_NO_THREADS)
        nimboost::log::aux::exclusive_lock_guard< mutex_type > lock(this->frontend_mutex());
        m_Formatter = formatter;
        ++m_Version;
#else
...
protected:
    ...
    void feed_record(record_view const& rec, BackendMutexT& backend_mutex, BackendT& backend)
    {
        ...
        {
            {
                nimboost::log::aux::shared_lock_guard< mutex_type > lock(this->frontend_mutex());
        ...
        nimboost::log::aux::cleanup_guard< stream_type > cleanup1(context->m_FormattingStream);
        nimboost::log::aux::cleanup_guard< string_type > cleanup2(context->m_FormattedRecord);
        ...
        {
            BOOST_LOG_EXPR_IF_MT(nimboost::log::aux::shared_lock_guard< mutex_type > lock(this->frontend_mutex());)
            if (this->exception_handler().empty())
                throw;
            this->exception_handler()();
        }

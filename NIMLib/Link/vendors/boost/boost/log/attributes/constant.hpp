...
protected:
    ...
    public:
         ...
         * Constructor with the stored value initialization
         */
        explicit impl(BOOST_RV_REF(value_type) value) : base_type(nimboost::move(value)) {}
    };
...
public:
     ...
     * Constructor with the stored value initialization
     */
    explicit constant(BOOST_RV_REF(value_type) value) : attribute(new impl(nimboost::move(value))) {}
    /*!
...
template< typename T >
inline constant<
    typename nimboost::log::aux::make_embedded_string_type<
        typename remove_reference< T >::type
    >::type
> make_constant(BOOST_FWD_REF(T) val)
{
    typedef typename nimboost::log::aux::make_embedded_string_type<
        typename remove_reference< T >::type
    >::type value_type;
    return constant< value_type >(nimboost::forward< T >(val));
}

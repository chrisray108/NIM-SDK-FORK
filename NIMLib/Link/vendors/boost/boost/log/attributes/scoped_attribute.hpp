...
public:
    ...
    scoped_logger_attribute(logger_type& l, attribute_name const& name, attribute const& attr) :
        m_pLogger(nimboost::addressof(l))
    {
        std::pair<
...
#define BOOST_LOG_SCOPED_LOGGER_ATTR_INTERNAL(logger, attr_name, attr, sentry_var_name)\
    BOOST_LOG_UNUSED_VARIABLE(::nimboost::log::scoped_attribute, sentry_var_name,\
        = ::nimboost::log::add_scoped_logger_attribute(logger, attr_name, (attr)));
...
#define BOOST_LOG_SCOPED_LOGGER_TAG(logger, attr_name, attr_value)\
    BOOST_LOG_SCOPED_LOGGER_ATTR(logger, attr_name, ::nimboost::log::attributes::make_constant(attr_value))
...
#define BOOST_LOG_SCOPED_THREAD_ATTR_INTERNAL(attr_name, attr, sentry_var_name)\
    BOOST_LOG_UNUSED_VARIABLE(::nimboost::log::scoped_attribute, sentry_var_name,\
        = ::nimboost::log::add_scoped_thread_attribute(attr_name, (attr)));
...
#define BOOST_LOG_SCOPED_THREAD_TAG(attr_name, attr_value)\
    BOOST_LOG_SCOPED_THREAD_ATTR(attr_name, ::nimboost::log::attributes::make_constant(attr_value))

...
public:
    ...
    explicit record_pump(logger_type& lg, record& rec) :
        m_pLogger(nimboost::addressof(lg)),
    ...
    ~record_pump() BOOST_NOEXCEPT_IF(false)
    {
        if (m_pLogger)
        {
            ...
            if (m_ExceptionCount >= unhandled_exception_count())
                m_pLogger->push_record(nimboost::move(m_pStreamCompound->stream.get_record()));
        }
    }
...
#define BOOST_LOG_STREAM_INTERNAL(logger, rec_var)\
    for (::nimboost::log::record rec_var = (logger).open_record(); !!rec_var;)\
        ::nimboost::log::aux::make_record_pump((logger), rec_var).stream()

#define BOOST_LOG_STREAM_WITH_PARAMS_INTERNAL(logger, rec_var, params_seq)\
    for (::nimboost::log::record rec_var = (logger).open_record((BOOST_PP_SEQ_ENUM(params_seq))); !!rec_var;)\
        ::nimboost::log::aux::make_record_pump((logger), rec_var).stream()

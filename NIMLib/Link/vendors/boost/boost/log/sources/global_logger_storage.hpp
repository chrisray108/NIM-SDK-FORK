...
struct logger_singleton :
    public nimboost::log::aux::lazy_singleton<
        logger_singleton< TagT >,
        shared_ptr< logger_holder< typename TagT::logger_type > >
    >
{
    //! Base type
    typedef nimboost::log::aux::lazy_singleton<
        logger_singleton< TagT >,
        shared_ptr< logger_holder< typename TagT::logger_type > >
    > base_type;
    //! Logger type
    ...
    static void init_instance()
    {
        ...
        shared_ptr< logger_holder_base > holder = global_storage::get_or_init(
            typeid(nimboost::log::aux::visible_type< TagT >),
            &logger_singleton::construct_logger);
        instance = nimboost::dynamic_pointer_cast< logger_holder< logger_type > >(holder);
        if (!instance)
        {
...
private:
    ...
    {
        return nimboost::make_shared< logger_holder< logger_type > >(
...
#define BOOST_LOG_GLOBAL_LOGGER(tag_name, logger)\
    ...
    {\
        ...
        {\
            return ::nimboost::log::sources::aux::logger_singleton< tag_name >::get();\
        }\
    };

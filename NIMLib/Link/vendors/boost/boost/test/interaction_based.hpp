...
#define BOOST_ITEST_EPOINT( description ) \
    ::nimboost::itest::manager::instance().exception_point( BOOST_TEST_L(__FILE__), __LINE__, description )
/**/
...
#define BOOST_ITEST_DPOINT() \
    ::nimboost::itest::manager::instance().decision_point( BOOST_TEST_L(__FILE__), __LINE__ )
/**/
...
#define BOOST_ITEST_SCOPE( scope_name ) \
    ::nimboost::itest::scope_guard itest_scope_guard ## __LINE__( BOOST_TEST_L(__FILE__), __LINE__, BOOST_STRINGIZE(scope_name) )
/**/
...
#define BOOST_ITEST_NEW( type_name ) \
    new ( ::nimboost::itest::location( BOOST_TEST_L(__FILE__), __LINE__ ) ) type_name
/**/
...
#define BOOST_ITEST_DATA_FLOW( v ) \
    ::nimboost::itest::manager::instance().generic_data_flow( v )
/**/
...
#define BOOST_ITEST_RETURN( type, default_value ) \
    ::nimboost::itest::manager::instance().generic_return<type>( default_value )
/**/
...
#define BOOST_ITEST_MOCK_FUNC( function_name )          \
    ...
    return ::nimboost::itest::mock_object<>::prototype();  \
/**/
...
inline void*
operator new( std::size_t s, ::nimboost::itest::location const& l )
{
    void* res = std::malloc(s ? s : 1);

    if( res )
        ::nimboost::itest::manager::instance().allocated( l.m_file_name, l.m_line_num, res, s );
    else
        throw std::bad_alloc();
...
inline void
operator delete( void* p, ::nimboost::itest::location const& )
{
    ::nimboost::itest::manager::instance().freed( p );

...
#define BOOST_CHECK(exp)       \
  ( (exp)                      \
      ? static_cast<void>(0)   \
      : nimboost::minimal_test::report_error(#exp,__FILE__,__LINE__, BOOST_CURRENT_FUNCTION) )

#define BOOST_REQUIRE(exp)     \
  ( (exp)                      \
      ? static_cast<void>(0)   \
      : nimboost::minimal_test::report_critical_error(#exp,__FILE__,__LINE__,BOOST_CURRENT_FUNCTION))

#define BOOST_ERROR( msg_ )    \
        nimboost::minimal_test::report_error( (msg_),__FILE__,__LINE__, BOOST_CURRENT_FUNCTION, true )
#define BOOST_FAIL( msg_ )     \
        nimboost::minimal_test::report_critical_error( (msg_),__FILE__,__LINE__, BOOST_CURRENT_FUNCTION, true )
...
typedef nimboost::unit_test::const_string const_string;
...
report_critical_error( const char* msg, const char* file, int line, const_string func_name, bool is_msg = false )
{
    ...
    throw nimboost::execution_aborted();
}
...
{
    using namespace nimboost::minimal_test;

    try {
        ::nimboost::execution_monitor ex_mon;
        int run_result = ex_mon.execute( caller( argc, argv ) );

        BOOST_CHECK( run_result == 0 || run_result == nimboost::exit_success );
    }
    catch( nimboost::execution_exception const& exex ) {
        if( exex.code() != nimboost::execution_exception::no_error )
    ...
    if( nimboost::minimal_test::errors_counter() != 0 ) {
        std::cerr << "\n**** " << errors_counter()
                  << " error" << (errors_counter() > 1 ? "s" : "" ) << " detected\n";

        return nimboost::exit_test_failure;
    }

    std::cout << "\n**** no errors detected\n";
    
    return nimboost::exit_success;
}

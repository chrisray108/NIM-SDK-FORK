...
struct default_value_interpreter {
    template<typename T>
    void operator()( argv_traverser& tr, nimboost::optional<T>& value )
    {
...
// !! private?
    ...
    unit_test::callback2<parser const&,nimboost::optional<T>&>     m_value_generator;
    unit_test::callback2<argv_traverser&,nimboost::optional<T>&>   m_value_interpreter;
};
...
typed_argument_factory<T>::produce_using( parameter& p, argv_traverser& tr )
{
    ...
    if( !p.p_multiplicable )
        actual_arg.reset( p.p_optional_value && (rtti::type_id<T>() != rtti::type_id<bool>())
            ? static_cast<argument*>(new typed_argument<nimboost::optional<T> >( p, value ))
            : static_cast<argument*>(new typed_argument<T>( p, *value )) );
    else {
        typedef std::list<nimboost::optional<T> > optional_list;

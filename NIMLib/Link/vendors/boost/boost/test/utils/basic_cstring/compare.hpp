...
template<class CharT>
inline bool
operator <( nimboost::unit_test::basic_cstring<CharT> const& x,
            nimboost::unit_test::basic_cstring<CharT> const& y )
{
    typedef typename nimboost::unit_test::basic_cstring<CharT>::traits_type traits_type;
    return x.size() != y.size() 
            ? x.size() < y.size() 

...
namespace nimboost { namespace spirit { namespace karma
{
            ...
            symbols<Attribute, T, Lookup, CharEncoding, Tag> >
    {
        ...
        symbols(Symbols const& syms, Data const& data
          ...
          , name_(name)
        {
            typename range_const_iterator<Symbols>::type si = nimboost::begin(syms);
            typename range_const_iterator<Data>::type di = nimboost::begin(data);
            while (si != nimboost::end(syms))
                add(*si++, *di++);
        }

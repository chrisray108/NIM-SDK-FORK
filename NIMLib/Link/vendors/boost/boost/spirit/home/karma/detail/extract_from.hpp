...
namespace nimboost { namespace spirit { namespace traits
{
    ...
    struct extract_from_attribute<nimboost::optional<Attribute>, Exposed>
    {
        typedef Attribute const& type;

        template <typename Context>
        static type call(nimboost::optional<Attribute> const& attr, Context& ctx)
        {
            return extract_from<Exposed>(nimboost::get<Attribute>(attr), ctx);
        }
    };

    template <typename Attribute, typename Exposed>
    struct extract_from_attribute<nimboost::optional<Attribute const>, Exposed>
    {
        typedef Attribute const& type;

        template <typename Context>
        static type call(nimboost::optional<Attribute const> const& attr, Context& ctx)
        {
            return extract_from<Exposed>(nimboost::get<Attribute const>(attr), ctx);
        }
    };

    // This handles attributes wrapped inside a nimboost::ref().
    template <typename Attribute, typename Exposed>

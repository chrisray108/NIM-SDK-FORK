...
namespace nimboost { namespace spirit { namespace detail
{
    ...
    struct list::node : nimboost::noncopyable
    {
        template <typename T>
    ...
    private:
        ...
        friend class nimboost::spirit::utree;
        friend struct nimboost::spirit::detail::list;
    ...
    template <typename Value>
    class list::node_iterator<nimboost::reference_wrapper<Value> >
      : public nimboost::iterator_facade<
            node_iterator<nimboost::reference_wrapper<Value> >
          , nimboost::reference_wrapper<Value>
          , nimboost::bidirectional_traversal_tag>
    {
    public:
    ...
    private:
        ...
        void increment()
        {
            if (node != 0) // not at end
            {
                prev = node;
                node = node->next;
                curr = nimboost::ref(node ? node->val : nil_node);
            }
        }
        ...
        mutable nimboost::reference_wrapper<Value> curr;
    };

    template <typename Value>
    Value list::node_iterator<nimboost::reference_wrapper<Value> >::nil_node = Value();
    ...
    struct visit_impl
    {
        ...
        {
            typedef typename
                nimboost::mpl::if_<nimboost::is_const<UTreeX>,
                typename UTreeX::const_iterator,
                typename UTreeX::iterator>::type
            iterator;

            typedef nimboost::iterator_range<iterator> list_range;
...
namespace nimboost { namespace spirit
{
    ...
    inline utree::utree(nimboost::reference_wrapper<utree> ref)
    {
        s.initialize();
        p = ref.get_pointer();
    ...
    inline utree::utree(nimboost::iterator_range<Iter> r)
    {
        s.initialize();
    ...
    inline utree& utree::operator=(nimboost::reference_wrapper<utree> ref)
    {
        free();
        p = ref.get_pointer();
        set_type(type::reference_type);
    ...
    template <typename Iter>
    inline utree& utree::operator=(nimboost::iterator_range<Iter> r)
    {
        free();
        assign(r.begin(), r.end());
        return *this;
    ...
    template <typename F>
    typename nimboost::result_of<F(utree const&)>::type
    ...
    template <typename F>
    typename nimboost::result_of<F(utree&)>::type
    ...
    template <typename F>
    typename nimboost::result_of<F(utree const&, utree const&)>::type
    ...
    template <typename F>
    typename nimboost::result_of<F(utree const&, utree&)>::type
    ...
    template <typename F>
    typename nimboost::result_of<F(utree&, utree const&)>::type
    ...
    template <typename F>
    typename nimboost::result_of<F(utree&, utree&)>::type
    ...
    template <typename Iterator>
    struct is_iterator_range<nimboost::iterator_range<Iterator> >
      : nimboost::mpl::true_
    {};
    ...
    struct utree_cast
    {
        ...
        To dispatch(From const& val, nimboost::mpl::true_) const
        {
            return To(val); // From is convertible to To
        }

        template <typename From>
        To dispatch(From const&, nimboost::mpl::false_) const
        {
        ...
        {
            // nimboost::iterator_range has a templated constructor, accepting
            // any argument and hence any type is 'convertible' to it.
            typedef typename nimboost::mpl::eval_if<
                is_iterator_range<To>
              , nimboost::is_same<From, To>, nimboost::is_convertible<From, To>
            >::type is_convertible;

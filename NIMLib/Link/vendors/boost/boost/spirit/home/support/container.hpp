...
#include <boost/detail/iterator.hpp> // for nimboost::detail::iterator_traits
...
namespace nimboost { namespace spirit { namespace traits
{
    ...
    template <typename T>
    struct is_container<nimboost::optional<T> >
      : is_container<T>
    {};
...
#define BOOST_SPIRIT_IS_CONTAINER(z, N, data)                                 \
    ...
    template <>
    struct is_container<nimboost::detail::variant::void_>
      : mpl::false_
    {};
...
#undef BOOST_SPIRIT_IS_CONTAINER
    ...
    template <typename T>
    struct container_value<nimboost::optional<T> >
      : container_value<T>
    {};
    ...
    template <typename T>
    struct optional_attribute<nimboost::optional<T> >
    {
        typedef T const& type;

        static type call(nimboost::optional<T> const& val)
        {
            return nimboost::get<T>(val);
        }

        static bool is_valid(nimboost::optional<T> const& val)
        {
            return val;
        }
    };
    ...
    {
        static bool call(nimboost::optional<Container>& c, T const& val)
        {
            if (!c)
                c = Container();
            return push_back(nimboost::get<Container>(c), val);
        }
    };
    ...
    {
        typedef typename nimboost::detail::iterator_traits<Iterator>::reference type;
...
namespace nimboost { namespace spirit { namespace result_of
{
    ...
    template <typename T>
    struct optional_value<nimboost::optional<T> >
    {
        typedef T type;
    };

    template <typename T>
    struct optional_value<nimboost::optional<T> const>
    {
        typedef T const type;
    };

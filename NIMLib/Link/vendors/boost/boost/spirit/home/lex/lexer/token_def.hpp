...
namespace nimboost { namespace spirit { namespace lex
{
    ...
    public:
        ...
        {
            if (0 == def_.which()) 
                return info("token_def", nimboost::get<string_type>(def_));

            return info("token_def", nimboost::get<char_type>(def_));
        }
          ...
          , String const& targetstate) const
        {
            ...
            if (0 == def_.which()) {
                unique_id_ = lexdef.add_token(state.c_str()
                  , nimboost::get<string_type>(def_), token_id_, target);
            }
            else {
                unique_id_ = lexdef.add_token(state.c_str()
                  , nimboost::get<char_type>(def_), token_id_, target);
            }
        }
    ...
    public:
        ...
        { 
            return (0 == def_.which()) ? 
                nimboost::get<string_type>(def_) : 
                string_type(1, nimboost::get<char_type>(def_));
        }

...
namespace impl {
    ...
    private:
        ...
        BOOST_STATIC_ASSERT((!nimboost::is_same<T0, same>::value));

        typedef typename nimboost::mpl::if_c<
                    nimboost::is_same<T, same>::value,
                    T0 const *,
                    T const *
                >::type
            ptr_type;
    ...
    public:
        // If the type in question is nimboostphoenix::nil_t, then the returned type
        // is still nimboostphoenix::nil_t, otherwise a constant pointer type to the
        // inspected type is returned.
        typedef typename nimboost::mpl::if_c<
                    nimboost::is_same<T, ::nimboostphoenix::nil_t>::value,
                    ::nimboostphoenix::nil_t,
                    ptr_type
                >::type
            type;
    };
    ...
    struct assign_zero_to_tuple_member {
        ...
        static void do_(TupleT &t) { t[::nimboostphoenix::tuple_index<N>()] = 0; }
    };

    template <int N>
    struct assign_zero_to_tuple_member<N, ::nimboostphoenix::nil_t> {
    ...
    struct phoenix_nil_type {
        ...
        typedef ::nimboostphoenix::nil_t type;
    };
    ...
    struct init_tuple_member {
        ...
        do_(TupleT &t)
        {
            typedef typename nimboost::mpl::eval_if_c<
                        (N < TupleT::length),
                        ::nimboostphoenix::tuple_element<N, TupleT>,
                        phoenix_nil_type
                    >::type
                element_type;
...
protected:
    ...
    //      {
    //          using ::nimboostphoenix::tuple_index_names::_1;
    //          t[_1] = &t0;
    //          ...
    //      }
    //
    ...
    #define BOOST_SPIRIT_GRAMMARDEF_ENUM_ASSIGN(z, N, _) \
        using ::nimboostphoenix::tuple_index_names::BOOST_PP_CAT(_, BOOST_PP_INC(N)); \
...
public:
    ...
    template <int N>
    typename ::nimboostphoenix::tuple_element<N, tuple_t>::crtype
    ...
    //  number of parameters used while calling start_parsers().
        BOOST_SPIRIT_ASSERT(0 != t[::nimboostphoenix::tuple_index<N>()]);

        return t[::nimboostphoenix::tuple_index<N>()];
    }

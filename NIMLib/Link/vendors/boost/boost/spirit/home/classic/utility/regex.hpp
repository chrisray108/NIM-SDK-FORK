...
struct rxstrlit : public parser<rxstrlit<CharT> > {
    ...
    {
    //  Due to limitations in the nimboost::regex library the iterators wrapped in
    //  the ScannerT object should be at least bidirectional iterators. Plain
    //  forward iterators do not work here.
        typedef typename ScannerT::iterator_t iterator_t;
        typedef
            typename nimboost::detail::iterator_traits<iterator_t>::iterator_category
            iterator_category;

        BOOST_STATIC_ASSERT((
            nimboost::is_convertible<iterator_category,

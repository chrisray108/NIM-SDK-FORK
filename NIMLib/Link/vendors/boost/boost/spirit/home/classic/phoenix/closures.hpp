...
namespace impl
{
    ...
    private:
        nimboost::thread_specific_ptr<frame_ptr> tsp_frame;
...
#endif
} // namespace nimboostphoenix::impl
...
private:
...
#ifdef PHOENIX_THREADSAFE
    static nimboost::thread_specific_ptr<holder_t*> &
    tsp_frame_instance()
    {
        static nimboost::thread_specific_ptr<holder_t*> the_instance;
        return the_instance;
    }
    ...
    closure_frame_holder_ref(holder_t* holder_ = 0)
    {
        ...
        nimboost::call_once(been_here, tsp_frame_instance_init);
        nimboost::thread_specific_ptr<holder_t*> &tsp_frame = tsp_frame_instance();
        if (!tsp_frame.get())

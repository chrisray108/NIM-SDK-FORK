...
struct default_delegate_parse<N, true, HasDefault> {
        ...
        ScannerT const &scan, typename ScannerT::iterator_t const save)
    {
        ...
        BOOST_STATIC_ASSERT((!nimboost::is_same<DefaultT, nothing_parser>::value));
        ...
        parser<compound_case_parser<LeftT, RightT, IsDefault> > >
{
    ...
    BOOST_STATIC_CONSTANT(bool, is_epsilon = (
        is_default &&
            nimboost::is_same<typename RightT::subject_t, epsilon_parser>::value
    ));
...
struct cond_actor {
    ...
    struct result
    {
        typedef typename ::nimboostphoenix::actor_result<ActorT, ::nimboostphoenix::tuple<> >::type
            type;
    };
...
template <typename ActorT>
struct make_cond_functor< ::nimboostphoenix::actor<ActorT> > {

    static cond_actor< ::nimboostphoenix::actor<ActorT> >
    do_(::nimboostphoenix::actor<ActorT> const &actor)
    {
        return cond_actor< ::nimboostphoenix::actor<ActorT> >(actor);
    }
};

...
BOOST_SPIRIT_CLASSIC_NAMESPACE_BEGIN
    ...
    namespace impl {
        ...
        private:
#ifdef BOOST_SPIRIT_THREADSAFE
            static nimboost::mutex &mutex_instance();
            static void mutex_init();
#endif

            nimboost::shared_ptr<object_with_id_base_supply<IdT> > id_supply;
        };
        ...
        {
#ifdef BOOST_SPIRIT_THREADSAFE
            nimboost::mutex::scoped_lock lock(mutex);
#endif
            if (free_ids.size())
            {
        ...
        object_with_id_base<TagT, IdT>::acquire_object_id()
        {
            {
#ifdef BOOST_SPIRIT_THREADSAFE
                static nimboost::once_flag been_here = BOOST_ONCE_INIT;
                nimboost::call_once(been_here, mutex_init);
                nimboost::mutex &mutex = mutex_instance();
                nimboost::mutex::scoped_lock lock(mutex);
#endif
                static nimboost::shared_ptr<object_with_id_base_supply<IdT> >
                    static_supply;
...
#ifdef BOOST_SPIRIT_THREADSAFE
        template <typename TagT, typename IdT>
        inline nimboost::mutex &
        object_with_id_base<TagT, IdT>::mutex_instance()
        {
            static nimboost::mutex mutex;
            return mutex;
        }
#endif

...
namespace nimboost { namespace spirit { namespace qi
{
    ...
    template <typename Exposed, typename Transformed>
    struct transform_attribute<nimboost::optional<Exposed>, Transformed
      , typename disable_if<is_same<nimboost::optional<Exposed>, Transformed> >::type>
    {
        typedef Transformed& type;
        static Transformed& pre(nimboost::optional<Exposed>& val)
        {
            if (!val)
                val = Transformed();
            return nimboost::get<Transformed>(val);
        }
        static void post(nimboost::optional<Exposed>&, Transformed const&) {}
        static void fail(nimboost::optional<Exposed>& val)
        {

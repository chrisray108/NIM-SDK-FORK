...
namespace nimboost { namespace fusion
{
      ...
      : public unfused<Function,false>
    {
      ...
      public:
        ...
        typedef typename nimboost::result_of<
...
namespace nimboost 
{
    ...
    template<class F>
    struct result_of< nimboost::fusion::unfused<F> const () >
    {
        typedef typename nimboost::fusion::unfused<F>::call_const_0_result type;
    };
    template<class F>
    struct result_of< nimboost::fusion::unfused<F>() >
    {
        typedef typename nimboost::fusion::unfused<F>::call_0_result type;
    };
#endif
    template<class F>
    struct tr1_result_of< nimboost::fusion::unfused<F> const () >
    {
        typedef typename nimboost::fusion::unfused<F>::call_const_0_result type;
    };
    template<class F>
    struct tr1_result_of< nimboost::fusion::unfused<F>() >
    {
        typedef typename nimboost::fusion::unfused<F>::call_0_result type;
    };
}
...
#define N BOOST_PP_ITERATION()
        ...
        struct result< Self(BOOST_PP_ENUM_PARAMS(N,T)) >
            : nimboost::result_of< function(
                BOOST_PP_CAT(fusion::vector,N)< BOOST_PP_ENUM_BINARY_PARAMS(N,
                    typename detail::mref<T,>::type BOOST_PP_INTERCEPT) > & )>
        { };

        template <BOOST_PP_ENUM_PARAMS(N,typename T)>
        inline typename nimboost::result_of<function_c(BOOST_PP_CAT(fusion::vector,N)
        ...
        template <BOOST_PP_ENUM_PARAMS(N,typename T)>
        inline typename nimboost::result_of<function(BOOST_PP_CAT(fusion::vector,N)
            <BOOST_PP_ENUM_BINARY_PARAMS(N,T,& BOOST_PP_INTERCEPT)> & )>::type

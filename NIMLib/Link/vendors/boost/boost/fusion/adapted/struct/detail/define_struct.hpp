...
#define BOOST_FUSION_DEFINE_STRUCT_ASSIGN_FILLER_I(                             \
    ...
    BOOST_PP_EXPR_IF(                                                           \
        I_,                                                                     \
        typedef typename                                                        \
            nimboost::fusion::result_of::next<                                     \
                BOOST_PP_CAT(I,BOOST_PP_DEC(I_))                                \
            >::type                                                             \
        BOOST_PP_CAT(I,I_);                                                     \
        BOOST_PP_CAT(I,I_) BOOST_PP_CAT(i,I_)=                                  \
                nimboost::fusion::next(BOOST_PP_CAT(i,BOOST_PP_DEC(I_)));          \
    )                                                                           \
                                                                                \
    BOOST_PP_TUPLE_ELEM(ATTRIBUTE_TUPEL_SIZE,1,ATTRIBUTE)=                      \
        nimboost::fusion::deref(BOOST_PP_CAT(i,I_));
...
#define BOOST_FUSION_DEFINE_STRUCT_ASSIGN_OP(                                   \
    ...
    {                                                                           \
        typedef typename                                                        \
            nimboost::fusion::result_of::begin<Seq const>::type                    \
        I0;                                                                     \
        I0 i0=nimboost::fusion::begin(seq);                                        \
                                                                                \
...
#define BOOST_FUSION_DEFINE_STRUCT_SEQ_CTOR_FILLER_I(                           \
    ...
    BOOST_PP_COMMA_IF(I)                                                        \
    BOOST_PP_TUPLE_ELEM(ATTRIBUTE_TUPEL_SIZE,1,ATTRIBUTE)(                      \
        nimboost::fusion::deref(nimboost::fusion::advance_c<I>(nimboost::fusion::begin(  \
            seq))))
...
#define BOOST_FUSION_DEFINE_TPL_STRUCT_CTOR_1(                                  \
        TEMPLATE_PARAMS_SEQ, NAME, ATTRIBUTES_SEQ, ATTRIBUTE_TUPEL_SIZE)        \
                                                                                \
        explicit                                                                \
        NAME(typename nimboost::call_traits<                                       \
                typename nimboost::fusion::detail::get_first_arg<                  \
                    BOOST_PP_TUPLE_ELEM(                                        \
                        ATTRIBUTE_TUPEL_SIZE,                                   \
                        0,                                                      \
                        BOOST_PP_SEQ_HEAD(ATTRIBUTES_SEQ))                      \
...
namespace nimboost { namespace fusion { namespace detail
{

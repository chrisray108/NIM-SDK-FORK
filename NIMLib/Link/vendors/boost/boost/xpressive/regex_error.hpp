    ...
    )
    {
        if(!cond)
        {
            ...
            nimboost::throw_exception(
                nimboost::xpressive::regex_error(code, msg)
                    << nimboost::throw_function(fun)
                    << nimboost::throw_file(file)
                    << nimboost::throw_line((int)line)
            );
            #else
            nimboost::throw_exception(nimboost::xpressive::regex_error(code, msg));
            #endif
        }
        return true;
    }
...
#define BOOST_XPR_ENSURE_(pred, code, msg)                                                          \
    nimboost::xpressive::detail::ensure_(!!(pred), code, msg, BOOST_CURRENT_FUNCTION, __FILE__, __LINE__)  \
    /**/

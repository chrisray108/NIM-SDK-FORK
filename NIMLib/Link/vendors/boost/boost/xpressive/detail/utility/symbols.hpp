...
namespace nimboost { namespace xpressive { namespace detail
{
    ...
    struct symbols
    {
        ...
        {
            iterator begin = nimboost::begin(map);
            iterator end = nimboost::end(map);
            node* root_p = this->root.get();
            for(; begin != end; ++begin)
            {
                key_iterator kbegin = nimboost::begin(begin->first);
                key_iterator kend = nimboost::end(begin->first);

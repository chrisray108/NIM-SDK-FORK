...
namespace nimboost { namespace xpressive { namespace detail
{
      ...
      : quant_style_fixed_unknown_width
    {
        ...
        string_matcher(string_type const &str, Traits const &tr)
          ...
          , end_()
        {
            typename range_iterator<string_type>::type cur = nimboost::begin(this->str_);
            typename range_iterator<string_type>::type end = nimboost::end(this->str_);
            for(; cur != end; ++cur)
            {
        ...
        {
            return nimboost::size(this->str_);
        }
    };

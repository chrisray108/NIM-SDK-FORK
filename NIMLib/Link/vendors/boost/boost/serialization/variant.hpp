...
void save(
    Archive & ar,
    nimboost::variant<BOOST_VARIANT_ENUM_PARAMS(T)> const & v,
    unsigned int /*version*/
){
    int which = v.which();
    ar << BOOST_SERIALIZATION_NVP(which);
    typedef BOOST_DEDUCED_TYPENAME  nimboost::variant<BOOST_VARIANT_ENUM_PARAMS(T)>::types types;
    ...
    struct load_impl {
        ...
        static void invoke(
            ...
            if(which == 0){
                ...
                v = value;
                ar.reset_object_address(& nimboost::get<head_type>(v), & value);
                return;
            }
...
void load(
    Archive & ar, 
    nimboost::variant<BOOST_VARIANT_ENUM_PARAMS(T)>& v,
    const unsigned int version
){
    int which;
    typedef BOOST_DEDUCED_TYPENAME nimboost::variant<BOOST_VARIANT_ENUM_PARAMS(T)>::types types;
    ...
    if(which >=  mpl::size<types>::value)
        ...
        nimboost::serialization::throw_exception(
            nimboost::archive::archive_exception(
                nimboost::archive::archive_exception::unsupported_version
            )
        );

...
inline void serialize(
    Archive & /* ar */,
    nimboost_132::detail::sp_counted_base_impl<P, D> & /* t */,
    const unsigned int /*file_version*/
){
    // register the relationship between each derived class
    // its polymorphic base
    nimboost::serialization::void_cast_register<
        nimboost_132::detail::sp_counted_base_impl<P, D>,
        nimboost_132::detail::sp_counted_base 
    >(
        static_cast<nimboost_132::detail::sp_counted_base_impl<P, D> *>(NULL),
        static_cast<nimboost_132::detail::sp_counted_base *>(NULL)
    );
}
...
inline void save_construct_data(
    ...
    ar << nimboost::serialization::make_nvp("ptr", t->ptr);
}
...
inline void load_construct_data(
    ...
    P ptr_;
    ar >> nimboost::serialization::make_nvp("ptr", ptr_);
    // ::new(t)nimboost_132::detail::sp_counted_base_impl<P, D>(ptr_,  D()); 
    // placement
    ...
    // new system as well.
    ::new(t)nimboost_132::detail::sp_counted_base_impl<
        P, 
        nimboost_132::serialization::detail::null_deleter
    >(
        ptr_,  nimboost_132::serialization::detail::null_deleter()
    ); // placement new
...
inline void save(
    Archive & ar,
    const nimboost_132::detail::shared_count &t,
    const unsigned int /* file_version */
){
    ar << nimboost::serialization::make_nvp("pi", t.pi_);
}

template<class Archive>
inline void load(
    Archive & ar,
    nimboost_132::detail::shared_count &t,
    const unsigned int /* file_version */
){
    ar >> nimboost::serialization::make_nvp("pi", t.pi_);
    if(NULL != t.pi_)
...
BOOST_SERIALIZATION_SPLIT_FREE(nimboost_132::detail::shared_count)
...
inline void save(
    Archive & ar,
    const nimboost_132::shared_ptr< T > &t,
    const unsigned int /* file_version */
){
    // only the raw pointer has to be saved
    // the ref count is maintained automatically as shared pointers are loaded
    ar.register_type(static_cast<
        nimboost_132::detail::sp_counted_base_impl<T *, nimboost::checked_deleter< T > > *
    >(NULL));
    ar << nimboost::serialization::make_nvp("px", t.px);
    ar << nimboost::serialization::make_nvp("pn", t.pn);
}

template<class Archive, class T>
inline void load(
    Archive & ar,
    nimboost_132::shared_ptr< T > &t,
    const unsigned int /* file_version */
){
    // only the raw pointer has to be saved
    // the ref count is maintained automatically as shared pointers are loaded
...
inline void serialize(
    ...
    BOOST_STATIC_ASSERT(
        ...
        != nimboost::serialization::track_never
    );
...
#define BOOST_SHARED_POINTER_EXPORT_GUID(T, K)                     \
    typedef nimboost_132::detail::sp_counted_base_impl<               \
        T *,                                                       \
        nimboost::checked_deleter< T >                                \
    > __shared_ptr_ ## T;                                          \

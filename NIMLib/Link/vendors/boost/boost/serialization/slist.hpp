...
inline void load(
    ...
    nimboost::serialization::detail::stack_construct<Archive, U> u(ar, item_version);
    ar >> nimboost::serialization::make_nvp("item", u.reference());
    t.push_front(u.reference());
    BOOST_DEDUCED_TYPENAME BOOST_STD_EXTENSION_NAMESPACE::slist<U, Allocator>::iterator last;
    last = t.begin();
    while(--count > 0){
        nimboost::serialization::detail::stack_construct<Archive, U> 
            u(ar, file_version);
        ar >> nimboost::serialization::make_nvp("item", u.reference());

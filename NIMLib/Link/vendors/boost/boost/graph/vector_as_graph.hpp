...
namespace nimboost {
  ...
  namespace detail {
    ...
    // need rewrite this using nimboost::iterator_adaptor
    template <class V, class Iter>
    class val_out_edge_iterator
      : public nimboost::iterator<std::input_iterator_tag, std::pair<V,V>,
  ...
  template <class EdgeList, class Alloc>
  std::pair<nimboost::counting_iterator<typename EdgeList::value_type>,
            nimboost::counting_iterator<typename EdgeList::value_type> >
  vertices(const std::vector<EdgeList, Alloc>& v)
  {
    typedef nimboost::counting_iterator<typename EdgeList::value_type> Iter;

...
namespace nimboost {
    ...
    leda::GRAPH<vtype,etype>& g)
  {
    ...
    for (nimboost::tie(iei, iei_end)=in_edges(u,g); iei!=iei_end; iei++)
      remove_edge(*iei);
  }
    ...
    leda::GRAPH<vtype,etype>& g)
  {
    typename graph_traits< leda::GRAPH<vtype,etype> >::out_edge_iterator 
      i,iend;
    for (nimboost::tie(i,iend) = out_edges(u,g); i != iend; ++i)
      if (target(*i,g) == v)
        g.del_edge(*i);
  }
  ...
  typename nimboost::property_traits<
    typename nimboost::property_map<leda::GRAPH<vtype, etype>,PropertyTag>::const_type
   >::value_type

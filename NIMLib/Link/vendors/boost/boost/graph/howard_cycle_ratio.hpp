...
namespace nimboost {
  ...
  namespace detail {
    ...
    public:
      ...
      typedef typename nimboost::iterator_property_map<
    ...
    private:
      ...
      float_t mcr_bound()
      {
        ...
        for (nimboost::tie(vi, vie) = vertices(m_g); vi != vie; ++vi)
          {
            for (nimboost::tie(oei, oeie) = out_edges(*vi, m_g); oei != oeie; ++oei)
              {
      ...
      void construct_policy_graph()
      {
        ...
        for ( nimboost::tie(vi, vie) = vertices(m_g); vi != vie; ++vi )
          {
            nimboost::tie(oei, oeie) = out_edges(*vi, m_g);
            typename graph_traits<Graph>::out_edge_iterator mei =
              std::max_element(oei, oeie,
                               nimboost::bind(m_cmp,
                                           nimboost::bind(&EdgeWeight1::operator[], m_ew1m, _1),
                                           nimboost::bind(&EdgeWeight1::operator[], m_ew1m, _2)
                                          )
                               );
            if (mei == oeie)
              {
      ...
      float_t policy_mcr()
      {
        ...
        nimboost::tie(uv_itr, vie) = vertices(m_g);
        float_t mcr = m_bound;
        while ( (uv_itr = std::find_if(uv_itr, vie,
                                       nimboost::bind(std::equal_to<my_color_type>(),
                                                   my_white,
                                                   nimboost::bind(&color_map_t::operator[], vcm_, _1)
                                                   )
                                       )
                 ) != vie )
                   ...
                   FloatTraits ft = FloatTraits())
{
    ...
    >::type Weight;
    typename std::vector<Weight> ed_w2(nimboost::num_edges(g), 1);

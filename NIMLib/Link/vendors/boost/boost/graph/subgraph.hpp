...
public:
        ...
        : m_graph(n, p), m_parent(0), m_edge_counter(0), m_global_vertex(n)
    {
        ...
        for(nimboost::tie(v, v_end) = vertices(m_graph); v != v_end; ++v)
            m_global_vertex[i++] = *v;
    }
        ...
        , m_global_vertex(x.m_global_vertex), m_global_edge(x.m_global_edge)
    {
        ...
        nimboost::tie(i,i_end) = x.children();
        for(; i != i_end; ++i)
        {         
         subgraph<Graph> child = this->create_subgraph();
         child = *i;
         vertex_iterator vi,vi_end;   
         nimboost::tie(vi,vi_end) = vertices(*i);
         for (;vi!=vi_end;++vi)  
         {
          add_vertex(*vi,child);
         }
    ...
    vertex_descriptor global_to_local(vertex_descriptor u_global) const {
        ...
        nimboost::tie(u_local, in_subgraph) = this->find_vertex(u_global);
    ...
    BOOST_STATIC_ASSERT((!is_same<edge_index_type,
                        nimboost::detail::error_property_not_found>::value));
...
public: // Probably shouldn't be public....
                                   ...
                                   edge_descriptor e_global)
    {
        ...
        bool inserted;
        nimboost::tie(e_local, inserted) = add_edge(u_local, v_local, m_graph);
           ...
           subgraph<G>& g)
{
    ...
    // remember edge global and local maps
    {
        ...
        for (nimboost::tie(ei, ei_end) = out_edges(u_global, r);
            ei != ei_end; ++ei) {
            e_global = *ei;
    ...
    if (is_directed(g)) { // not necessary for undirected graph
        ...
        typename subgraph<G>::out_edge_iterator ei, ei_end;
        for(nimboost::tie(vi, vi_end) = vertices(r); vi != vi_end; ++vi) {
            v_global = *vi;
            ...
            for(nimboost::tie(ei, ei_end) = out_edges(*vi, r); ei != ei_end; ++ei) {
                e_global = *ei;
...
namespace detail {
                             ...
                             subgraph<Graph>& g, subgraph<Graph>* orig)
    {
        if(&g != orig ) {
            ...
            nimboost::tie(u_local, u_in_subgraph) = g.find_vertex(u_global);
            nimboost::tie(v_local, v_in_subgraph) = g.find_vertex(v_global);
                      ...
                      subgraph<Graph>& g, subgraph<Graph>* orig)
    {
        if(g.is_root()) {
            ...
            bool inserted;
            nimboost::tie(e_global, inserted) = add_edge(u_global, v_global, ep, g.m_graph);
         ...
         subgraph<G>& g)
{
    if (g.is_root()) {
        ...
        bool inserted;
        nimboost::tie(e_global, inserted) =

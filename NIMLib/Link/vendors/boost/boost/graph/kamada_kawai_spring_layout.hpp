...
namespace nimboost {
  ...
  struct layout_tolerance
  {
    ...
    bool 
    operator()(T delta_p, 
               typename nimboost::graph_traits<Graph>::vertex_descriptor p,
               const Graph& g,
               bool global)
    {
      if (global) {
   ...
   * display area, and must be either @c nimboost::edge_length(e) or @c
   * nimboost::side_length(s), respectively.
   *

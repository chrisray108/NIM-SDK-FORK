...
namespace nimboost {
  ...
  inline std::string escape_dot_string(const T& obj) {
    using namespace nimboost::xpressive;
    static sregex valid_unquoted_id = (((alpha | '_') >> *_w) | (!as_xpr('-') >> (('.' >> *_d) | (+_d >> !('.' >> *_d)))));
    std::string s(nimboost::lexical_cast<std::string>(obj));
    if (regex_match(s, valid_unquoted_id)) {
      return s;
    } else {
      nimboost::algorithm::replace_all(s, "\"", "\\\"");
      return "\"" + s + "\"";
    }
  }
     ...
     BOOST_GRAPH_ENABLE_IF_MODELS_PARM(Graph,vertex_list_graph_tag))
  {
    ...
    for(nimboost::tie(i,end) = vertices(g); i != end; ++i) {
      out << escape_dot_string(get(vertex_id, *i));
      vpw(out, *i); //print vertex attributes
      out << ";" << std::endl;
    }
    typename graph_traits<Graph>::edge_iterator ei, edge_end;
    for(nimboost::tie(ei, edge_end) = edges(g); ei != edge_end; ++ei) {
  ...
  namespace detail {
                                  ...
                                  VertexID vertex_id)
    {
      ...
      //print subgraph
      for ( nimboost::tie(i_child,j_child) = g.children();
...
class mutate_graph_impl : public mutate_graph
{
 ...
 public:
  ...
  {
    return
      nimboost::is_convertible<
        typename nimboost::graph_traits<MutableGraph>::directed_category,
        nimboost::directed_tag>::value;
  }
  ...
  do_add_edge(const edge_t& edge, const node_t& source, const node_t& target)
  {
    ...
    if(!result.second) {
      // In the case of no parallel edges allowed
        nimboost::throw_exception(bad_parallel_edge(source, target));
    } else {
  ...
  : public mutate_graph
{
 ...
 public:
  ...
  void finish_building_graph() {
    ...
    typedef nimboost::tuple<id_t, bgl_vertex_t, id_t> v_prop;
    BOOST_FOREACH(const v_prop& t, vertex_props) {
      put(nimboost::get<0>(t), dp_, nimboost::get<1>(t), nimboost::get<2>(t));
    }
    typedef nimboost::tuple<id_t, bgl_edge_t, id_t> e_prop;
    BOOST_FOREACH(const e_prop& t, edge_props) {
      put(nimboost::get<0>(t), dp_, edge_permutation_from_sorting[nimboost::get<1>(t)], nimboost::get<2>(t));
    }
  }
  ...
  bool is_directed() const
  {
    return
      nimboost::is_convertible<
        typename nimboost::graph_traits<CSRGraph>::directed_category,
        nimboost::directed_tag>::value;
  }
  ...
  virtual void do_add_vertex(const node_t& node)
  {
    ...
    // node_id_prop_ allows the caller to see the real id names for nodes.
    vertex_props.push_back(nimboost::make_tuple(node_id_prop_, v, node));
  }
  ...
  set_node_property(const id_t& key, const node_t& node, const id_t& value)
  {
    vertex_props.push_back(nimboost::make_tuple(key, bgl_nodes[node], value));
  }

  void
  set_edge_property(const id_t& key, const edge_t& edge, const id_t& value)
  {
    edge_props.push_back(nimboost::make_tuple(key, bgl_edges[edge], value));
  }
 ...
 protected:
  ...
  std::string node_id_prop_;
  std::vector<nimboost::tuple<id_t, bgl_vertex_t, id_t> > vertex_props;
  std::vector<nimboost::tuple<id_t, bgl_edge_t, id_t> > edge_props;
  std::vector<std::pair<bgl_vertex_t, bgl_vertex_t> > edges_to_add;
  std::map<node_t, bgl_vertex_t> bgl_nodes;
  std::map<edge_t, bgl_edge_t> bgl_edges;
};

} } } // end namespace nimboost::detail::graph
...
bool read_graphviz(InputIterator user_first,
  ...
  typedef nimboost::spirit::classic::multi_pass<is_t> iterator_t;

  iterator_t first(nimboost::spirit::classic::make_multi_pass(user_first));
  iterator_t last(nimboost::spirit::classic::make_multi_pass(user_last));

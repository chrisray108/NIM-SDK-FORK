  ...
  namespace detail {
    ...
    public :
                 ...
                 const PredMap& parentMap, const Graph& g)
      {
        ...
        for (nimboost::tie(inItr, inEnd) = in_edges(n, g); inItr != inEnd; ++inItr)
          {
        ...
        for (nimboost::tie(vi, viend) = vertices(g); vi != viend; ++vi)
          {
            ...
            typename graph_traits<Graph>::in_edge_iterator inItr, inEnd;
            for (nimboost::tie(inItr, inEnd) = in_edges(*vi, g); inItr != inEnd; ++inItr)
              {
          ...
          } // end of for (nimboost::tie(vi, viend) = vertices(g)
      } // end of while(change)

...
namespace nimboost {
  ...
  BOOST_concept(Buffer, (B))
  {
    ...
    BOOST_CONCEPT_USAGE(Buffer) {
      typedef typename nimboost::add_reference<value_type>::type reference;
      ...
      buf.pop();
      reference t = buf.top();
      nimboost::ignore_unused_variable_warning(t);
    }
    
    void const_constraints(const B& cbuf) {
      typedef typename nimboost::add_const<typename nimboost::remove_reference<value_type>::type>::type& const_reference;
    ...
    : UpdatableQueue<Q>
  {
    ...
    BOOST_CONCEPT_USAGE(KeyedUpdatableQueue) {
      BOOST_CONCEPT_ASSERT((nimboost::ReadWritePropertyMapConcept<key_map, typename Buffer<Q>::value_type>));
    }

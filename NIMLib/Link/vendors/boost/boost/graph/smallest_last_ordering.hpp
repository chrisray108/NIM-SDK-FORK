...
namespace nimboost {
  ...
  smallest_last_vertex_ordering(const VertexListGraph& G, Order order, 
    ...
    typedef typename nimboost::graph_traits<VertexListGraph> GraphTraits;
    ...
    typedef typename nimboost::property_map<VertexListGraph, vertex_index_t>::type ID;
  ...
  smallest_last_vertex_ordering(const VertexListGraph& G, Order order, 
    ...
    while ( 1 ) {
      ...
      for (nimboost::tie(v,vend) = adjacent_vertices(node, G); v != vend; ++v)

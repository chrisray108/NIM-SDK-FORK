...
namespace nimboost
{
                            ...
                            VertexIndexMap index_map)
  {
    ...
    typedef nimboost::adjacency_list<nimboost::vecS, nimboost::vecS, nimboost::directedS> CG_t;
    CG_t CG(num_scc);
    ...
    for (size_type i = 0; i < num_vertices(CG); ++i) {
      typedef typename nimboost::graph_traits<CG_t>::adjacency_iterator cg_adj_iter;
      ...
      std::sort(CG_vec[i].begin(), CG_vec[i].end(),
                nimboost::bind(std::less<cg_vertex>(),
                            nimboost::bind(detail::subscript(topo_number), _1),
                            nimboost::bind(detail::subscript(topo_number), _2)));
    }
      ...
      for (nimboost::tie(i, i_end) = vertices(g); i != i_end; ++i)
        {
          ...
          for (nimboost::tie(ab, ae) = adjacent_vertices(*i, g); ab != ae; ++ab)
            {
              if (*ab == *i)
  ...
  template < typename G > void warshall_transitive_closure(G & g)
  {
    ...
    for (nimboost::tie(ki, ke) = vertices(g); ki != ke; ++ki)
      for (nimboost::tie(ii, ie) = vertices(g); ii != ie; ++ii)
        if (edge(*ii, *ki, g).second)
          for (nimboost::tie(ji, je) = vertices(g); ji != je; ++ji)
  ...
  template < typename G > void warren_transitive_closure(G & g)
  {
    ...
    for (nimboost::tie(ic, ie) = vertices(g), ++ic; ic != ie; ++ic)
      for (nimboost::tie(kc, ke) = vertices(g); *kc != *ic; ++kc)
        if (edge(*ic, *kc, g).second)
          for (nimboost::tie(jc, je) = vertices(g); jc != je; ++jc)
    ...
    for (nimboost::tie(ic, ie) = vertices(g), --ie; ic != ie; ++ic)

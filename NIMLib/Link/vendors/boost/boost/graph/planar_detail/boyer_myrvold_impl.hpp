...
namespace nimboost
{
  ...
  class boyer_myrvold_impl
  {
    ...
    typedef nimboost::shared_ptr< face_handle_list_t > face_handle_list_ptr_t;
    typedef nimboost::shared_ptr< vertex_list_t > vertex_list_ptr_t;
    typedef nimboost::tuple<vertex_t, bool, bool> merge_stack_frame_t;
  ...
  private:
    ...
    void walkup(vertex_t v)
    {
      ...
      for(nimboost::tie(oi,oi_end) = out_edges(v,g); oi != oi_end; ++oi)
        {
          edge_t e(*oi);
                      ...
                      if (first_tail != v)
                        {
                          ...
                          nimboost::tie(first_side_vertex, first_tail) 
                      ...
                      else if (second_tail != v)
                        {
                          ...
                          nimboost::tie(second_side_vertex, second_tail) 
    ...
    void clean_up_embedding(graph::detail::store_embedding)
    {
      ...
      for(nimboost::tie(xi,xi_end) = vertices(g); xi != xi_end; ++xi)
        {
  ...
  public:
    ...
    void extract_kuratowski_subgraph(OutputIterator o_itr, EdgeIndexMap em)
    {
      ...
      for(nimboost::tie(ei, ei_end) = edges(g); ei != ei_end; ++ei)
        {
          edge_t e(*ei);
          goal_edge[e] 
      ...
      if (bicomp_root != v)
        {
          ...
          for(nimboost::tie(vi,vi_end) = vertices(g); vi != vi_end; ++vi)
            if (lower_face_vertex[*vi])
              for(nimboost::tie(oei,oei_end) = out_edges(*vi,g); oei != oei_end; ++oei)
      ...
      else 
        {
              ...
              for(; old_face_itr != old_face_end; ++old_face_itr)
                {
                  ...
                  if (w == graph_traits<Graph>::null_vertex() && 
                      ...
                      outer_face_edge[e] &&
                      outer_face_edge[*nimboost::next(old_face_itr)] &&
                      !seen_x_or_y
                      )
                    {
                      w = current_vertex;
          ...
          for(nimboost::tie(v_edge_itr,v_edge_end) = out_edges(v,g); 
          ...
          if (chosen_case == detail::BM_CASE_E)
            {
              ...
              for(nimboost::tie(oei, oei_end) = out_edges(w,g); oei != oei_end; ++oei)
                {
                  if (!outer_face_edge[*oei])
                    goal_edge[*oei] = true;
                }
      ...
      while(child != parent)
        {
          ...
          nimboost::tie(parent, child) = std::make_pair( dfs_parent[parent], parent );
        }

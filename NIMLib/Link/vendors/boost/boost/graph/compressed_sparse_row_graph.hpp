...
namespace detail {
  template <typename T>
  struct default_construct_iterator: public nimboost::iterator_facade<default_construct_iterator<T>, T, nimboost::random_access_traversal_tag, const T&> {
    typedef nimboost::iterator_facade<default_construct_iterator<T>, T, std::random_access_iterator_tag, const T&> base_type;
    T saved_value;
                                              ...
                                              VertexProperty, Vertex, typed_identity_property_map<Vertex> >
{
 ...
 public:
  ...
  compressed_sparse_row_graph(construct_inplace_from_sources_and_targets_t,
    ...
    : inherited_vertex_properties(numverts), m_property(prop)
  {
    m_forward.assign_sources_and_targets_global(sources, targets, numverts, nimboost::typed_identity_property_map<vertices_size_type>());
  }
  ...
  compressed_sparse_row_graph(construct_inplace_from_sources_and_targets_t,
    ...
    : inherited_vertex_properties(numverts), m_property(prop)
  {
    m_forward.assign_sources_and_targets_global(sources, targets, edge_props, numverts, nimboost::typed_identity_property_map<vertices_size_type>());
  }
    ...
    : inherited_vertex_properties(numverts), m_property(prop)
  {
    ...
    nimboost::graph::detail::split_into_separate_coords
  ...
  {
    std::vector<vertex_descriptor> sources, targets;
    nimboost::graph::detail::split_into_separate_coords_filtered
  ...
  add_edges_internal(InputIterator first, InputIterator last,
    ...
    typedef typename nimboost::graph_traits<Graph>::vertex_descriptor vertex_t;
  ...
  add_edges_internal(InputIterator first, InputIterator last,
    ...
    typedef std::vector<
              nimboost::tuple<vertex_pair,
                           edge_bundled> >
      edge_vector_t;
    edge_vector_t new_edges
      (nimboost::make_zip_iterator(nimboost::make_tuple(first, ep_iter)),
       nimboost::make_zip_iterator(nimboost::make_tuple(last, ep_iter_end)));
    if (new_edges.empty()) return;
    std::sort(new_edges.begin(), new_edges.end(),
              nimboost::detail::compare_first<
                std::less<vertex_pair> >());
    m_forward.add_edges_sorted_internal
      (nimboost::make_transform_iterator(
         new_edges.begin(),
         nimboost::detail::my_tuple_get_class<0, vertex_pair>()),
       nimboost::make_transform_iterator(
         new_edges.end(),
         nimboost::detail::my_tuple_get_class<0, vertex_pair>()),
       nimboost::make_transform_iterator(
         new_edges.begin(),
         nimboost::detail::my_tuple_get_class
           <1, edge_bundled>()),
       global_to_local);
  }
                                              ...
                                              VertexProperty, Vertex, typed_identity_property_map<Vertex> >
{
 ...
 public:
  ...
  typedef detail::compressed_sparse_row_structure<EdgeProperty, Vertex, EdgeIndex> forward_type;
  typedef EdgeIndex /* typename nimboost::mpl::if_c<nimboost::is_same<EdgeProperty, nimboost::no_property>, nimboost::no_property, EdgeIndex> */ backward_edge_property;
  ...
  void set_up_backward_property_links() {
    ...
    m_backward.assign_unsorted_multi_pass_edges
      (detail::transpose_edges(
       ...
       nimboost::counting_iterator<EdgeIndex>(0),
...
{
  typedef nimboost::ref_property_map<BOOST_CSR_GRAPH_TYPE*, typename BOOST_CSR_GRAPH_TYPE::graph_property_type> type;
  typedef nimboost::ref_property_map<BOOST_CSR_GRAPH_TYPE*, const typename BOOST_CSR_GRAPH_TYPE::graph_property_type> const_type;
};

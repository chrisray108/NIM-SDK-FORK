...
struct update_position_visitor {
  ...
  typedef nimboost::on_examine_vertex event_filter;
...
struct gursoy_shortest
{
  ...
  {
    nimboost::dijkstra_shortest_paths(g, s, weight_map(weight).
      visitor(nimboost::make_dijkstra_visitor(std::make_pair(
       nimboost::record_distances(node_distance, nimboost::on_edge_relaxed()),
        update_position))));
  }
};
...
struct gursoy_shortest<dummy_property_map>
{
  ...
  run(const Graph& g, typename graph_traits<Graph>::vertex_descriptor s,
      ...
      dummy_property_map)
  {
    nimboost::breadth_first_search(g, s,
      visitor(nimboost::make_bfs_visitor(std::make_pair(
        nimboost::record_distances(node_distance, nimboost::on_tree_edge()),
        update_position))));
  }
};
   ...
   EdgeWeightMap weight)
{
  ...
  typedef nimboost::iterator_property_map<std::vector<double>::iterator, 
                                       VertexIndexMap,
                                       double, double&>
  ...
  for (nimboost::tie(i, iend) = vertices(graph); i != iend; ++i) {
  ...
  nimboost::detail::update_position_visitor<
                        ...
                        EdgeWeightMap weight) 
{
  ...
  typedef nimboost::iterator_property_map<std::vector<int>::iterator, 

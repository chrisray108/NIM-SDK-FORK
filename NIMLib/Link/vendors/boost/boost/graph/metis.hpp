...
void metis_reader::edge_iterator::advance(bool skip_initial_read)
{
  do {
      ...
      if (self->line_in >> std::ws >> self->edge.second) {
        ...
        if (self->has_edge_weights()) {
          if (!(self->line_in >> self->edge_weight))
            nimboost::throw_exception(metis_input_exception());
        }
        return;
      }
    ...
    if (!self->in) nimboost::throw_exception(metis_input_exception());
    ...
    while (weights_left > 0) {
      ...
      else nimboost::throw_exception(metis_input_exception());
      --weights_left;
    }
...
void metis_reader::start()
{
  ...
  if (!in || line.empty()) nimboost::throw_exception(metis_input_exception());

  // Determine number of vertices and edges in the graph
  line_in.str(line);
  if (!(line_in >> n_vertices >> n_edges)) nimboost::throw_exception(metis_input_exception());

...
#include <boost/tuple/tuple.hpp> // for nimboost::tie
...
struct get_all_adjacent_vertices {
    ...
    operator()(BOOST_FWD_REF(Vertex) v, BOOST_FWD_REF(Graph) g) const {
        return adjacent_vertices(nimboost::forward<Vertex>(v),
                                 nimboost::forward<Graph>(g));
    }
};
...
bool contains(Container const& c, Value const& v) {
    return std::find(nimboost::begin(c), nimboost::end(c), v) != nimboost::end(c);
}
...
private:
    ...
    bool circuit(Vertex start, Vertex v) {
        ...
        AdjacencyIterator const w_end = nimboost::end(adj_vertices);

        for (AdjacencyIterator w_it = nimboost::begin(adj_vertices);
             w_it != w_end;
             ++w_it)
        {
            Vertex const w = *w_it;
...
void call_hawick_circuits(Graph const& graph,
    ...
    for (nimboost::tie(start, last) = vertices(graph); start != last; ++start) {
...
void call_hawick_circuits(Graph const& graph, BOOST_FWD_REF(Visitor) visitor) {
    call_hawick_circuits<GetAdjacentVertices>(
        graph, nimboost::forward<Visitor>(visitor), get(vertex_index, graph)
    );
}
...
void hawick_circuits(BOOST_FWD_REF(Graph) graph,
    ...
    >(
        nimboost::forward<Graph>(graph),
        nimboost::forward<Visitor>(visitor),
        nimboost::forward<VertexIndexMap>(vertex_index_map)
    );
}
...
void hawick_circuits(BOOST_FWD_REF(Graph) graph,
    ...
    hawick_circuits_detail::call_hawick_circuits<
        hawick_circuits_detail::get_all_adjacent_vertices
    >(nimboost::forward<Graph>(graph), nimboost::forward<Visitor>(visitor));
}

/*!
 * Same as `nimboost::hawick_circuits`, but duplicate circuits caused by parallel

...
namespace nimboost {
  ...
  namespace detail {
    ...
    public:
      ...
      nimboost::tuple<size_type, size_type, size_type>
      term_set() const {
        return nimboost::make_tuple(term_in_count_, term_out_count_, 
                                 term_both_count_);
      }
    ...
    struct equivalent_edge_exists {
      typedef typename nimboost::graph_traits<Graph>::edge_descriptor edge_type;
    ...
    struct equivalent_edge_exists<Graph, typename nimboost::disable_if<is_multigraph<Graph> >::type> {
      template<typename EdgePredicate>
      bool operator()(typename graph_traits<Graph>::vertex_descriptor s,
                      typename graph_traits<Graph>::vertex_descriptor t, 
                      EdgePredicate is_valid_edge, const Graph& g) {
        ...
        bool found;
        nimboost::tie(e, found) = edge(s, t, g);
        if (!found)
          return false;
    ...
    class state {
      ...
      inline bool comp_term_sets(graph1_size_type a, 
                                 graph2_size_type b,
                                 nimboost::mpl::int_<subgraph_mono>) const {
        return a <= b;
      }
      ...
      inline bool comp_term_sets(graph1_size_type a, 
                                 graph2_size_type b,
                                 nimboost::mpl::int_<subgraph_iso>) const {
        return a <= b;
      }
      ...
      inline bool comp_term_sets(graph1_size_type a, 
                                 graph2_size_type b,
                                 nimboost::mpl::int_<isomorphism>) const {
        return a == b;
      }
    ...
    public:
      ...
      bool feasible(const vertex1_type& v_new, const vertex2_type& w_new) {
        ...
        if (problem_selection != subgraph_mono) { // subgraph_iso and isomorphism
          return comp_term_sets(term_in1_count, term_in2_count,
                                nimboost::mpl::int_<problem_selection>()) &&
      ...
      bool valid() const {
        nimboost::tuple<graph1_size_type, graph1_size_type, graph1_size_type> term1;
        nimboost::tuple<graph2_size_type, graph2_size_type, graph2_size_type> term2;
        
        term1 = state1_.term_set();
        term2 = state2_.term_set();
        
        return comp_term_sets(nimboost::get<0>(term1), nimboost::get<0>(term2),
                              nimboost::mpl::int_<problem_selection>()) &&
               comp_term_sets(nimboost::get<1>(term1), nimboost::get<1>(term2),
                              nimboost::mpl::int_<problem_selection>()) &&
               comp_term_sets(nimboost::get<2>(term1), nimboost::get<2>(term2),
                              nimboost::mpl::int_<problem_selection>()); 
      }
    ...
    bool match(const Graph1& graph1, const Graph2& graph2, 
      ...
      nimboost::tie(graph2_verts_iter, graph2_verts_iter_end) = vertices(graph2);
    ...
    void sort_vertices(const Graph& graph, IndexMap index_map, VertexOrder& order) {
      ...
      nimboost::range::sort(order, vertex_in_out_degree_cmp<Graph>(graph));
      ...
      nimboost::range::sort(order, vertex_frequency_degree_cmp<Graph, frequency_map_type>(graph, freq));

...
namespace nimboost {
  namespace graph { namespace distributed { namespace detail {
    ...
    class parallel_dfs
    {
       ...
       */
      typedef nimboost::parallel::detail::untracked_pair<vertex_descriptor, vertex_descriptor> vertex_pair;
          ...
          case discover_msg:
            {
              ...
              if (get(color, p.first) == Color::white()) {
                ...
                for (nimboost::tie(ei,ei_end) = out_edges(p.first, g); ei != ei_end; ++ei)
                {

...
class mpi_process_group
{
 ...
 public:
  ...
  typedef nimboost::mpi::packed_oprimitive::buffer_type buffer_type;
  ...
  typedef nimboost::mpi::communicator communicator_type;
...
private:
   ...
   */
  class trigger_base : nimboost::noncopyable
  {
  public:
...
public:
  ...
  std::pair<nimboost::mpi::communicator, int> 
...
private:
  ...
  void receive_batch(nimboost::mpi::status& status) const;
...
send_oob(const mpi_process_group& pg, mpi_process_group::process_id_type dest,
         int tag, const T& value, int block=-1)
{
  using nimboost::mpi::packed_oarchive;
...
BOOST_IS_BITWISE_SERIALIZABLE(nimboost::graph::distributed::mpi_process_group::message_header)
namespace nimboost { namespace mpi {
    template<>
    struct is_mpi_datatype<nimboost::graph::distributed::mpi_process_group::message_header> : mpl::true_ { };
} } // end namespace nimboost::mpi

namespace std {
/// optimized swap for outgoing messages
inline void 
swap(nimboost::graph::distributed::mpi_process_group::outgoing_messages& x,
     nimboost::graph::distributed::mpi_process_group::outgoing_messages& y)
{
  x.swap(y);
}
...
BOOST_CLASS_IMPLEMENTATION(nimboost::graph::distributed::mpi_process_group::outgoing_messages,object_serializable)
BOOST_CLASS_TRACKING(nimboost::graph::distributed::mpi_process_group::outgoing_messages,track_never)

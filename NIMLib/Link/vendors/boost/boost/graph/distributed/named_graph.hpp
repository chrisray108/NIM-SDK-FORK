...
using nimboost::parallel::trigger_receive_context;
using nimboost::detail::parallel::pair_with_property;
...
public:
   ...
   *  function uses the @c nimboost::hash value of the name, modulo @c p.
   */
                           ...
                           ProcessGroup, void>
{
  ...
  typedef typename mpl::if_<is_same<InDistribution, defaultS>,
                            nimboost::parallel::variant_distribution<ProcessGroup, 
                                                                  VertexSize>,
  ...
  typedef typename mpl::if_<is_same<InDistribution, defaultS>,
                            nimboost::parallel::block, type>::type
    default_type;
};
...
protected:
  ...
  nimboost::parallel::detail::untracked_pair<vertex_descriptor, bool>
  handle_find_vertex(int source, int tag, const vertex_name_type& msg,
                     trigger_receive_context);

  template<typename U, typename V>
  void handle_add_edge(int source, int tag, const nimboost::parallel::detail::untracked_pair<U, V>& msg,
                       trigger_receive_context);

  template<typename U, typename V>
  nimboost::parallel::detail::untracked_pair<edge_descriptor, bool> 
  handle_add_edge_with_reply(int source, int tag, const nimboost::parallel::detail::untracked_pair<U, V>& msg,
                             trigger_receive_context);
...
{
  using nimboost::parallel::detail::make_untracked_pair;
  ...
  if (vertex_name_type* v_name = nimboost::get<vertex_name_type>(&v)) {
    ...
    if (vertex_name_type* u_name = nimboost::get<vertex_name_type>(&u))
      ...
      send(self.process_group(), self.distribution_(*v_name),
           ...
           make_untracked_pair(nimboost::get<vertex_descriptor>(u), *v_name));
  } else {
    if (vertex_name_type* u_name = nimboost::get<vertex_name_type>(&u))
      ...
      send(self.process_group(), self.distribution_(*u_name),
           ...
           make_untracked_pair(*u_name, nimboost::get<vertex_descriptor>(v)));
    else
      ...
      add_edge(nimboost::get<vertex_descriptor>(u), 
               nimboost::get<vertex_descriptor>(v), 
               self.derived());
  }
}
...
BGL_NAMED_GRAPH::lazy_add_edge::commit() const
{
  ...
  if (const vertex_name_type* u_name = nimboost::get<vertex_name_type>(&u)) {
    /// We haven't resolved the source vertex to a descriptor yet, so
    ...
    if (const vertex_name_type* v_name = nimboost::get<vertex_name_type>(&v))
    ...
    u_owner 
      = get(vertex_owner, self.derived(),
            nimboost::get<vertex_descriptor>(u));
    if (u_owner == rank) {
      ...
      return add_edge(nimboost::get<vertex_descriptor>(u), 
                      nimboost::get<vertex_descriptor>(v), 
                      self.derived());
    } else {
...
{
  using nimboost::detail::parallel::make_pair_with_property;
  ...
  if (vertex_name_type* v_name = nimboost::get<vertex_name_type>(&v)) {
    ...
    if (vertex_name_type* u_name = nimboost::get<vertex_name_type>(&u))
      ...
      send(self.process_group(), self.distribution_(*v_name),
           ...
           make_pair_with_property(nimboost::get<vertex_descriptor>(u), *v_name, 
                                   property));
  } else {
    if (vertex_name_type* u_name = nimboost::get<vertex_name_type>(&u))
      // We haven't resolved the source vertex to a descriptor yet, so
      ...
      send(self.process_group(), self.distribution_(*u_name),
           ...
           make_pair_with_property(*u_name, nimboost::get<vertex_descriptor>(v), 
                                   property));
    else
      // We have descriptors for both of the vertices, either of which
      // may be remote or local. Tell the owner of the source vertex
            ...
            const BGL_NAMED_GRAPH& g)
{
  ...
  if (owner == process_id(g.process_group())) {
    ...
    nimboost::parallel::detail::untracked_pair<vertex_descriptor, bool> result;
...
template<BGL_NAMED_GRAPH_PARAMS>
nimboost::parallel::detail::untracked_pair<typename BGL_NAMED_GRAPH::vertex_descriptor, bool>
BGL_NAMED_GRAPH::
handle_find_vertex(int source, int /*tag*/, const vertex_name_type& msg, 
                   trigger_receive_context)
{
...
void
BGL_NAMED_GRAPH::
handle_add_edge(int source, int /*tag*/, const nimboost::parallel::detail::untracked_pair<U, V>& msg, 
                trigger_receive_context)
{
...
template<BGL_NAMED_GRAPH_PARAMS>
template<typename U, typename V>
nimboost::parallel::detail::untracked_pair<typename BGL_NAMED_GRAPH::edge_descriptor, bool>
BGL_NAMED_GRAPH::
handle_add_edge_with_reply(int source, int /*tag*/, const nimboost::parallel::detail::untracked_pair<U, V>& msg,
                           trigger_receive_context)
{

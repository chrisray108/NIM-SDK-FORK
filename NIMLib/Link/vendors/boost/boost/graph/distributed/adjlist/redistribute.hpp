...
private:
  ...
  {
    ar & nimboost::serialization::base_object<inherited>(*this) 
...
{
  using nimboost::parallel::inplace_all_to_all;
  ...
  for (nimboost::tie(vi, vi_end) = vertices(*this); vi != vi_end; ++vi) {
  ...
  // an all-to-all transfer.
  for (nimboost::tie(ei, ei_end) = edges(*this); ei != ei_end; ++ei) {
  ...
  if (has_stable_descriptors) {
    ...
    for (nimboost::tie(vi, vi_end) = vertices(*this); vi != vi_end; /* in loop */) {

...
namespace nimboost
{
  ...
  struct triangulation_visitor : public planar_face_traversal_visitor
  {
    ...
    void end_face()
    {
      ...
      for(nimboost::tie(ai,ai_end) = adjacent_vertices(vertices_on_face.front(),g); 
          ai != ai_end; ++ai
          )
        {
      ...
      for(face_iterator fi = nimboost::next(nimboost::next(vertices_on_face.begin())); 
          fi != fi_end; ++fi
          )
        {
      ...
      if (marked_neighbor == vertices_on_face.end())
        {
          add_edge_range(
                         vertices_on_face[0],
                         nimboost::next(nimboost::next(vertices_on_face.begin())),

...
namespace nimboost {
    ...
    namespace detail {
        ...
        class constant_value_property_map
          : public nimboost::put_get_helper<ValueType,
        ...
        void compute_in_degree_map(Graph& g, CoreMap d, EdgeWeightMap wm)
        {
            ...
            for (nimboost::tie(vi,vi_end) = vertices(g); vi!=vi_end; ++vi) {
                for (nimboost::tie(ei,ei_end) = out_edges(*vi,g); ei!=ei_end; ++ei) {
        ...
        core_numbers_impl(Graph& g, CoreMap c, PositionMap pos, Visitor vis)
        {
            ...
            std::copy(nimboost::make_reverse_iterator(bin.end()-2),
                nimboost::make_reverse_iterator(bin.begin()),
                nimboost::make_reverse_iterator(bin.end()-1));

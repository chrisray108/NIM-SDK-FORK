...
namespace nimboost
{
  namespace detail {
      ...
      const Zero& zero)
    {
      ...
      for (nimboost::tie(k, lastk) = vertices(g); k != lastk; k++)
        for (nimboost::tie(i, lasti) = vertices(g); i != lasti; i++)
          if(d[*i][*k] != inf)
            for (nimboost::tie(j, lastj) = vertices(g); j != lastj; j++)
              if(d[*k][*j] != inf)
                d[*i][*j] = 
    ...
    const Infinity& inf, const Zero& zero)
  {
    ...
    for(nimboost::tie(firstv, lastv) = vertices(g); firstv != lastv; firstv++)
      for(nimboost::tie(firstv2, lastv2) = vertices(g); firstv2 != lastv2; firstv2++)
    ...
    for(nimboost::tie(first, last) = edges(g); first != last; first++)
    {

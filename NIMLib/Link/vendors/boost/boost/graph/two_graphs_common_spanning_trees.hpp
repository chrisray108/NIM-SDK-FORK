  ...
  )
{
  ...
  nimboost::tuples::tie(current, last) = edges(iG);
  for(; current != last; ++current)
    if(iG_bimap.right.find(*current) == iG_bimap.right.end())
      return;
  nimboost::tuples::tie(current, last) = edges(vG);
  for(; current != last; ++current)
  ...
  )
{
  ...
  nimboost::tuples::tie(curr, last) = edges(iG);
  for(; curr != last; ++curr)
    iGO.push_back(*curr);

  nimboost::tuples::tie(curr, last) = edges(vG);
  for(; curr != last; ++curr)
    vGO.push_back(*curr);

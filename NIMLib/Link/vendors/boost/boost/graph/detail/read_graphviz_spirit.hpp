...
typedef nimboost::function2<void, id_t const&, id_t const&> actor_t;
...
/////////////////////////////////////////////////////////////////////////////
struct id_closure : nimboost::spirit::classic::closure<id_closure, node_t> {
  member1 name;
};


struct node_id_closure : nimboost::spirit::classic::closure<node_id_closure, node_t> {
  member1 name;
};

struct attr_list_closure : nimboost::spirit::classic::closure<attr_list_closure, actor_t> {
  member1 prop_actor;
};

struct property_closure : nimboost::spirit::classic::closure<property_closure, id_t, id_t> {
  member1 key;
  member2 value;
};

struct data_stmt_closure : nimboost::spirit::classic::closure<data_stmt_closure,
...
struct subgraph_closure : nimboost::spirit::classic::closure<subgraph_closure,
...
struct dot_grammar : public nimboost::spirit::classic::grammar<dot_grammar> { 
  mutate_graph& graph_;
  ...
  struct definition {
    ...
    keyword_p("0-9a-zA-Z_") {
      ...
      using namespace nimboostphoenix;
      ...
      a_list
          = list_p( ID[(a_list.key = arg1),
                    ...
                    >> !( ch_p('=')
                          ...
                          [nimboostphoenix::bind(&definition::call_prop_actor)
      ...
      node_id
          = ( ID[node_id.name = arg1] >> (!port) )
             [nimboostphoenix::bind(&definition::memoize_node)(var(*this))];

      graph_stmt
          = (ID[graph_stmt.key = arg1] >>
             ch_p('=') >>
             ID[graph_stmt.value = arg1])
        [nimboostphoenix::bind(&definition::call_graph_prop)
         (var(*this),graph_stmt.key,graph_stmt.value)]
        ; // Graph property.

      attr_stmt
          = (as_lower_d[keyword_p("graph")]
             >> attr_list(actor_t(nimboostphoenix::bind(&definition::default_graph_prop)
                                     (var(*this),arg1,arg2))))
          | (as_lower_d[keyword_p("node")]
             >> attr_list(actor_t(nimboostphoenix::bind(&definition::default_node_prop)
                                     (var(*this),arg1,arg2))))
          | (as_lower_d[keyword_p("edge")]
             >> attr_list(actor_t(nimboostphoenix::bind(&definition::default_edge_prop)
                                     (var(*this),arg1,arg2))))
          ;

      // edge_head is set depending on the graph type (directed/undirected)
      edgeop = ch_p('-') >> ch_p(nimboost::ref(edge_head));

      edgeRHS
          =  +(    edgeop[(data_stmt.sources = data_stmt.dests),
                          (data_stmt.dests = construct_<nodes_t>())]
                   >> ( subgraph[data_stmt.dests = arg1]
                      | node_id[nimboostphoenix::bind(&definition::insert_node)
                                (var(*this),data_stmt.dests,arg1)]
                      )
                   [nimboostphoenix::bind(&definition::activate_edge)
      ...
      data_stmt
          = ( subgraph[(data_stmt.dests = arg1),// will get moved in rhs
            ...
            | node_id[(nimboostphoenix::bind(&definition::insert_node)
                     ...
                     !attr_list(
                       actor_t(nimboostphoenix::bind(&definition::edge_prop)
                               (var(*this),arg1,arg2)))
                  ].else_p[
                     if_p(data_stmt.saw_node)[
                         !attr_list(
                           actor_t(nimboostphoenix::bind(&definition::node_prop)
                                   (var(*this),arg1,arg2)))
      ...
      the_grammar
          = (!as_lower_d[keyword_p("strict")])
            >>  ( as_lower_d[keyword_p("graph")][
                   (var(edge_head) = '-'),
                   (nimboostphoenix::bind(&definition::check_undirected)(var(*this)))]
                | as_lower_d[keyword_p("digraph")][
                   (var(edge_head) = '>'),
                   (nimboostphoenix::bind(&definition::check_directed)(var(*this)))]
                )
            >> (!ID) >> ch_p('{') >> stmt_list >> ch_p('}');

    } // definition()

    typedef nimboost::spirit::classic::rule<ScannerT> rule_t;
    ...
    void check_undirected() {
      if(self.graph_.is_directed())
          nimboost::throw_exception(nimboost::undirected_graph_error());
    }

    void check_directed() {
      if(!self.graph_.is_directed())
          nimboost::throw_exception(nimboost::directed_graph_error());
    }
   ...
   void default_edge_prop(id_t const& key, id_t const& value) {
    ...
    // Keywords;
    const nimboost::spirit::classic::distinct_parser<> keyword_p;
    //
    // rules that make up the grammar
    //
    nimboost::spirit::classic::rule<ScannerT,id_closure::context_t> ID;
    nimboost::spirit::classic::rule<ScannerT,property_closure::context_t> a_list;
    nimboost::spirit::classic::rule<ScannerT,attr_list_closure::context_t> attr_list;
    rule_t port_location;
    rule_t port_angle;
    rule_t port;
    nimboost::spirit::classic::rule<ScannerT,node_id_closure::context_t> node_id;
    nimboost::spirit::classic::rule<ScannerT,property_closure::context_t> graph_stmt;
    rule_t attr_stmt;
    nimboost::spirit::classic::rule<ScannerT,data_stmt_closure::context_t> data_stmt;
    nimboost::spirit::classic::rule<ScannerT,subgraph_closure::context_t> subgraph;
    rule_t edgeop;
    rule_t edgeRHS;
    rule_t stmt;
    rule_t stmt_list;
...
//
struct dot_skipper : public nimboost::spirit::classic::grammar<dot_skipper>
{
    dot_skipper() {}
    ...
    struct definition
    {
      ...
      nimboost::spirit::classic::rule<ScannerT>  skip;
      nimboost::spirit::classic::rule<ScannerT> const&
...
bool read_graphviz_spirit(MultiPassIterator begin, MultiPassIterator end,
  ...
  using namespace nimboost;
  using namespace nimboost::spirit::classic;

  typedef MultiPassIterator iterator_t;
  typedef skip_parser_iteration_policy< nimboost::detail::graph::dot_skipper>
    iter_policy_t;
  typedef scanner_policies<iter_policy_t> scanner_policies_t;
  typedef scanner<iterator_t, scanner_policies_t> scanner_t;

  ::nimboost::detail::graph::mutate_graph_impl<MutableGraph> 
      m_graph(graph, dp, node_id);

  ::nimboost::detail::graph::dot_grammar p(m_graph);
  ::nimboost::detail::graph::dot_skipper skip_p;

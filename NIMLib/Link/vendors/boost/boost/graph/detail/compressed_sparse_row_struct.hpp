...
namespace detail {
  ...
  class compressed_sparse_row_structure :
    ...
    assign_unsorted_multi_pass_edges(MultiPassInputIterator edge_begin,
      ...
      typedef nimboost::transform_iterator<nimboost::graph::detail::project1st<edge_type>, MultiPassInputIterator> source_iterator;
      typedef nimboost::transform_iterator<nimboost::graph::detail::project2nd<edge_type>, MultiPassInputIterator> target_iterator;
      source_iterator sources_begin(edge_begin, nimboost::graph::detail::project1st<edge_type>());
      source_iterator sources_end(edge_end, nimboost::graph::detail::project1st<edge_type>());
      target_iterator targets_begin(edge_begin, nimboost::graph::detail::project2nd<edge_type>());
      target_iterator targets_end(edge_end, nimboost::graph::detail::project2nd<edge_type>());

      nimboost::graph::detail::count_starts
        (sources_begin, sources_end, m_rowstart.begin(), numlocalverts,
         source_pred, nimboost::make_property_map_function(global_to_local));

      m_column.resize(m_rowstart.back());
      inherited_edge_properties::resize(m_rowstart.back());

      nimboost::graph::detail::histogram_sort
        (sources_begin, sources_end, m_rowstart.begin(), numlocalverts,
    ...
    void assign_from_sorted_edges(
      ...
      if (numedges == 0) {
        numedges = nimboost::graph::detail::reserve_count_for_single_pass(edge_begin, edge_end);
      }
      m_column.clear();
      m_column.reserve(numedges_or_zero);
    ...
    void assign_sources_and_targets_global(std::vector<vertex_descriptor>& sources,
      ...
      nimboost::graph::detail::count_starts
        (sources.begin(), sources.end(), m_rowstart.begin(), numverts,
         keep_all(), nimboost::make_property_map_function(global_to_local));
      nimboost::graph::detail::histogram_sort_inplace
        (sources.begin(), m_rowstart.begin(), numverts,
         targets.begin(), nimboost::make_property_map_function(global_to_local));
    ...
    void assign_sources_and_targets_global(std::vector<vertex_descriptor>& sources,
      ...
      nimboost::graph::detail::histogram_sort_inplace
        (sources.begin(), m_rowstart.begin(), numverts,
         ...
         nimboost::make_property_map_function(global_to_local));
      // Now targets is the correct vector (properly sorted by source) for
           ...
           vertices_size_type numverts, edges_size_type numedges)
    {
      ...
      EdgeIndex current_edge = 0;
      typedef typename nimboost::graph_traits<Graph>::vertex_descriptor g_vertex;
      typedef typename nimboost::graph_traits<Graph>::out_edge_iterator
        g_out_edge_iter;
      ...
      for (Vertex i = 0; i != numverts; ++i) {
        ...
        for (nimboost::tie(ei, ei_end) = out_edges(v, g); ei != ei_end; ++ei) {
    ...
    add_edges_sorted_internal(
        ...
        const GlobalToLocal& global_to_local) {
      typedef nimboost::reverse_iterator<BidirectionalIteratorOrig> BidirectionalIterator;
      typedef nimboost::reverse_iterator<EPIterOrig> EPIter;
      // Flip sequence
  ...
  class csr_edge_iterator
    : public iterator_facade<csr_edge_iterator<CSRGraph>,
                             typename CSRGraph::edge_descriptor,
                             nimboost::forward_traversal_tag,
                             typename CSRGraph::edge_descriptor>
  {
   private:
    typedef typename CSRGraph::edge_descriptor edge_descriptor;
   ...
   public: // See above
    friend class nimboost::iterator_core_access;
  ...
  struct transpose_iterator_gen {
    ...
    typedef nimboost::transform_iterator<transpose, Iter> type;
  ...
  {
    typedef typename nimboost::graph_traits<GraphT>::vertices_size_type
      vertices_size_type;
    typedef typename nimboost::graph_traits<GraphT>::edge_descriptor edge_descriptor;
  ...
  edge_to_index_pair
    <GraphT,
     typename nimboost::property_map<GraphT,nimboost::vertex_index_t>::const_type>
  make_edge_to_index_pair(const GraphT& g)
  {
    typedef typename nimboost::property_map<GraphT,
                                         nimboost::vertex_index_t>::const_type
      VertexIndexMap;
    return edge_to_index_pair<GraphT, VertexIndexMap>(g,
                                                     get(nimboost::vertex_index,
                                                         g));
  }

  template<typename GraphT, typename VertexIndexMap, typename Iter>
  nimboost::transform_iterator<edge_to_index_pair<GraphT, VertexIndexMap>, Iter>
  make_edge_to_index_pair_iter(const GraphT& g, const VertexIndexMap& index,
                               Iter it) {
    return nimboost::transform_iterator<edge_to_index_pair<GraphT, VertexIndexMap>, Iter>(it, edge_to_index_pair<GraphT, VertexIndexMap>(g, index));
  }

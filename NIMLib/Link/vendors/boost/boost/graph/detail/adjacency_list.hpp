...
namespace nimboost {
  ...
  namespace detail {
    ...
    {
      nimboost::graph_detail::erase_if(el, detail::target_is<vertex_descriptor>(v));
    }
    // O(log(E/V))
    template <class EdgeList, class vertex_descriptor>
    void erase_from_incidence_list(EdgeList& el, vertex_descriptor v,
    ...
    template <class Vertex>
    class stored_edge
      : public nimboost::equality_comparable1< stored_edge<Vertex>,
        nimboost::less_than_comparable1< stored_edge<Vertex> > >
    {
    public:
      ...
      const detail::stored_ra_edge_iter<Vertex, EdgeVec, Property>& e)
  {
    ...
    namespace detail {
      ...
      remove_directed_edge_if_dispatch(incidence_iterator first,
                                       ...
                                       nimboost::allow_parallel_edge_tag)
      {
        // remove_if
      ...
      remove_directed_edge_if_dispatch(incidence_iterator first,
                                       ...
                                       EdgeList& el,
                                       Predicate pred,
                                       nimboost::disallow_parallel_edge_tag)
      {
                       ...
                       directed_graph_helper<Config>& g_)
    {
      ...
      nimboost::tie(first, last) = out_edges(u, g);
    ...
    remove_edge_if(Predicate pred, directed_graph_helper<Config>& g_)
    {
      ...
      for (nimboost::tie(vi, vi_end) = vertices(g); vi != vi_end; ++vi)
                 ...
                 directed_graph_helper<Config>& g_)
    {
      ...
      for (nimboost::tie(vi, viend) = vertices(g); vi != viend; ++vi)
             ...
             directed_graph_helper<Config>& g_)
    {
      ...
      bool inserted;
      nimboost::tie(i, inserted) = nimboost::graph_detail::push(g.out_edge_list(u),
                                            StoredEdge(v, p));
    ...
    namespace detail {
      ...
      remove_edge_and_property(Graph& g, EdgeList& el, Vertex v,
                               nimboost::allow_parallel_edge_tag cat)
      {
        typedef typename Graph::global_edgelist_selector EdgeListS;
        BOOST_STATIC_ASSERT((!is_same<EdgeListS, vecS>::value));
        ...
        for (; i != end; ++i) {
          if ((*i).get_target() == v) {
            ...
            bool skip = (nimboost::next(i) != end && i->get_iter() == nimboost::next(i)->get_iter());
    ...
    struct list_edge // short name due to VC++ truncation and linker problems
      : public nimboost::detail::edge_base<nimboost::undirected_tag, Vertex>
    {
      typedef EdgeProperty property_type;
      typedef nimboost::detail::edge_base<nimboost::undirected_tag, Vertex> Base;
    ...
    remove_edge_if(Predicate pred, undirected_graph_helper<Config>& g_)
    {
      ...
      nimboost::tie(ei, ei_end) = edges(g);
                 ...
                 undirected_graph_helper<Config>& g_)
    {
      ...
      while (true) {
        ...
        nimboost::tie(ei, ei_end) = out_edges(u, g);
        if (ei == ei_end) break;
        remove_edge(*ei, g);
      }
    }
             ...
             undirected_graph_helper<Config>& g_)
    {
      ...
      if (inserted) {
        nimboost::graph_detail::push(g.out_edge_list(v), StoredEdge(u, p_iter, &g.m_edges));
                       ...
                       bidirectional_graph_helper_with_property<Config>& g_)
    {
      ...
      for (nimboost::tie(out_i, out_end) = out_edges(u, g); out_i != out_end; ++out_i)
        if (pred(*out_i)) {
          detail::remove_directed_edge_dispatch
                      ...
                      bidirectional_graph_helper_with_property<Config>& g_)
    {
      ...
      for (nimboost::tie(in_i, in_end) = in_edges(v, g); in_i != in_end; ++in_i)
        if (pred(*in_i)) {
      ...
      nimboost::tie(first, last) = in_edges(v, g);
      typedef typename Config::edge_parallel_category Cat;
      detail::remove_directed_edge_if_dispatch
             ...
             bidirectional_graph_helper_with_property<Config>& g_)
    {
      ...
      nimboost::tie(i, inserted) = nimboost::graph_detail::push(g.out_edge_list(u),
                                        StoredEdge(v, p_iter, &g.m_edges));
      if (inserted) {
        nimboost::graph_detail::push(in_edge_list(g, v), StoredEdge(u, p_iter, &g.m_edges));
                          ...
                          const adj_list_helper<Config, Base>& g_)
    {
      ...
      typedef typename Config::inv_adjacency_iterator inv_adjacency_iterator;
      typename Config::in_edge_iterator first, last;
      nimboost::tie(first, last) = in_edges(u, g);
               ...
               const adj_list_helper<Config, Base>& g_)
    {
      ...
      nimboost::tie(first, last) = graph_detail::
    ...
    namespace detail {
      ...
      inline
      typename nimboost::property_map<typename Config::graph_type,
        Property>::type
      get_dispatch(adj_list_helper<Config,Base>&, Property p,
                   nimboost::edge_property_tag) {
        typedef typename Config::graph_type Graph;
        typedef typename nimboost::property_map<Graph, Property>::type PA;
        return PA(p);
      }
      template <class Config, class Base, class Property>
      inline
      ...
      get_dispatch(const adj_list_helper<Config,Base>&, Property p,
                   nimboost::edge_property_tag) {
        typedef typename Config::graph_type Graph;
        typedef typename nimboost::property_map<Graph, Property>::const_type PA;
        return PA(p);
      }
    ...
    template <class Config, class Base, class Property>
    inline
    typename nimboost::property_map<typename Config::graph_type, Property>::type
    ...
    inline
    typename nimboost::property_traits<
      typename nimboost::property_map<typename Config::graph_type,
        Property>::type
    >::reference
    ...
    {
      typedef typename Config::graph_type Graph;
      typedef typename nimboost::property_map<Graph, Property>::type Map;
    ...
    public:
      ...
      inline void copy_impl(const adj_list_impl& x_)
      {
        ...
        for (nimboost::tie(vi, vi_end) = vertices(x); vi != vi_end; ++vi) {
        ...
        for (nimboost::tie(ei, ei_end) = edges(x); ei != ei_end; ++ei) {
          edge_descriptor e;
          bool inserted;
          vertex_descriptor s = source(*ei,x), t = target(*ei,x);
          nimboost::tie(e, inserted) = add_edge(vertex_map[(stored_vertex*)s],
    ...
    add_vertex(adj_list_impl<Derived, Config, Base>& g_)
    {
      ...
      bool inserted;
      nimboost::tie(pos,inserted) = nimboost::graph_detail::push(g.m_vertices, v);
      v->m_position = pos;
      g.added_vertex(v);
      return v;
    }
                              ...
                              adj_list_impl<Derived, Config, Base>& g_)
    {
      typedef typename Config::stored_vertex stored_vertex;
      Derived& g = static_cast<Derived&>(g_);
      g.removing_vertex(u, nimboost::graph_detail::iterator_stability(g_.m_vertices));
    ...
    public:
      ...
      //    protected:
      inline nimboost::integer_range<vertex_descriptor> vertex_set() const {
        return nimboost::integer_range<vertex_descriptor>(0, m_vertices.size());
      }
      inline OutEdgeList& out_edge_list(vertex_descriptor v) {
      ...
      inline void copy_impl(const vec_adj_list_impl& x_)
      {
        ...
        for (nimboost::tie(ei, ei_end) = edges(x); ei != ei_end; ++ei) {
          edge_descriptor e;
          bool inserted;
          nimboost::tie(e, inserted) = add_edge(source(*ei,x), target(*ei,x) , *this);
          *((edge_property_type*)e.m_eproperty)
            = *((edge_property_type*)(*ei).m_eproperty);
        }
      }
                              ...
                              vec_adj_list_impl<Graph, Config, Base>& g_)
    {
      ...
      Graph& g = static_cast<Graph&>(g_);
      g.removing_vertex(v, nimboost::graph_detail::iterator_stability(g_.m_vertices));
  ...
  namespace detail {
    ...
    struct adj_list_gen
    {
      ...
      struct config
      {
        ...
        typedef nimboost::integer_range<vertex_descriptor> RandVertexList;
        ...
        typedef nimboost::detail::iterator_traits<OutEdgeIter> OutEdgeIterTraits;
        ...
        typedef nimboost::detail::iterator_traits<EdgeIter> EdgeIterTraits;
    ...
    struct adj_list_vertex_property_map
      : public nimboost::put_get_helper<
          Reference,
        ...
        >
    {
      ...
      typedef nimboost::lvalue_property_map_tag category;
    ...
    struct adj_list_vertex_all_properties_map
      : public nimboost::put_get_helper<PropRef,
        ...
        >
    {
      typedef ValueType value_type;
      typedef Reference reference;
      typedef typename nimboost::graph_traits<Graph>::vertex_descriptor key_type;
      typedef nimboost::lvalue_property_map_tag category;
    ...
    struct vec_adj_list_vertex_all_properties_map
      : public nimboost::put_get_helper<PropertyRef,
        ...
        >
    {
      ...
      typedef nimboost::readable_property_map_tag category;
  ...
  namespace detail {
    ...
    struct adj_list_choose_vertex_pa
      : nimboost::mpl::if_<
          nimboost::is_same<Tag, vertex_all_t>,
...
namespace nimboost {
  ...
  template <typename V>
  struct hash< nimboost::detail::stored_edge<V> >
  {
    std::size_t
    operator()(const nimboost::detail::stored_edge<V>& e) const
    {
  ...
  struct hash< nimboost::detail::stored_edge_property <V,P> >
  {
    std::size_t
    operator()(const nimboost::detail::stored_edge_property<V,P>& e) const
    {
      return hash<V>()(e.m_target);
    }
  };
  ...
  struct hash< nimboost::detail::stored_edge_iter<V,I, P> >
  {
    std::size_t
    operator()(const nimboost::detail::stored_edge_iter<V,I,P>& e) const
    {
      return hash<V>()(e.m_target);
    }
  };

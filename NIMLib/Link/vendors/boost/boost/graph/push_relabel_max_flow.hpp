    ...
    public:
          ...
          work_since_last_update(0)
      {
        ...
        for (nimboost::tie(u_iter, u_end) = vertices(g); u_iter != u_end; ++u_iter)
          for (nimboost::tie(ei, e_end) = out_edges(*u_iter, g); ei != e_end; ++ei) {
        ...
        for (nimboost::tie(a_iter, a_end) = out_edges(src, g); a_iter != a_end; ++a_iter)
        ...
        if (overflow_detected)
          ...
          for (nimboost::tie(a_iter, a_end) = out_edges(src, g); 
               a_iter != a_end; ++a_iter) {
      ...
      void global_distance_update()
      {
        ...
        while (! Q.empty()) {
          ...
          for (nimboost::tie(ai, a_end) = out_edges(u, g); ai != a_end; ++ai) {
      ...
      void discharge(vertex_descriptor u)
      {
        ...
        while (1) {
          ...
          for (nimboost::tie(ai, ai_end) = current[u]; ai != ai_end; ++ai) {
            edge_descriptor a = *ai;
      ...
      void convert_preflow_to_flow()
      {
        ...
        // handle self-loops
        for (nimboost::tie(u_iter, u_end) = vertices(g); u_iter != u_end; ++u_iter)
          for (nimboost::tie(ai, a_end) = out_edges(*u_iter, g); ai != a_end; ++ai)
        ...
        if (! bos_null) {
          for (u = tos; u != bos; u = topo_next[get(index, u)]) {
            nimboost::tie(ai, a_end) = out_edges(u, g);
      ...
      nimboost::queue<vertex_descriptor> Q;

...
struct tracking_level<nimboost::adjacency_list<OEL,VL,D,VP,EP,GP,EL> > {
...
inline void save(
    Archive & ar,
    const nimboost::adjacency_list<OEL,VL,D,VP,EP,GP,EL> &graph,
...
inline void load(
    Archive & ar,
    nimboost::adjacency_list<OEL,VL,D,VP,EP,GP,EL> &graph,
    const unsigned int /* file_version */
){
  typedef adjacency_list<OEL,VL,D,VP,EP,GP,EL> Graph;
  typedef typename graph_traits<Graph>::vertex_descriptor Vertex;
  ...
  while(E-- > 0){
    ...
    nimboost::tie(e,inserted) = add_edge(verts[u], verts[v], graph);
    ar >> serialization::make_nvp("edge_property", get(edge_all_t(), graph, e) );
  }
  ar >> serialization::make_nvp("graph_property", get_property(graph, graph_all_t()) );
}
...
inline void serialize(
    ...
    nimboost::serialization::split_free(ar, graph, file_version);
}

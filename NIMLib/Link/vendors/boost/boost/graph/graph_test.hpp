...
namespace nimboost {
  ...
  struct graph_test
  {
       ...
       const Graph& g)
    {
      ...
      for (; p.first != p.second; ++p.first) {
        edge_t e = *p.first;
        BOOST_CHECK(find_if(edge_set, connects(source(e, g), target(e, g), g)) != nimboost::end(edge_set));
        ...
        for (k = vertex_set.begin(); k != vertex_set.end(); ++k) {
          ...
          if (p.second == true)
            BOOST_CHECK(find_if(edge_set, 
              connects(source(p.first, g), target(p.first, g), g)) != nimboost::end(edge_set));
        }
    }
  ...
  for (typename nimboost::graph_traits<Graph>::vertex_iterator 
           bgl_first_9 = vertices(g).first, bgl_last_9 = vertices(g).second;
       bgl_first_9 != bgl_last_9; bgl_first_9 = bgl_last_9)
    for (typename nimboost::graph_traits<Graph>::vertex_descriptor v;

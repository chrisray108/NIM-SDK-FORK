...
class convex_topology 
{
 ...
 public:
  ...
  double distance(point a, point b) const 
  {
    ...
    for (std::size_t i = 0; i < Dims; ++i) {
      double diff = b[i] - a[i];
      dist = nimboost::math::hypot(dist, diff);
    }
  ...
  double norm(point_difference delta) const {
    double n = 0.;
    for (std::size_t i = 0; i < Dims; ++i)
      n = nimboost::math::hypot(n, delta[i]);
    return n;
  }
...
class heart_topology 
{
  ...
  bool in_heart(point p) const 
  {
    ...
    if (nimboost::math::hypot(p[0] - -500, p[1] - -500) <= 500. * nimboost::math::constants::root_two<double>())
 ...
 public:
  ...
  point random_point() const 
  {
    point result;
    do {
      result[0] = (*rand)() * (1000 + 1000 * nimboost::math::constants::root_two<double>()) - (500 + 500 * nimboost::math::constants::root_two<double>());
      result[1] = (*rand)() * (2000 + 500 * (nimboost::math::constants::root_two<double>() - 1)) - 2000;
  ...
  double distance(point a, point b) const 
  {
    if (segment_within_heart(a, b)) {
      // Straight line
      return nimboost::math::hypot(b[0] - a[0], b[1] - a[1]);
    } else {
      // Straight line bending around (0, 0)
      return nimboost::math::hypot(a[0], a[1]) + nimboost::math::hypot(b[0], b[1]);
    }
  }
  ...
  point move_position_toward(point a, double fraction, point b) const 
  {
    if (segment_within_heart(a, b)) {
      ...
      double distance_to_point_a = nimboost::math::hypot(a[0], a[1]);
      double distance_to_point_b = nimboost::math::hypot(b[0], b[1]);

...
namespace nimboost {
  ...
  struct MASVisitorConcept {
    void constraints() {
      nimboost::function_requires< nimboost::CopyConstructibleConcept<Visitor> >();
    ...
    Visitor vis;
    Graph g;
    typename nimboost::graph_traits<Graph>::vertex_descriptor u;
    typename nimboost::graph_traits<Graph>::edge_descriptor e;
  };
  ...
  public:
    ...
    {
      invoke_visitors(m_vis, u, g, ::nimboost::on_initialize_vertex());
    }
    ...
    {
      invoke_visitors(m_vis, u, g, ::nimboost::on_start_vertex());
    }
    ...
    {
      invoke_visitors(m_vis, e, g, ::nimboost::on_examine_edge());
    }
    ...
    {
      invoke_visitors(m_vis, u, g, ::nimboost::on_finish_vertex());
    }
  ...
  namespace detail {
    template <class Graph, class WeightMap, class MASVisitor, class VertexAssignmentMap, class KeyedUpdatablePriorityQueue>
      void
      maximum_adjacency_search(const Graph& g, WeightMap weights, MASVisitor vis, const typename nimboost::graph_traits<Graph>::vertex_descriptor start, VertexAssignmentMap assignments, KeyedUpdatablePriorityQueue pq) {
      typedef typename nimboost::graph_traits<Graph>::vertex_descriptor vertex_descriptor;
  ...
  template <class Graph, class WeightMap, class MASVisitor, class VertexAssignmentMap, class KeyedUpdatablePriorityQueue>
    void
maximum_adjacency_search(const Graph& g, WeightMap weights, MASVisitor vis, const typename nimboost::graph_traits<Graph>::vertex_descriptor start, VertexAssignmentMap assignments, KeyedUpdatablePriorityQueue pq) {
    BOOST_CONCEPT_ASSERT((nimboost::IncidenceGraphConcept<Graph>));
    BOOST_CONCEPT_ASSERT((nimboost::VertexListGraphConcept<Graph>));
    typedef typename nimboost::graph_traits<Graph>::vertex_descriptor vertex_descriptor;
    typedef typename nimboost::graph_traits<Graph>::vertices_size_type vertices_size_type;
    typedef typename nimboost::graph_traits<Graph>::edge_descriptor edge_descriptor;
    BOOST_CONCEPT_ASSERT((nimboost::Convertible<typename nimboost::graph_traits<Graph>::directed_category, nimboost::undirected_tag>));
    BOOST_CONCEPT_ASSERT((nimboost::ReadablePropertyMapConcept<WeightMap, edge_descriptor>));
    // typedef typename nimboost::property_traits<WeightMap>::value_type weight_type;
    nimboost::function_requires< MASVisitorConcept<MASVisitor, Graph> >();
    ...
    namespace detail {
      ...
      struct mas_dispatch {
        ...
        static result_type apply(const Graph& g, 
          ...
          typedef nimboost::detail::make_priority_queue_from_arg_pack_gen<nimboost::graph::keywords::tag::max_priority_queue, weight_type, vertex_descriptor, std::greater<weight_type> > default_pq_gen_type;

          default_pq_gen_type pq_gen(choose_param(get_param(params, nimboost::distance_zero_t()), weight_type(0)));

          typename nimboost::result_of<default_pq_gen_type(const Graph&, const ArgPack&)>::type pq = pq_gen(g, params);

          nimboost::maximum_adjacency_search
               (g,
                w,
                params [ _visitor | make_mas_visitor(null_visitor())],
                params [ _root_vertex | *vertices(g).first],
                params [ _vertex_assignment_map | nimboost::detail::make_property_map_from_arg_pack_gen<nimboost::graph::keywords::tag::vertex_assignment_map, vertex_descriptor>(vertex_descriptor())(g, params)],
                pq
                );
        }
      };
      ...
      template <>
      struct mas_dispatch<nimboost::param_not_found> {
        typedef void result_type;
        ...
        static result_type apply(const Graph& g, 
          ...
          typedef typename nimboost::property_map<Graph, edge_weight_t> WeightMap;
          typedef typename WeightMap::value_type weight_type;

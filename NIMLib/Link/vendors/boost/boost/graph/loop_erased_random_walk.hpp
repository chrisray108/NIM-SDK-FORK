...
namespace nimboost {
  ...
  void loop_erased_random_walk(
         const Graph& g,
         typename nimboost::graph_traits<Graph>::vertex_descriptor s,
         NextEdge next_edge,
         ColorMap color,
         std::vector<typename nimboost::graph_traits<Graph>::vertex_descriptor>& path
  ) {
    ...
    typedef nimboost::color_traits<color_t> color_gen;
  ...
  class unweighted_random_out_edge_gen {
    ...
    typedef nimboost::graph_traits<Graph> gt;
    ...
    operator()(typename gt::vertex_descriptor src, const Graph& g) const {
      ...
      return nimboost::random_out_edge(g, src, gen);
    }
  };
  ...
  class weighted_random_out_edge_gen {
    ...
    operator()(typename gt::vertex_descriptor src, const Graph& g) const {
      if (out_degree(src, g) == 0) throw loop_erased_random_walk_stuck();
      return nimboost::weighted_random_out_edge(g, src, weight, gen);
    }
  };
}

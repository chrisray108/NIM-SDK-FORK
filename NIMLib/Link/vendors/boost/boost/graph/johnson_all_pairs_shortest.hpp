...
namespace nimboost {
               ...
               DistanceZero zero)
  {
    ...
    typename Traits2::vertex_descriptor s = *vertices(g2).first;
    {
      ...
      int i = 1;
      for (nimboost::tie(v, v_end) = vertices(g1); v != v_end; ++v, ++i) {
        typename Traits2::edge_descriptor e; bool z;
        nimboost::tie(e, z) = add_edge(s, get(id1, *v) + 1, g2);
        put(w, e, zero);
        verts1[i] = *v;
      }
      typename Traits1::edge_iterator e, e_end;
      for (nimboost::tie(e, e_end) = edges(g1); e != e_end; ++e) {
        typename Traits2::edge_descriptor e2; bool z;
        nimboost::tie(e2, z) = add_edge(get(id1, source(*e, g1)) + 1, 
                                     get(id1, target(*e, g1)) + 1, g2);
        put(w, e2, get(w1, *e));
        if (is_undirected) {
          nimboost::tie(e2, z) = add_edge(get(id1, target(*e, g1)) + 1, 
    ...
    for (nimboost::tie(v, v_end) = vertices(g2); v != v_end; ++v)
      put(d, *v, inf);

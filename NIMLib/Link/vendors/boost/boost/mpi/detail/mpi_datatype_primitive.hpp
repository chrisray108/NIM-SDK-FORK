...
public:
    ...
    {
      if (x.count())
        save_impl(x.address(), nimboost::mpi::get_mpi_datatype(*x.address()), x.count());
    }
    ...
    MPI_Datatype get_mpi_datatype()
    {
      if (!is_committed)
      {
       ...
       BOOST_MPI_CHECK_RESULT(MPI_Type_create_struct,
                    (
                      addresses.size(),
                      nimboost::serialization::detail::get_data(lengths),
                      nimboost::serialization::detail::get_data(addresses),
                      nimboost::serialization::detail::get_data(types),
                      &datatype_
                    ));
#else
    ...
    void save(const T & t)
    {
        save_impl(&t, nimboost::mpi::get_mpi_datatype(t), 1);
    }

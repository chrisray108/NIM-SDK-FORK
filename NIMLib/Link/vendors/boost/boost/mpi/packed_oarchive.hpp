...
public:
  ...
  packed_oarchive( MPI_Comm const & comm, buffer_type & b, unsigned int flags = nimboost::archive::no_header)
         : oprimitive(b,comm),
  ...
  packed_oarchive ( MPI_Comm const & comm, unsigned int flags =  nimboost::archive::no_header)
  ...
  void save_override(archive::class_id_type & t, int version){
    const nimboost::int_least16_t x = t;
    * this->This() << x;
  }
...
// required by export
BOOST_SERIALIZATION_REGISTER_ARCHIVE(nimboost::mpi::packed_oarchive)
BOOST_SERIALIZATION_USE_ARRAY_OPTIMIZATION(nimboost::mpi::packed_oarchive)

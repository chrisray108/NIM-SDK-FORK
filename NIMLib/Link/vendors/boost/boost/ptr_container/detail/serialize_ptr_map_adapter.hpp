...
void save(Archive& ar, const ptr_container_detail::ptr_map_adapter_base<T, VoidPtrMap, CloneAllocator,Ordered>& c, unsigned int /*version*/)
{
    ...
    ar << nimboost::serialization::make_nvp( ptr_container_detail::count(), 
                                          ptr_container_detail::serialize_as_const(c.size()) );

    const_iterator i = c.begin(), e = c.end();
    for(; i != e; ++i)
    {
        ar << nimboost::serialization::make_nvp( ptr_container_detail::first(), i->first );
        ar << nimboost::serialization::make_nvp( ptr_container_detail::second(),
...
void load(Archive& ar, ptr_map_adapter<T, VoidPtrMap, CloneAllocator,Ordered>& c, unsigned int /*version*/)
{
    ...
    c.clear();
    size_type n;
    ar >> nimboost::serialization::make_nvp( ptr_container_detail::count(), n );

    for(size_type i = 0u; i != n; ++i)
    {
        key_type key;
        T* value;
        ar >> nimboost::serialization::make_nvp( ptr_container_detail::first(), key );
        ar >> nimboost::serialization::make_nvp( ptr_container_detail::second(), value );

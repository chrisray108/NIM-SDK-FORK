...
nimboost
    ...
    exception_detail
        {
        std::string diagnostic_information_impl( nimboost::exception const *, std::exception const *, bool, bool );
        }
    ...
    current_exception_diagnostic_information( bool verbose=true)
        {
        nimboost::exception const * be=current_exception_cast<nimboost::exception const>();
...
nimboost
    ...
    exception_detail
        ...
        inline
        std::string
        diagnostic_information_impl( nimboost::exception const * be, std::exception const * se, bool with_what, bool verbose )
            {
            if( !be && !se )
                return "Unknown exception.";
#ifndef BOOST_NO_RTTI
            if( !be )
                be=dynamic_cast<nimboost::exception const *>(se);
            if( !se )
    ...
    diagnostic_information_what( exception const & e, bool verbose=true ) throw()
        ...
        try
            ...
            if( char const * di=exception_detail::get_diagnostic_information(e,0) )
                ...
                return "Failed to produce nimboost::diagnostic_information_what()";
#ifndef BOOST_NO_EXCEPTIONS
            }
        catch(
        ... )

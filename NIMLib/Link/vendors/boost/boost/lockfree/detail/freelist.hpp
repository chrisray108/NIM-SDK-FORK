...
{
public:
    typedef nimboost::uint16_t tag_t;
    typedef nimboost::uint16_t index_t;
...
struct compiletime_sized_freelist_storage
{
    ...
    nimboost::array<char, size * sizeof(T)> data;
    ...
    Alloc
{
    ...
    {
        if (count > 65535)
            nimboost::throw_exception(std::runtime_error("boost.lockfree: freelist size is limited to a maximum of 65535 objects"));

...
typedef parameter::parameters<nimboost::parameter::optional<tag::capacity>,
...
protected:
    ...
    ConstIterator push(ConstIterator begin, ConstIterator end, T * internal_buffer, size_t max_size)
    {
        ...
        const ConstIterator last = nimboost::next(begin, input_count);

        if (write_index + input_count > max_size) {
            /* copy data in two sections */
            const size_t count0 = max_size - write_index;
            const ConstIterator midpoint = nimboost::next(begin, count0);
...
private:
    ...
    {
        if (nimboost::has_trivial_destructor<T>::value) {
    ...
    public ringbuffer_base<T>
{
    ...
    typedef typename nimboost::aligned_storage<max_size * sizeof(T),
 ...
 *
 *  - \c nimboost::lockfree::allocator<>, defaults to \c nimboost::lockfree::allocator<std::allocator<T>> <br>
...
public:
    ...
    /// \copydoc nimboost::lockfree::spsc_queue::consume_one(Functor & rhs)
    template <typename Functor>
    ...
    /// \copydoc nimboost::lockfree::spsc_queue::consume_all(Functor & rhs)
    template <typename Functor>

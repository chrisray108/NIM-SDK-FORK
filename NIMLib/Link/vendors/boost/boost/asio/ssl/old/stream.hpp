 ...
 * nimboost::asio::ssl::context context(io_service, nimboost::asio::ssl::context::sslv23);
 * nimboost::asio::ssl::stream<nimboost::asio::ip::tcp::socket> sock(io_service, context);
 * @endcode
 *
 * @par Concepts:
...
public:
  ...
  explicit stream(Arg& arg, basic_context<Context_Service>& context)
    : next_layer_(arg),
      service_(nimboost::asio::use_service<Service>(next_layer_.get_io_service())),
      impl_(service_.null())
  {
   ...
   * @returns The number of bytes of data that can be read without blocking.
   */
  std::size_t in_avail(nimboost::system::error_code& ec)
  {

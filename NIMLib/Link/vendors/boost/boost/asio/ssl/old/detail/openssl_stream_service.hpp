...
class openssl_stream_service
  : public nimboost::asio::detail::service_base<openssl_stream_service>
{
private:
  ...
  public:
    typedef nimboost::function<
      void (const nimboost::system::error_code&, size_t)> func_t;

    base_handler(nimboost::asio::io_service& io_service)
      : op_(NULL)
      , io_service_(io_service)
      , work_(io_service)
    {}
    
    void do_func(const nimboost::system::error_code& error, size_t size)
    {
      func_(error, size);
    }
  ...
  private:
    ...
    nimboost::asio::io_service& io_service_;
    nimboost::asio::io_service::work work_;
  };  // class base_handler
  ...
  {
  public:
    io_handler(Handler handler, nimboost::asio::io_service& io_service)
      : base_handler<Stream>(io_service)
      , handler_(handler)
    {
      this->set_func(nimboost::bind(
        &io_handler<Stream, Handler>::handler_impl, 
        this, nimboost::arg<1>(), nimboost::arg<2>() ));
    }

  private:
    Handler handler_;
    void handler_impl(const nimboost::system::error_code& error, size_t size)
    {
  ...
  {
  public:
    handshake_handler(Handler handler, nimboost::asio::io_service& io_service)
      : base_handler<Stream>(io_service)
      , handler_(handler)
    {
      this->set_func(nimboost::bind(
  ...
  private:
    Handler handler_;
    void handler_impl(const nimboost::system::error_code& error, size_t)
    {
  ...
  {
  public:
    shutdown_handler(Handler handler, nimboost::asio::io_service& io_service)
      : base_handler<Stream>(io_service),
        handler_(handler)
    { 
      this->set_func(nimboost::bind(
...
public:
  ...
  explicit openssl_stream_service(nimboost::asio::io_service& io_service)
    : nimboost::asio::detail::service_base<openssl_stream_service>(io_service),
      strand_(io_service)
  {
  }
  ...
  template <typename Stream>
  nimboost::system::error_code handshake(impl_type& impl, Stream& next_layer,
      stream_base::handshake_type type, nimboost::system::error_code& ec)
  {
    try
    {
    ...
    catch (nimboost::system::system_error& e)
    {
      ec = e.code();
      return ec;
    }
      ...
      stream_base::handshake_type type, Handler handler)
  {
    ...
    strand_.post(nimboost::bind(&openssl_operation<Stream>::start, op));
  }

  // Shut down SSL on the stream.
  template <typename Stream>
  nimboost::system::error_code shutdown(impl_type& impl, Stream& next_layer,
  ...
  std::size_t write_some(impl_type& impl, Stream& next_layer,
      const Const_Buffers& buffers, nimboost::system::error_code& ec)
  {
    size_t bytes_transferred = 0;
    try
    {
      nimboost::asio::const_buffer buffer =
        nimboost::asio::detail::buffer_sequence_adapter<
          nimboost::asio::const_buffer, Const_Buffers>::first(buffers);

      std::size_t buffer_size = nimboost::asio::buffer_size(buffer);
      ...
      nimboost::function<int (SSL*)> send_func =
        nimboost::bind(nimboost::type<int>(), &::SSL_write, nimboost::arg<1>(),  
      ...
      const Const_Buffers& buffers, Handler handler)
  {
    ...
    {
      get_io_service().post(nimboost::asio::detail::bind_handler(
            handler, nimboost::system::error_code(), 0));
      return;
    }
  ...
  std::size_t read_some(impl_type& impl, Stream& next_layer,
      const Mutable_Buffers& buffers, nimboost::system::error_code& ec)
  {
    size_t bytes_transferred = 0;
    try
    {
      nimboost::asio::mutable_buffer buffer =
        nimboost::asio::detail::buffer_sequence_adapter<
          nimboost::asio::mutable_buffer, Mutable_Buffers>::first(buffers);
      ...
      nimboost::function<int (SSL*)> recv_func =
        nimboost::bind(nimboost::type<int>(), &::SSL_read, nimboost::arg<1>(),
            nimboost::asio::buffer_cast<void*>(buffer),
            static_cast<int>(buffer_size));
...
private:  
  nimboost::asio::io_service::strand strand_;

  typedef nimboost::asio::detail::mutex mutex_type;

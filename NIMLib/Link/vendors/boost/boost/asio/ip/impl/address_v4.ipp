...
address_v4::address_v4(unsigned long addr)
{
  ...
  addr_.s_addr = nimboost::asio::detail::socket_ops::host_to_network_long(
      static_cast<nimboost::asio::detail::u_long_type>(addr));
}
...
{
  return nimboost::asio::detail::socket_ops::network_to_host_long(addr_.s_addr);
}
...
std::string address_v4::to_string(nimboost::system::error_code& ec) const
{
  char addr_str[nimboost::asio::detail::max_addr_v4_str_len];
  const char* addr =
    nimboost::asio::detail::socket_ops::inet_ntop(
        BOOST_ASIO_OS_DEF(AF_INET), &addr_, addr_str,
        nimboost::asio::detail::max_addr_v4_str_len, 0, ec);
  if (addr == 0)
    return std::string();
  return addr;
}
...
{
  address_v4 tmp;
  if (nimboost::asio::detail::socket_ops::inet_pton(

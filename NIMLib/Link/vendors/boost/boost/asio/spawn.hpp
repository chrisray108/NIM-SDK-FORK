...
public:
  ...
  typedef nimboost::coroutines::push_coroutine<void> callee_type;
#else
  typedef nimboost::coroutines::coroutine<void()> callee_type;
#endif
...
#elif defined(BOOST_COROUTINES_UNIDRECT) || defined(BOOST_COROUTINES_V2)
  typedef nimboost::coroutines::pull_coroutine<void> caller_type;
#else
  typedef nimboost::coroutines::coroutine<void()>::caller_type caller_type;
#endif
   ...
   * } @endcode
   */
  basic_yield_context operator[](nimboost::system::error_code& ec) const
  {
...
/**
 * @defgroup spawn nimboost::asio::spawn
 *
 ...
 *
 * @code nimboost::asio::spawn(my_strand, do_echo);
 *
 * // ...
 *
 * void do_echo(nimboost::asio::yield_context yield)
 * {
 *   try
 *   {
 *     char data[128];
 ...
 *           nimboost::asio::buffer(data), yield);
 *
 *       nimboost::asio::async_write(my_socket,
 *           nimboost::asio::buffer(data, length), yield);
 *     }
 *   }
...
void spawn(BOOST_ASIO_MOVE_ARG(Handler) handler,
    ...
    const nimboost::coroutines::attributes& attributes
      = nimboost::coroutines::attributes());

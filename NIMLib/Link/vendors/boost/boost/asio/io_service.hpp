 ...
 *
 * @li nimboost::asio::ip::tcp::socket
 * @li nimboost::asio::ip::tcp::acceptor
 * @li nimboost::asio::ip::udp::socket
 * @li nimboost::asio::deadline_timer.
 *
 ...
 *
 * @code
 * nimboost::asio::io_service io_service;
 * ...
 * for (;;)
 * {
 *   try
 ...
 * creating an object of type nimboost::asio::io_service::work:
 *
 * @code nimboost::asio::io_service io_service;
 * nimboost::asio::io_service::work work(io_service);
 * ... @endcode
 *
 ...
 *
 * @code nimboost::asio::io_service io_service;
 * auto_ptr<nimboost::asio::io_service::work> work(
 *     new nimboost::asio::io_service::work(io_service));
 * ...
...
public:
   ...
   * without blocking.
   */
  BOOST_ASIO_DECL std::size_t run(nimboost::system::error_code& ec);
   ...
   * @return The number of handlers that were executed.
   */
  BOOST_ASIO_DECL std::size_t run_one(nimboost::system::error_code& ec);
   ...
   * @return The number of handlers that were executed.
   */
  BOOST_ASIO_DECL std::size_t poll(nimboost::system::error_code& ec);
   ...
   * @return The number of handlers that were executed.
   */
  BOOST_ASIO_DECL std::size_t poll_one(nimboost::system::error_code& ec);
   ...
   * @code io_service.dispatch(nimboost::bind(f, a1, ... an)); @endcode
   */
   ...
   *
   * @throws nimboost::system::system_error Thrown on failure. If the notification
   ...
   * function:
   * @code my_io_service.notify_fork(nimboost::asio::io_service::fork_prepare);
   * if (fork() == 0)
   * {
   *   // This is the child process.
   *   my_io_service.notify_fork(nimboost::asio::io_service::fork_child);
   * }
   * else
   * {
   *   // This is the parent process.
   *   my_io_service.notify_fork(nimboost::asio::io_service::fork_parent);
   * } @endcode
   *
   ...
   */
  BOOST_ASIO_DECL void notify_fork(nimboost::asio::io_service::fork_event event);
   ...
   *
   * @throws nimboost::asio::service_already_exists Thrown if a service of the
   * given type is already present in the io_service.
   *
   * @throws nimboost::asio::invalid_service_owner Thrown if the service's owning
...
private:
  ...
  nimboost::asio::detail::service_registry* service_registry_;
...
public:
   ...
   */
  explicit work(nimboost::asio::io_service& io_service);
  ...
  nimboost::asio::io_service& get_io_service();
...
protected:
   ...
   */
  BOOST_ASIO_DECL service(nimboost::asio::io_service& owner);
...
private:
  ...
  BOOST_ASIO_DECL virtual void fork_service(
      nimboost::asio::io_service::fork_event event);

  friend class nimboost::asio::detail::service_registry;
  struct key
  {
    key() : type_info_(0), id_(0) {}
    const std::type_info* type_info_;
    const nimboost::asio::io_service::id* id_;
  } key_;

  nimboost::asio::io_service& owner_;
  service* next_;
};
...
class service_id
  : public nimboost::asio::io_service::id
{
};
...
{
public:
  static nimboost::asio::detail::service_id<Type> id;

  // Constructor.
  service_base(nimboost::asio::io_service& io_service)
    : nimboost::asio::io_service::service(io_service)
  {
  }
};

template <typename Type>
nimboost::asio::detail::service_id<Type> service_base<Type>::id;

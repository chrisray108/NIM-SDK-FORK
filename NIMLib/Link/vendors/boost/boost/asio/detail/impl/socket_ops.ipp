...
{
#if defined(BOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
  ec = nimboost::system::error_code(WSAGetLastError(),
      nimboost::asio::error::get_system_category());
#else
  ec = nimboost::system::error_code(errno,
      nimboost::asio::error::get_system_category());
#endif
  return return_value;
}
...
socket_type accept(socket_type s, socket_addr_type* addr,
    std::size_t* addrlen, nimboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = nimboost::asio::error::bad_descriptor;
  ...
  ec = nimboost::system::error_code();
  return new_s;
}

socket_type sync_accept(socket_type s, state_type state,
    socket_addr_type* addr, std::size_t* addrlen, nimboost::system::error_code& ec)
{
  // Accept a socket.
  for (;;)
  {
    ...
    else if (ec == nimboost::asio::error::connection_aborted)
    {
...
void complete_iocp_accept(socket_type s,
    ...
    socket_type new_socket, nimboost::system::error_code& ec)
{
  // Map non-portable errors to their portable counterparts.
  if (ec.value() == ERROR_NETNAME_DELETED)
    ec = nimboost::asio::error::connection_aborted;
  ...
  if (!ec)
  {
    ...
    if (addr && addrlen)
    {
      ...
      {
        ec = nimboost::asio::error::invalid_argument;
      }
      else
      {
...
bool non_blocking_accept(socket_type s,
    ...
    nimboost::system::error_code& ec, socket_type& new_socket)
{
  for (;;)
  {
...
int close(socket_type s, state_type& state,
    bool destruction, nimboost::system::error_code& ec)
{
  int result = 0;
  if (s != invalid_socket)
  {
    ...
    if (destruction && (state & user_set_linger))
    {
      ...
      opt.l_onoff = 0;
      opt.l_linger = 0;
      nimboost::system::error_code ignored_ec;
    ...
    if (result != 0
        && (ec == nimboost::asio::error::would_block
          || ec == nimboost::asio::error::try_again))
    {
...
bool set_user_non_blocking(socket_type s,
    state_type& state, bool value, nimboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = nimboost::asio::error::bad_descriptor;
...
int shutdown(socket_type s, int what, nimboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = nimboost::asio::error::bad_descriptor;
    ...
    std::size_t addrlen, nimboost::system::error_code& ec)
{
  ...
  else if (ec == nimboost::asio::error::try_again)
    ec = nimboost::asio::error::no_buffer_space;
    ...
    std::size_t addrlen, nimboost::system::error_code& ec)
{
  ...
  if (ec != nimboost::asio::error::in_progress
      && ec != nimboost::asio::error::would_block)
  {
  ...
  ec = nimboost::system::error_code(connect_error,
      nimboost::asio::error::get_system_category());
}

bool non_blocking_connect(socket_type s, nimboost::system::error_code& ec)
{
...
int socketpair(int af, int type, int protocol,
    socket_type sv[2], nimboost::system::error_code& ec)
{
#if defined(BOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(af);
  (void)(type);
  (void)(protocol);
  (void)(sv);
  ec = nimboost::asio::error::operation_not_supported;
  return socket_error_retval;
#else
  clear_last_error();
...
bool sockatmark(socket_type s, nimboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = nimboost::asio::error::bad_descriptor;
...
# if defined(ENOTTY)
  if (ec.value() == ENOTTY)
    ec = nimboost::asio::error::not_socket;
...
size_t available(socket_type s, nimboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = nimboost::asio::error::bad_descriptor;
...
int listen(socket_type s, int backlog, nimboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = nimboost::asio::error::bad_descriptor;
...
signed_size_type recv(socket_type s, buf* bufs, size_t count,
    int flags, nimboost::system::error_code& ec)
{
  clear_last_error();
#if defined(BOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
  // Receive some data.
  ...
  if (ec.value() == ERROR_NETNAME_DELETED)
    ec = nimboost::asio::error::connection_reset;
  else if (ec.value() == ERROR_PORT_UNREACHABLE)
    ec = nimboost::asio::error::connection_refused;
  if (result != 0)
    return socket_error_retval;
  ec = nimboost::system::error_code();
...
size_t sync_recv(socket_type s, state_type state, buf* bufs,
    size_t count, int flags, bool all_empty, nimboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = nimboost::asio::error::bad_descriptor;
...
void complete_iocp_recv(state_type state,
    ...
    nimboost::system::error_code& ec, size_t bytes_transferred)
{
  // Map non-portable errors to their portable counterparts.
  if (ec.value() == ERROR_NETNAME_DELETED)
  {
    if (cancel_token.expired())
      ec = nimboost::asio::error::operation_aborted;
    else
      ec = nimboost::asio::error::connection_reset;
  }
  else if (ec.value() == ERROR_PORT_UNREACHABLE)
...
signed_size_type recvmsg(socket_type s, buf* bufs, size_t count,
    int in_flags, int& out_flags, nimboost::system::error_code& ec)
{
  clear_last_error();
#if defined(BOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
  out_flags = 0;
...
int setsockopt(socket_type s, state_type& state, int level, int optname,
    const void* optval, std::size_t optlen, nimboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = nimboost::asio::error::bad_descriptor;
...
int getsockopt(socket_type s, state_type state, int level, int optname,
    void* optval, size_t* optlen, nimboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = nimboost::asio::error::bad_descriptor;
...
int getpeername(socket_type s, socket_addr_type* addr,
    std::size_t* addrlen, bool cached, nimboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = nimboost::asio::error::bad_descriptor;
  ...
  if (cached)
  {
    ...
    {
      ec = nimboost::asio::error::not_connected;
      return socket_error_retval;
    }
...
int ioctl(socket_type s, state_type& state, int cmd,
    ioctl_arg_type* arg, nimboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = nimboost::asio::error::bad_descriptor;
...
int select(int nfds, fd_set* readfds, fd_set* writefds,
    fd_set* exceptfds, timeval* timeout, nimboost::system::error_code& ec)
{
  clear_last_error();
#if defined(BOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
  ...
  // system clock (as wrapped by nimboost::posix_time::microsec_clock) typically
...
int poll_read(socket_type s, state_type state, nimboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = nimboost::asio::error::bad_descriptor;
  ...
  if (result == 0)
    ec = (state & user_set_non_blocking)
      ? nimboost::asio::error::would_block : nimboost::system::error_code();
  else if (result > 0)
    ec = nimboost::system::error_code();
  return result;
}

int poll_write(socket_type s, state_type state, nimboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = nimboost::asio::error::bad_descriptor;
...
int poll_connect(socket_type s, nimboost::system::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = nimboost::asio::error::bad_descriptor;
...
const char* inet_ntop(int af, const void* src, char* dest, size_t length,
    unsigned long scope_id, nimboost::system::error_code& ec)
{
  clear_last_error();
#if defined(BOOST_ASIO_WINDOWS_RUNTIME)
  ...
  {
    ec = nimboost::asio::error::address_family_not_supported;
    return 0;
  }
#elif defined(BOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
  using namespace std; // For memcpy.
...
int gethostname(char* name, int namelen, nimboost::system::error_code& ec)
{
  clear_last_error();
#if defined(BOOST_ASIO_WINDOWS_RUNTIME)
  try
  ...
  {
    ec = nimboost::system::error_code(e->HResult,
        nimboost::system::system_category());
    return -1;
  }
...
inline nimboost::system::error_code translate_netdb_error(int error)
{
  switch (error)
  {
  case 0:
    return nimboost::system::error_code();
  case HOST_NOT_FOUND:
    return nimboost::asio::error::host_not_found;
  case TRY_AGAIN:
    return nimboost::asio::error::host_not_found_try_again;
  case NO_RECOVERY:
    return nimboost::asio::error::no_recovery;
  case NO_DATA:
    return nimboost::asio::error::no_data;
  default:
    BOOST_ASIO_ASSERT(false);
    return nimboost::asio::error::invalid_argument;
  }
}

inline hostent* gethostbyaddr(const char* addr, int length, int af,
    hostent* result, char* buffer, int buflength, nimboost::system::error_code& ec)
{
  clear_last_error();
#if defined(BOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(buffer);
...
inline hostent* gethostbyname(const char* name, int af, struct hostent* result,
    char* buffer, int buflength, int ai_flags, nimboost::system::error_code& ec)
{
  clear_last_error();
#if defined(BOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(buffer);
    ...
    const addrinfo_type* hintsp, addrinfo_type** result)
{
  ...
  for (gai_search* sptr = search; sptr < search + search_count; ++sptr)
  {
    ...
    if (hptr == 0)
    {
      ...
      freeaddrinfo_emulation(aihead);
      gai_free(canon);
      if (ec == nimboost::asio::error::host_not_found)
        return EAI_NONAME;
      if (ec == nimboost::asio::error::host_not_found_try_again)
        return EAI_AGAIN;
      if (ec == nimboost::asio::error::no_recovery)
        return EAI_FAIL;
      if (ec == nimboost::asio::error::no_data)
        return EAI_NONAME;
      return EAI_NONAME;
    }
...
inline nimboost::system::error_code getnameinfo_emulation(
    ...
    nimboost::system::error_code& ec)
{
  ...
  case BOOST_ASIO_OS_DEF(AF_INET):
    ...
    {
      return ec = nimboost::asio::error::invalid_argument;
    }
  ...
  case BOOST_ASIO_OS_DEF(AF_INET6):
    ...
    break;
  default:
    return ec = nimboost::asio::error::address_family_not_supported;
  }
  ...
  if (host && hostlen > 0)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          return ec = nimboost::asio::error::host_not_found;
        }
  ...
  if (serv && servlen > 0)
  {
    if (flags & NI_NUMERICSERV)
    {
      if (servlen < 6)
      {
        return ec = nimboost::asio::error::no_buffer_space;
      }
#if defined(BOOST_ASIO_HAS_SECURE_RTL)
...
inline nimboost::system::error_code translate_addrinfo_error(int error)
{
  switch (error)
  {
  case 0:
  ...
  case EAI_FAIL:
    return nimboost::asio::error::no_recovery;
  case EAI_FAMILY:
    return nimboost::asio::error::address_family_not_supported;
  case EAI_MEMORY:
    return nimboost::asio::error::no_memory;
  case EAI_NONAME:
  ...
  case EAI_NODATA:
    ...
    return nimboost::asio::error::host_not_found;
  case EAI_SERVICE:
    return nimboost::asio::error::service_not_found;
  case EAI_SOCKTYPE:
    return nimboost::asio::error::socket_type_not_supported;
  default: // Possibly the non-portable EAI_SYSTEM.
#if defined(BOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
    return nimboost::system::error_code(
        WSAGetLastError(), nimboost::asio::error::get_system_category());
#else
    return nimboost::system::error_code(
        errno, nimboost::asio::error::get_system_category());
#endif
  }
}

nimboost::system::error_code getaddrinfo(const char* host,
    const char* service, const addrinfo_type& hints,
    addrinfo_type** result, nimboost::system::error_code& ec)
{
...
nimboost::system::error_code background_getaddrinfo(
...
nimboost::system::error_code getnameinfo(const socket_addr_type* addr,
    std::size_t addrlen, char* host, std::size_t hostlen,
    char* serv, std::size_t servlen, int flags, nimboost::system::error_code& ec)
{
#if defined(BOOST_ASIO_WINDOWS) || defined(__CYGWIN__)
# if defined(_WIN32_WINNT) && (_WIN32_WINNT >= 0x0501) || defined(UNDER_CE)
  // Building for Windows XP, Windows Server 2003, or later.
...
nimboost::system::error_code sync_getnameinfo(
    const socket_addr_type* addr, std::size_t addrlen,
    char* host, std::size_t hostlen, char* serv,
    std::size_t servlen, int sock_type, nimboost::system::error_code& ec)
{
...
nimboost::system::error_code background_getnameinfo(
    const weak_cancel_token_type& cancel_token,
    const socket_addr_type* addr, std::size_t addrlen,
    char* host, std::size_t hostlen, char* serv,
    std::size_t servlen, int sock_type, nimboost::system::error_code& ec)

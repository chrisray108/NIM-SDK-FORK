...
kqueue_reactor::kqueue_reactor(nimboost::asio::io_service& io_service)
  : nimboost::asio::detail::service_base<kqueue_reactor>(io_service),
...
void kqueue_reactor::fork_service(nimboost::asio::io_service::fork_event fork_ev)
{
  if (fork_ev == nimboost::asio::io_service::fork_child)
  {
        ...
        state != 0; state = state->next_)
    {
      ...
      {
        nimboost::system::error_code error(errno,
            nimboost::asio::error::get_system_category());
        nimboost::asio::detail::throw_error(error);
      }
    }
  }
}
...
int kqueue_reactor::do_kqueue_create()
{
  ...
  if (fd == -1)
  {
    nimboost::system::error_code ec(errno,
        nimboost::asio::error::get_system_category());
    nimboost::asio::detail::throw_error(ec, "kqueue");
  }
  return fd;
}

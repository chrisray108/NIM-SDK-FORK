...
win_iocp_io_service::win_iocp_io_service(
    nimboost::asio::io_service& io_service, size_t concurrency_hint)
  : nimboost::asio::detail::service_base<win_iocp_io_service>(io_service),
    iocp_(),
    outstanding_work_(0),
    stopped_(0),
    ...
    dispatch_required_(0)
{
  ...
  if (!iocp_.handle)
  {
    ...
    nimboost::system::error_code ec(last_error,
        nimboost::asio::error::get_system_category());
    nimboost::asio::detail::throw_error(ec, "iocp");
  }
}
...
nimboost::system::error_code win_iocp_io_service::register_handle(
    HANDLE handle, nimboost::system::error_code& ec)
{
...
size_t win_iocp_io_service::run(nimboost::system::error_code& ec)
{
...
size_t win_iocp_io_service::run_one(nimboost::system::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_service::poll(nimboost::system::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_service::poll_one(nimboost::system::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
  ...
  if (::InterlockedExchange(&stopped_, 1) == 0)
  {
    if (::InterlockedExchange(&stop_event_posted_, 1) == 0)
    {
      if (!::PostQueuedCompletionStatus(iocp_.handle, 0, 0, 0))
      {
        ...
        nimboost::system::error_code ec(last_error,
            nimboost::asio::error::get_system_category());
        nimboost::asio::detail::throw_error(ec, "pqcs");
      }
    }
  }
}
...
void win_iocp_io_service::on_completion(win_iocp_operation* op,
    const nimboost::system::error_code& ec, DWORD bytes_transferred)
{
  // Flag that the operation is ready for invocation.
  op->ready_ = 1;
...
size_t win_iocp_io_service::do_one(bool block, nimboost::system::error_code& ec)
{
  for (;;)
  {
    ...
    if (overlapped)
    {
      ...
      nimboost::system::error_code result_ec(last_error,
          nimboost::asio::error::get_system_category());
      ...
      {
        result_ec = nimboost::system::error_code(static_cast<int>(op->Offset),
            *reinterpret_cast<nimboost::system::error_category*>(op->Internal));
...
void win_iocp_io_service::do_add_timer_queue(timer_queue_base& queue)
{
  ...
  if (!waitable_timer_.handle)
  {
    ...
    if (waitable_timer_.handle == 0)
    {
      ...
      nimboost::system::error_code ec(last_error,
          nimboost::asio::error::get_system_category());
      nimboost::asio::detail::throw_error(ec, "timer");
    }

...
namespace nimboost {
    ...
    public:
        ...
        const charT& at(size_t pos) const {
            if ( pos >= len_ )
                BOOST_THROW_EXCEPTION( std::out_of_range ( "nimboost::string_ref::at" ) );
            return ptr_[pos];
            }
...
namespace std {
    // Hashing
    template<> struct hash<nimboost::string_ref>;
    template<> struct hash<nimboost::u16string_ref>;
    template<> struct hash<nimboost::u32string_ref>;
    template<> struct hash<nimboost::wstring_ref>;
}
#endif

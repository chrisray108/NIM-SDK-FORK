...
struct add_alignment
{
    ...
    struct apply
        : mpl::size_t<
              ::nimboost::math::static_lcm<
                  BOOST_MPL_AUX_VALUE_WKND(State)::value
                , ::nimboost::alignment_of<Item>::value
                >::value
            >
    {};
};
...
//
// This implementation is designed to prefer nimboost::blank over other potential
...
private: // helpers, for metafunction result (below)
    ...
    // [...now search the rest of the sequence for nimboost::blank...]

    typedef typename mpl::iter_fold_if<
          mpl::iterator_range< first_result_it,end_it >
        , first_result_index, mpl::protect< mpl::next<> >
        , mpl::protect< mpl::not_same_as<nimboost::blank> >
        >::type second_result_;
...
struct is_variant_move_noexcept {
    typedef typename nimboost::mpl::find_if<
        Types, mpl::not_<nimboost::is_nothrow_move_constructible<nimboost::mpl::_1> >
    >::type iterator_t;

    typedef typename nimboost::mpl::end<Types>::type end_t;
...
# pragma warning( disable : 4702 ) // unreachable code 
    ...
    T& operator()(U&) const
    {
        ...
        BOOST_ASSERT(false);
        return ::nimboost::detail::variant::forced_return< T& >();
    }
#if defined BOOST_MSVC 
# pragma warning( pop ) 
#endif
...
public: // internal visitor interface
    ...
    template <typename T>
        BOOST_VARIANT_AUX_RETURN_VOID_TYPE
    internal_visit(nimboost::detail::variant::backup_holder<T>& operand, long) const
    {
    ...
    template <typename T>
        BOOST_VARIANT_AUX_RETURN_VOID_TYPE
    internal_visit(const nimboost::detail::variant::backup_holder<T>& operand, long) const
    {
        new(storage_) T( operand.get() );
        BOOST_VARIANT_AUX_RETURN_VOID;
    }
...
public: // internal visitor interface
    ...
    internal_visit(nimboost::detail::variant::backup_holder<T>& operand, long) const
    {
        new(storage_) T( ::nimboost::detail::variant::move(operand.get()) );
        BOOST_VARIANT_AUX_RETURN_VOID;
    }

    template <typename T>
        BOOST_VARIANT_AUX_RETURN_VOID_TYPE
    internal_visit(T& operand, int) const BOOST_NOEXCEPT_IF(BOOST_NOEXCEPT_EXPR(T(nimboost::declval<T>())))
    {
        new(storage_) T(::nimboost::detail::variant::move(operand));
        BOOST_VARIANT_AUX_RETURN_VOID;
    }
};
#endif
...
public: // internal visitor interfaces
    ...
    {
        lhs_content.get()
            = ::nimboost::detail::variant::move(static_cast<backup_holder<T>* >(rhs_storage_)->get());
        BOOST_VARIANT_AUX_RETURN_VOID;
    }
    ...
    internal_visit(T& lhs_content, int) const
    {
        ...
        // Hint: Are any of the bounded types const-qualified or references?
        //
        lhs_content = ::nimboost::detail::variant::move(*static_cast<T* >(rhs_storage_));
        BOOST_VARIANT_AUX_RETURN_VOID;
    }
...
public: // visitor interface
    ...
    bool operator()(T& lhs)
    {
        lhs = ::nimboost::detail::variant::move(rhs_);
        return true;
    }
...
private: // helpers, for visitor interface (below)
        ...
        )
    {
        ...
        LhsT backup_lhs_content(
              ::nimboost::detail::variant::move(lhs_content)
            ); // nothrow
        ...
        BOOST_CATCH (...)
        {
            ...
            new(lhs_.storage_.address())
                LhsT(
                      ::nimboost::detail::variant::move(backup_lhs_content)
                    ); // nothrow
...
public: // internal visitor interfaces
    ...
    void operator()(T& operand) const
    {
        ...
        // ...and swap:
        ::nimboost::detail::variant::move_swap( operand, other );
    }
...
public: // internal visitor interfaces, cont.
    ...
    template <typename T>
        BOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(nimboost::recursive_wrapper<T>& operand, long)
    {
    ...
    template <typename T>
        BOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(const nimboost::recursive_wrapper<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
    ...
    template <typename T>
        BOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(nimboost::detail::reference_content<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
    ...
    template <typename T>
        BOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(const nimboost::detail::reference_content<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
    ...
    template <typename T>
        BOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(nimboost::detail::variant::backup_holder<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
    ...
    template <typename T>
        BOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(const nimboost::detail::variant::backup_holder<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
...
private: // helpers, for typedefs (below)
    ...
    BOOST_STATIC_ASSERT((
          ::nimboost::mpl::not_< mpl::empty<specified_types> >::value
        ));
...
#   endif
        ...
        template <typename T>
        int internal_visit(nimboost::detail::reference_content<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(const nimboost::detail::reference_content<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(nimboost::detail::variant::backup_holder<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(const nimboost::detail::variant::backup_holder<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(nimboost::recursive_wrapper<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(const nimboost::recursive_wrapper<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }
...
private: // helpers, for structors, below 
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
    template <typename T>
    typename nimboost::enable_if<nimboost::is_rvalue_reference<T&&> >::type convert_construct(
          T&& operand
        , int
        , mpl::false_ = mpl::false_() // is_foreign_variant
        )
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
    template <typename Variant>
    typename nimboost::enable_if<nimboost::is_rvalue_reference<Variant&&> >::type convert_construct(
          Variant&& operand
        , long
        , mpl::true_// is_foreign_variant
        )
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
    template <typename Variant>
    typename nimboost::enable_if<nimboost::is_rvalue_reference<Variant&&> >::type convert_construct_variant(Variant&& operand)
    {
        // [Determine if the given variant is itself a bounded type, or if its
        //  content needs to be converted (i.e., it is a 'foreign' variant):]
        //
    ...
    void convert_construct(
          nimboost::variant<BOOST_VARIANT_ENUM_PARAMS(U)>& operand
        , long
        )
    {
    ...
    template <BOOST_VARIANT_ENUM_PARAMS(typename U)>
    void convert_construct(
          const nimboost::variant<BOOST_VARIANT_ENUM_PARAMS(U)>& operand
        , long
        )
    {
        convert_construct_variant(operand);    
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
    template <class T>
    variant(T&& operand, typename nimboost::enable_if<nimboost::is_rvalue_reference<T&&> >::type* = 0, 
        typename nimboost::disable_if<nimboost::is_const<T> >::type* = 0)
    {
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
    template <class T>
    typename nimboost::enable_if_c<nimboost::is_rvalue_reference<T&&>::value && !nimboost::is_const<T>::value, variant& >::type 
        operator=(T&& rhs)
    {
...
#if !BOOST_WORKAROUND(BOOST_MSVC, <= 1300)
    BOOST_STATIC_ASSERT(( ::nimboost::mpl::is_sequence<Types>::value ));
#endif

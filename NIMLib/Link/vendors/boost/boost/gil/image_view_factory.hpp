...
typename detail::channel_pointer_type<HomogeneousView>::type interleaved_view_get_raw_data(const HomogeneousView& view) {
    ...
    BOOST_STATIC_ASSERT((nimboost::is_pointer<typename HomogeneousView::x_iterator>::value));
...
struct color_converted_view_type : public detail::_color_converted_view_type<SrcView,
    ...
    GIL_CLASS_REQUIRE(DstP, nimboost::gil, MutablePixelConcept)//why does it have to be mutable???
};
...
private:
    GIL_CLASS_REQUIRE(View, nimboost::gil, ImageViewConcept)

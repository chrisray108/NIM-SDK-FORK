...
nimboost::mpl::aux::weighted_tag<1>::type operator==( assert_, assert_ );
nimboost::mpl::aux::weighted_tag<2>::type operator!=( assert_, assert_ );
nimboost::mpl::aux::weighted_tag<3>::type operator>(  assert_, assert_ );
nimboost::mpl::aux::weighted_tag<4>::type operator>=( assert_, assert_ );
nimboost::mpl::aux::weighted_tag<5>::type operator<( assert_, assert_ );
nimboost::mpl::aux::weighted_tag<6>::type operator<=( assert_, assert_ );
...
struct eval_assert {
    ...
    typedef typename ::nimboost::mpl::if_c<p_type::value,
...
template< typename Pred >
typename assert_arg_type< nimboost::mpl::not_<Pred> >::type 
...
BOOST_MPL_AUX_ASSERT_CONSTANT( \
    ...
    , BOOST_PP_CAT(mpl_assertion_in_line_,BOOST_MPL_AUX_PP_COUNTER()) = sizeof( \
          nimboost::mpl::assertion_failed<false>( \
              nimboost::mpl::make_assert_arg< \
                  typename nimboost::mpl::eval_assert<void pred>::type \
                >() \
            ) \
        ) \
    ) \
...
BOOST_MPL_AUX_ASSERT_CONSTANT( \
    ...
    , BOOST_PP_CAT(mpl_assertion_in_line_,BOOST_MPL_AUX_PP_COUNTER()) = sizeof( \
          nimboost::mpl::assertion_failed<false>( \
              nimboost::mpl::make_assert_arg< \
                  typename nimboost::mpl::eval_assert_not<void pred>::type \
                >() \
            ) \
        ) \
    ) \
...
BOOST_MPL_AUX_ASSERT_CONSTANT( \
    ...
    , BOOST_PP_CAT(mpl_assertion_in_line_,BOOST_MPL_AUX_PP_COUNTER()) = sizeof( \
          nimboost::mpl::assertion_failed<false>( \
              nimboost::mpl::assert_arg( (void (*) pred)0, 1 ) \
            ) \
        ) \
    ) \
/**/
...
enum { \
      BOOST_PP_CAT(mpl_assertion_in_line_,BOOST_MPL_AUX_PP_COUNTER()) = sizeof( \
          nimboost::mpl::assertion<false>::failed( \
              nimboost::mpl::assert_not_arg( (void (*) pred)0, 1 ) \
            ) \
        ) \
}\
/**/
...
BOOST_MPL_AUX_ASSERT_CONSTANT( \
    ...
    , BOOST_PP_CAT(mpl_assertion_in_line_,counter) = sizeof( \
        nimboost::mpl::assertion_failed<BOOST_PP_CAT(mpl_assert_rel_value,counter)>( \
            (nimboost::mpl::failed ************ ( nimboost::mpl::assert_relation< \
                  nimboost::mpl::assert_::relations( sizeof( \
                      nimboost::mpl::assert_::arg rel nimboost::mpl::assert_::arg \
                    ) ) \
                , x \
                , y \
                >::************)) 0 ) \
...
BOOST_MPL_AUX_ASSERT_CONSTANT( \
    ...
    , BOOST_PP_CAT(mpl_assertion_in_line_,counter) = sizeof( \
        nimboost::mpl::assertion_failed<BOOST_PP_CAT(mpl_assert_rel_value,counter)>( \
              nimboost::mpl::assert_rel_arg( nimboost::mpl::assert_relation< \
                  nimboost::mpl::assert_::relations(BOOST_PP_CAT(mpl_assert_rel,counter)) \
                , x \
                , y \
                >() ) \
            ) \
...
BOOST_MPL_AUX_ASSERT_CONSTANT( \
      std::size_t \
    , BOOST_PP_CAT(mpl_assertion_in_line_,BOOST_MPL_AUX_PP_COUNTER()) = sizeof( \
        nimboost::mpl::assertion_failed<(x rel y)>( nimboost::mpl::assert_rel_arg( \
              nimboost::mpl::BOOST_MPL_AUX_ASSERT_RELATION(x,y,(&nimboost::mpl::operator rel))() \
            ) ) \
        ) \
    ) \
/**/
...
BOOST_MPL_AUX_ASSERT_CONSTANT( \
      std::size_t \
    , BOOST_PP_CAT(mpl_assertion_in_line_,BOOST_MPL_AUX_PP_COUNTER()) = sizeof( \
        nimboost::mpl::assertion_failed<(x rel y)>( (nimboost::mpl::failed ************ ( \
            nimboost::mpl::BOOST_MPL_AUX_ASSERT_RELATION(x,y,(&nimboost::mpl::operator rel))::************))0 ) \
        ) \
    ) \
/**/
#   endif
...
struct msg; \
typedef struct BOOST_PP_CAT(msg,counter) : nimboost::mpl::assert_ \
{ \
    using nimboost::mpl::assert_::types; \
    static nimboost::mpl::failed ************ (msg::************ assert_arg()) types_ \
    { return 0; } \
} BOOST_PP_CAT(mpl_assert_arg,counter); \
BOOST_MPL_AUX_ASSERT_CONSTANT( \
      std::size_t \
    , BOOST_PP_CAT(mpl_assertion_in_line_,counter) = sizeof( \
        nimboost::mpl::assertion<(c)>::failed( BOOST_PP_CAT(mpl_assert_arg,counter)::assert_arg() ) \
        ) \
    ) \
/**/
#else
...
BOOST_MPL_AUX_ASSERT_CONSTANT( \
      std::size_t \
    , BOOST_PP_CAT(mpl_assertion_in_line_,counter) = sizeof( \
        nimboost::mpl::assertion_failed<(c)>( BOOST_PP_CAT(mpl_assert_arg,counter)::assert_arg() ) \
        ) \
    ) \
/**/
#endif

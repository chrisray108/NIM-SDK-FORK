         ...
         >
{
   ...
   list_impl(BOOST_RV_REF(list_impl) x)
      : data_(::nimboost::move(x.priv_value_traits()))
   {
      this->priv_size_traits().set_size(size_type(0));
      node_algorithms::init_header(this->get_root_node());
      this->swap(x);
   ...
   static list_impl &priv_container_from_end_iterator(const const_iterator &end_iterator)
   {
      root_plus_size *r = detail::parent_from_member<root_plus_size, node>
         ( nimboost::intrusive::detail::to_raw_pointer(end_iterator.pointed_node()), &root_plus_size::root_);
   ...
   >::type
{
   ...
   {  return static_cast<list &>(this->Base::operator=(::nimboost::move(static_cast<Base&>(x))));  }

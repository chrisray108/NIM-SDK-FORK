...
namespace detail
{
    ...
    public:
        ...
        typedef typename nimboost::mpl::if_c< nimboost::has_trivial_assign<T>::value
    ...
    private:
        ...
        {
            copy_rai( begin, end, where, nimboost::has_trivial_assign<T>() );
        }

        static void copy_rai( const T* begin, const T* end,
                              pointer where, const nimboost::true_type& )
        {
        ...
        static void copy_rai( I begin, I end,
                              pointer where, const nimboost::integral_constant<bool, b>& )
        {
        ...
        {
            assign_impl( begin, end, where, nimboost::has_trivial_assign<T>() );
        }

        template< class I, class I2 >
        static void assign_impl( I begin, I end, I2 where, const nimboost::true_type& )
        {
            std::memcpy( where, begin, sizeof(T) * std::distance(begin,end) );
        }

        template< class I, class I2 >
        static void assign_impl( I begin, I end, I2 where, const nimboost::false_type& )
        {
            for( ; begin != end; ++begin, ++where )
                *where = *begin;
        }

        void unchecked_push_back_n( size_type n, const nimboost::true_type& )
        {
            std::uninitialized_fill( end(), end() + n, T() );
            size_ += n;
        }

        void unchecked_push_back_n( size_type n, const nimboost::false_type& )
        {
            for( size_type i = 0u; i < n; ++i )
                unchecked_push_back();
        }

        void auto_buffer_destroy( pointer where, const nimboost::false_type& )
        {
            (*where).~T();
        }

        void auto_buffer_destroy( pointer, const nimboost::true_type& )
        { }

        void auto_buffer_destroy( pointer where )
        {
            auto_buffer_destroy( where, nimboost::has_trivial_destructor<T>() );
        }

        void destroy_back_n( size_type n, const nimboost::false_type& )
        {
            BOOST_ASSERT( n > 0 );
        ...
        void destroy_back_n( size_type n, const nimboost::true_type& )
        { }

        void destroy_back_n( size_type n )
        {
            destroy_back_n( n, nimboost::has_trivial_destructor<T>() );
        }

        void auto_buffer_destroy( const nimboost::false_type& x )
        {
            if( size_ )
        ...
        void auto_buffer_destroy( const nimboost::true_type& )
        {
            deallocate( buffer_, members_.capacity_ );
        }

        pointer move_to_new_buffer( size_type new_capacity, const nimboost::false_type& )
        {
            pointer new_buffer = allocate( new_capacity ); // strong
            nimboost::multi_index::detail::scope_guard guard =
                nimboost::multi_index::detail::make_obj_guard( *this,
        ...
        pointer move_to_new_buffer( size_type new_capacity, const nimboost::true_type& )
        {
            pointer new_buffer = allocate( new_capacity ); // strong
                                 ...
                                 const nimboost::true_type& )
        {
            ...
            nimboost::swap( l.size_, r.size_ );
            nimboost::swap( l.members_.capacity_, r.members_.capacity_ );
        }

        static void swap_helper( auto_buffer& l, auto_buffer& r,
                                 const nimboost::false_type& )
        {
            BOOST_ASSERT( l.is_on_stack() && r.is_on_stack() );
            ...
            size_type i = 0u;
            for(  ; i < min_size; ++i )
                nimboost::swap( (*smallest)[i], (*largest)[i] );
        ...
        void grow_back( size_type n, const nimboost::true_type& )
        {
            BOOST_ASSERT( size_ + n <= members_.capacity_ );
            size_ += n;
        }

        void grow_back( size_type n, const nimboost::false_type& )
        {
        ...
        {
            grow_back( n, nimboost::has_trivial_constructor<T>() );
        }

        void grow_back_one( const nimboost::true_type& )
        {
            BOOST_ASSERT( size_ + 1 <= members_.capacity_ );
            size_ += 1;
        }

        void grow_back_one( const nimboost::false_type& )
        {
            unchecked_push_back();
        }
        ...
        void grow_back_one()
        {
            grow_back_one( nimboost::has_trivial_constructor<T>() );
        }
    ...
    public:
        ...
        ~auto_buffer()
        {
            ...
            if( buffer_ ) // do we need this check? Yes, but only
                ...
                auto_buffer_destroy( nimboost::has_trivial_destructor<T>() );
        }
    ...
    public:
        ...
        {
            BOOST_ASSERT( size_ + n <= members_.capacity_ );
            unchecked_push_back_n( n, nimboost::has_trivial_assign<T>() );
        }
        ...
        {
            BOOST_ASSERT( !empty() );
            auto_buffer_destroy( buffer_ + size_ - 1, nimboost::has_trivial_destructor<T>() );
            --size_;
        }
        ...
        void swap( auto_buffer& r )
        {
            ...
            if( both_on_heap )
            {
                nimboost::swap( get_allocator(), r.get_allocator() );
                nimboost::swap( members_.capacity_, r.members_.capacity_ );
                nimboost::swap( buffer_, r.buffer_ );
                nimboost::swap( size_, r.size_ );
                BOOST_ASSERT( is_valid() );
            ...
            if( exactly_one_on_stack )
            {
                ...
                nimboost::swap( get_allocator(), r.get_allocator() );  // assume nothrow
                nimboost::swap( members_.capacity_, r.members_.capacity_ );
                nimboost::swap( size_, r.size_ );
            ...
            BOOST_ASSERT( on_stack && r_on_stack );
            swap_helper( *this, r, nimboost::has_trivial_assign<T>() );
            BOOST_ASSERT( is_valid() );
            BOOST_ASSERT( r.is_valid() );
        }

    private:
        typedef nimboost::aligned_storage< N * sizeof(T),
                                        nimboost::alignment_of<T>::value >
                               storage;

...
#define BOOST_IOSTREAMS_PIPABLE(filter, arity) \
    ...
    ::nimboost::iostreams::pipeline< \
        ::nimboost::iostreams::detail::pipeline_segment< \
    ...
    > operator|( const filter BOOST_IOSTREAMS_TEMPLATE_ARGS(arity, T)& f, \
                 const Component& c ) \
    { \
        typedef ::nimboost::iostreams::detail::pipeline_segment< \
                    filter BOOST_IOSTREAMS_TEMPLATE_ARGS(arity, T) \
                > segment; \
        return ::nimboost::iostreams::pipeline<segment, Component> \
                   (segment(f), c); \
    } \
    /**/

...
#else
# define BOOST_IOSTREAMS_COMPONENT(chain, index, target) \
    chain.component( index, ::nimboost::type< target >() ) \
    /**/
#endif
...
public:
    ...
    const std::type_info& component_type(int n) const
    {
        if (static_cast<size_type>(n) >= size())
            nimboost::throw_exception(std::out_of_range("bad chain offset"));
        return (*nimboost::next(list().begin(), n))->component_type();
    }
    ...
    template<typename T>
    T* component(int n) const { return component(n, nimboost::type<T>()); }
...
#endif
    template<typename T>
    T* component(int n, nimboost::type<T>) const
    {
        if (static_cast<size_type>(n) >= size())
            nimboost::throw_exception(std::out_of_range("bad chain offset"));
        streambuf_type* link = *nimboost::next(list().begin(), n);
...
private:
                   ...
                   std::streamsize pback_size = -1)
    {
        ...
        if (is_complete())
            nimboost::throw_exception(std::logic_error("chain complete"));
        ...
        void close()
            {
                if ((flags_ & f_open) != 0) {
                    ...
                    try {
                        nimboost::iostreams::detail::execute_foreach(
...
#define BOOST_IOSTREAMS_DECL_CHAIN(name_, default_char_) \
    ...
    class name_ : public nimboost::iostreams::detail::chain_base< \
    ...
    public: \
        ...
        typedef Mode                                   mode; \
    private: \
        typedef nimboost::iostreams::detail::chain_base< \
                    name_<Mode, Ch, Tr, Alloc>, \
                    Ch, Tr, Alloc, Mode \
                >                                      base_type; \
    public: \
...
#else
    template<typename T>
    T* component(int n, nimboost::type<T> t) const

...
// nimboost::filesystem::path, nimboost::filesystem::wpath, 
// and nimboost::iostreams::detail::path (used to store either a std::string or a
// std::wstring).
    ...
    : detail::mapped_file_params_base 
{
    ...
    // with nimboost::filesystem::wpath
...
private:
    ...
    friend struct nimboost::iostreams::operations<mapped_file_source>;
public:
...
private:
    ...
    nimboost::shared_ptr<impl_type> pimpl_;
};
...
private:
    ...
    typedef basic_mapped_file_params<detail::path>  param_type;
    friend struct nimboost::iostreams::operations<mapped_file >;
...
public:
    friend struct nimboost::iostreams::operations<mapped_file_sink>;
...
mapped_file_source::mapped_file_source( 
    const Path& path, size_type length, nimboost::intmax_t offset)
...
void mapped_file_source::open(const basic_mapped_file_params<Path>& p)
{
    ...
    if (params.flags) {
        if (params.flags != mapped_file::readonly)
            nimboost::throw_exception(BOOST_IOSTREAMS_FAILURE("invalid flags"));
    } else {
        if (params.mode & BOOST_IOS::out)
            nimboost::throw_exception(BOOST_IOSTREAMS_FAILURE("invalid mode"));
...
mapped_file_sink::mapped_file_sink(
    ...
    nimboost::intmax_t offset, mapmode flags )
...
struct operations<mapped_file_source>
    : nimboost::iostreams::detail::close_impl<closable_tag>
{

...
struct resolve_traits {
    typedef typename 
            mpl::if_<
                nimboost::detail::is_incrementable<T>,
...
#  if !BOOST_WORKAROUND(BOOST_MSVC, <= 1300)
    ...
    range_adapter< Mode, nimboost::iterator_range<Iter> > 
    resolve(const nimboost::iterator_range<Iter>& rng)
    { return range_adapter< Mode, nimboost::iterator_range<Iter> >(rng); }
#  endif // #if BOOST_WORKAROUND(BOOST_MSVC, <= 1300)

...
#include <boost/detail/iterator.hpp>             // nimboost::iterator_traits.
...
// Description: Device based on an instance of nimboost::iterator_range.
// Template parameters:
//     Mode - A mode tag.
...
private:
    ...
    typedef nimboost::detail::iterator_traits<iterator>  iter_traits;
...
struct range_adapter_impl<std::forward_iterator_tag> {
        ...
        (Iter& cur, Iter& last, const Ch* s, std::streamsize n)
    {
        ...
        if (cur == last && n > 0)
            nimboost::throw_exception(write_area_exhausted());
        return n;
    }
};

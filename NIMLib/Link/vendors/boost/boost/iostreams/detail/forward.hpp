...
#define BOOST_IOSTREAMS_FORWARD(class, impl, device, params, args) \
    ...
    { this->impl(::nimboost::iostreams::detail::wrap(t) args()); } \
    class(device& t params()) \
    { this->impl(::nimboost::iostreams::detail::wrap(t) args()); } \
    class(const ::nimboost::reference_wrapper<device>& ref params()) \
    { this->impl(ref args()); } \
    void open(const device& t params()) \
    { this->impl(::nimboost::iostreams::detail::wrap(t) args()); } \
    void open(device& t params()) \
    { this->impl(::nimboost::iostreams::detail::wrap(t) args()); } \
    void open(const ::nimboost::reference_wrapper<device>& ref params()) \
    { this->impl(ref args()); } \
...
# define BOOST_IOSTREAMS_DISABLE_IF_SAME(device, param) \
    , typename nimboost::disable_if< nimboost::is_same<device, param> >::type* = 0 \
    /**/
#else 
# define BOOST_IOSTREAMS_DISABLE_IF_SAME(device, param)
#endif

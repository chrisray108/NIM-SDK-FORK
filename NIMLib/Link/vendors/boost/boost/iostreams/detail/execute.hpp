 ...
 * nimboost::iostreams::detail::execute_all() and the function template 
...
template< typename Op, 
          typename Result = // VC6.5 workaround.
                  ...
                  typename nimboost::result_of<Op()>::type
              #else
...
#define BOOST_PP_LOCAL_MACRO(n) \
   ...
   { \
       ...
       try { \
           r = nimboost::iostreams::detail::execute_all( \
...
Op execute_foreach(InIt first, InIt last, Op op)
{
    ...
    try {
        ...
        try {
            ++first;
            nimboost::iostreams::detail::execute_foreach(first, last, op);
        } catch (...) { }
        throw;
    }
    ++first;
    return nimboost::iostreams::detail::execute_foreach(first, last, op);
}

...
{
    if (!ibeg_) 
        nimboost::throw_exception(cant_read());
    if (!gptr()) 
        init_get_area();
    return gptr() != iend_ ? 
...
direct_streambuf<T, Tr>::pbackfail(int_type c)
{
    ...
    nimboost::throw_exception(bad_putback());
}
...
direct_streambuf<T, Tr>::overflow(int_type c)
{
    using namespace std;
    if (!obeg_)
        nimboost::throw_exception(BOOST_IOSTREAMS_FAILURE("no write access"));
...
void direct_streambuf<T, Tr>::close_impl(BOOST_IOS::openmode which)
{
    ...
    nimboost::iostreams::close(*storage_, which);
}

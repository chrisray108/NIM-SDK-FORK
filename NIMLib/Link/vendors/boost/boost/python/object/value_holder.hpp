...
{
    if (void* wrapped = holds_wrapped(dst_t, nimboost::addressof(m_held), nimboost::addressof(m_held)))
        return wrapped;
    
    type_info src_t = python::type_id<Value>();
    return src_t == dst_t ? nimboost::addressof(m_held)
        : find_static_type(nimboost::addressof(m_held), src_t, dst_t);
}
...
}}} // namespace nimboost::python::objects
...
# endif
    ...
    {
        python::detail::initialize_wrapper(self, nimboost::addressof(this->m_held));
    }

...
namespace posix_time {
  ...
  inline
  std::tm to_tm(const nimboost::posix_time::ptime& t) {
    std::tm timetm = nimboost::gregorian::to_tm(t.date());
    nimboost::posix_time::time_duration td = t.time_of_day();
  ...
  inline
  std::tm to_tm(const nimboost::posix_time::time_duration& td) {
    std::tm timetm;
  ...
  ptime ptime_from_tm(const std::tm& timetm) {
    nimboost::gregorian::date d = nimboost::gregorian::date_from_tm(timetm);
   ...
   *       nimboost::date_time::winapi::file_time type.
   */
  template< typename TimeT, typename FileTimeT >
  inline
  TimeT from_ftime(const FileTimeT& ft)
  {
    return nimboost::date_time::time_from_ftime<TimeT>(ft);
  }

#endif // BOOST_HAS_FTIME

} } //namespace nimboost::posix_time

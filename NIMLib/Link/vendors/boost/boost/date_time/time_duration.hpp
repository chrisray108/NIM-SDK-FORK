...
namespace date_time {
  ...
  */
  ...
  class time_duration : private
      nimboost::less_than_comparable<T
    , nimboost::equality_comparable<T
    > >
   ...
   */
  template<class base_duration, nimboost::int64_t frac_of_second>
  ...
  private:
    ...
    BOOST_STATIC_CONSTANT(nimboost::int64_t, adjustment_ratio = (traits_type::ticks_per_second >= frac_of_second ? traits_type::ticks_per_second / frac_of_second : frac_of_second / traits_type::ticks_per_second));

  public:
    explicit subsecond_duration(nimboost::int64_t ss) :

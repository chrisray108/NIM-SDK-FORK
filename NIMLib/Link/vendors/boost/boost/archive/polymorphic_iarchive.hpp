...
public:
    ...
    virtual void load(nimboost::long_long_type & t) = 0;
    virtual void load(nimboost::ulong_long_type & t) = 0;
    ...
    void load_override(
        ...
        const
        #endif
        nimboost::serialization::nvp< T > & t,
        int
    ){
        load_start(t.name());
...
public:
    ...
    virtual const detail::basic_pointer_iserializer * load_pointer(
        ...
        const detail::basic_pointer_iserializer * (*finder)(
            const nimboost::serialization::extended_type_info & type
        )
    ) = 0;
};
...
// required by export
BOOST_SERIALIZATION_REGISTER_ARCHIVE(nimboost::archive::polymorphic_iarchive)

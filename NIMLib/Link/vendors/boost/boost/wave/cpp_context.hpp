...
//
//      The nimboost::wave::context template is the main interface class to 
...
>
class context : private nimboost::noncopyable
{
private:
...
// the given iterator should be at least a forward iterator type
    BOOST_CLASS_REQUIRE(IteratorT, nimboost, ForwardIteratorConcept);
...
// type of a token sequence
    typedef std::list<token_type, nimboost::fast_pool_allocator<token_type> > 
        token_sequence_type;
...
// stack of shared_ptr's to the pending iteration contexts 
    typedef nimboost::shared_ptr<base_iteration_context<context, lexer_type> > 
        iteration_ptr_type;
    typedef nimboost::wave::util::iteration_context_stack<iteration_ptr_type> 
...
// iterator interface
    ...
    { 
        ...
        if (filename != "<Unknown>" && filename != "<stdin>") {
            using namespace nimboost::filesystem;
...
// maintain defined macros
    ...
    { 
        return nimboost::wave::util::add_macro_definition(*this, 
...
// Iterate over names of defined macros
    typedef nimboost::wave::util::macromap<context> macromap_type;
...
// get the Wave version information 
    ...
    { 
        nimboost::wave::util::predefined_macros p; 
...
// access current language options
    void set_language(nimboost::wave::language_support language_,
    ...
    nimboost::wave::language_support get_language() const { return language; }
...
protected:
    friend class nimboost::wave::pp_iterator<context>;
    friend class nimboost::wave::impl::pp_iterator_functor<context>;
#endif
...
// conditional compilation contexts
    ...
    typename nimboost::wave::util::if_block_stack::size_type get_if_block_depth() const 
...
///////////////////////////////////////////////////////////////////////////////
    ...
    // remove any contained placeholder
        nimboost::wave::util::impl::remove_placeholders(expanded);
    }
...
// the main input stream
    ...
    nimboost::wave::util::if_block_stack ifblocks;   // conditional compilation contexts
    nimboost::wave::util::include_paths includes;    // lists of include directories to search
    iteration_context_stack_type iter_ctxs;       // iteration contexts
    macromap_type macros;                         // map of defined macros
    nimboost::wave::language_support language;       // supported language/extensions
...
>
struct tracking_level<nimboost::wave::context<Iterator, LexIterator, InputPolicy, Hooks> >
{
...
template<
    ...
    typename InputPolicy, typename Hooks
>
struct version<nimboost::wave::context<Iterator, LexIterator, InputPolicy, Hooks> >
{
    typedef nimboost::wave::context<Iterator, LexIterator, InputPolicy, Hooks>
        target_type;

...
template <typename IteratorT,
    typename PositionT = nimboost::wave::util::file_position_type,
...
public:
    ...
    lexer(IteratorT const &first, IteratorT const &last,
        PositionT const &pos, nimboost::wave::language_support language_);
    ~lexer();
...
private:
    ...
    string_type value;
    bool at_eof;
    nimboost::wave::language_support language;
#if BOOST_WAVE_SUPPORT_PRAGMA_ONCE != 0
...
lexer<IteratorT, PositionT, TokenT>::lexer(IteratorT const &first,
        ...
        nimboost::wave::language_support language_)
...
#endif
{
    ...
    scanner.act_in_c99_mode = nimboost::wave::need_c99(language_);
#endif

#if BOOST_WAVE_SUPPORT_IMPORT_KEYWORD != 0
    scanner.enable_import_keyword = !nimboost::wave::need_c99(language_);
#else
    scanner.enable_import_keyword = false;
#endif

    scanner.detect_pp_numbers = nimboost::wave::need_prefer_pp_numbers(language_);
    scanner.single_line_only = nimboost::wave::need_single_line(language_);

#if BOOST_WAVE_SUPPORT_CPP0X != 0
    scanner.act_in_cpp0x_mode = nimboost::wave::need_cpp0x(language_);
#else
...
lexer<IteratorT, PositionT, TokenT>::get(TokenT& result)
{
    ...
    // test identifier characters for validity (throws if invalid chars found)
        ...
        if (!nimboost::wave::need_no_character_validation(language))
    ...
    // test literal characters for validity (throws if invalid chars found)
        value = string_type((char const *)scanner.tok,
            scanner.cur-scanner.tok);
        if (nimboost::wave::need_convert_trigraphs(language))
    ...
    case T_LONGINTLIT:  // supported in C++11, C99 and long_long mode
        value = string_type((char const *)scanner.tok,
            scanner.cur-scanner.tok);
        if (!nimboost::wave::need_long_long(language)) {
...
//     std::cerr << nimboost::wave::get_token_name(id) << ": " << value << std::endl;
...
public:
    ...
    lex_functor(IteratorT const &first, IteratorT const &last,
            PositionT const &pos, nimboost::wave::language_support language)

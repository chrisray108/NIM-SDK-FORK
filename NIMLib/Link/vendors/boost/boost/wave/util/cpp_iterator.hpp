...
retrieve_macroname(ContextT& ctx, ParseNodeT const &node,
    nimboost::spirit::classic::parser_id id, TokenT &macroname, PositionT& act_pos,
    bool update_position)
{
ParseNodeT const *name_node = 0;

    using nimboost::spirit::classic::find_node;
...
retrieve_macrodefinition(
    ParseNodeT const &node, nimboost::spirit::classic::parser_id id,
    ContainerT &macrodefinition, PositionT& act_pos, bool update_position)
{
    using namespace nimboost::wave;
...
std::pair<const_tree_iterator, const_tree_iterator> nodes;
    ...
    using nimboost::spirit::classic::get_node_range;
...
bool add_macro_definition(ContextT &ctx, std::string macrostring,
    bool is_predefined, nimboost::wave::language_support language)
{
    typedef typename ContextT::token_type token_type;
    typedef typename ContextT::lexer_type lexer_type;
    typedef typename token_type::position_type position_type;
    typedef nimboost::wave::grammars::predefined_macros_grammar_gen<lexer_type>
        predef_macros_type;
...
nimboost::spirit::classic::tree_parse_info<lexer_type> hit =
...
bool has_parameters = false;
    ...
    if (!nimboost::wave::util::retrieve_macroname(ctx, *hit.trees.begin(),
            BOOST_WAVE_PLAIN_DEFINE_ID, macroname, act_pos, true))
        return false;
    has_parameters = nimboost::wave::util::retrieve_macrodefinition(*hit.trees.begin(),
        BOOST_WAVE_MACRO_PARAMETERS_ID, macroparameters, act_pos, true);
    nimboost::wave::util::retrieve_macrodefinition(*hit.trees.begin(),
...
public:
// interface to the nimboost::spirit::classic::iterator_policies::functor_input policy
...
// type of a token sequence
    ...
    typedef nimboost::wave::grammars::cpp_grammar_gen<lexer_type, token_sequence_type>
        cpp_grammar_type;
...
// parse tree related types
    ...
    typedef nimboost::spirit::classic::tree_parse_info<lexer_type, node_factory_type>
        tree_parse_info_type;
    typedef nimboost::spirit::classic::tree_match<lexer_type, node_factory_type>
    ...
    :   ctx(ctx_),
        iter_ctx(new base_iteration_context_type(ctx,
                lexer_type(first_, last_, pos_,
                    nimboost::wave::enable_prefer_pp_numbers(ctx.get_language())),
                lexer_type(),
                pos_.get_file().c_str()
            )),
        seen_newline(true), skipped_newline(false),
        must_emit_line_directive(false), act_pos(ctx_.get_main_pos()),
        whitespace(nimboost::wave::need_insert_whitespace(ctx.get_language()))
    {
...
private:
    ...
    nimboost::shared_ptr<base_iteration_context_type> iter_ctx;
    ...
    // re-tokenized
    nimboost::wave::util::insert_whitespace_detection whitespace;
};
...
pp_iterator_functor<ContextT>::returned_from_include()
{
...
#if BOOST_WAVE_SUPPORT_PRAGMA_ONCE != 0
        namespace fs = nimboost::filesystem;
    ...
    // report unbalanced #if/#endif now to make it possible to recover properly
        if (iter_ctx->if_block_depth != ctx.get_if_block_depth()) {
            using nimboost::wave::util::impl::escape_lit;
...
pp_iterator_functor<ContextT>::operator()()
{
    ...
    catch (nimboost::wave::cpplexer::lexing_exception const& e) {
...
//             whitespace.shift_tokens(T_NEWLINE);
        ...
        token_sequence_type pending;
            ...
            std::string file("\"");
            nimboost::filesystem::path filename(
    ...
    // skip null pp directive (no need to do it via the parser)
        if (it != iter_ctx->last && T_POUND == BASE_TOKEN(token_id(*it))) {
            ...
            // report invalid pp directive
                ...
                string_type str(nimboost::wave::util::impl::as_string<string_type>(
                    iter_ctx->first, it));
...
// test, if this file is known through a #pragma once directive
    ...
    nimboost::shared_ptr<base_iteration_context_type> new_iter_ctx (
...
string_type s (impl::trim_whitespace(nimboost::wave::util::impl::as_string(expanded)));
...
position_type pos(act_token.get_position());
    ...
    if (!nimboost::wave::util::retrieve_macroname(ctx, node,
            BOOST_WAVE_PLAIN_DEFINE_ID, macroname, pos, false))
        return;
    has_parameters = nimboost::wave::util::retrieve_macrodefinition(node,
        BOOST_WAVE_MACRO_PARAMETERS_ID, macroparameters, pos, false);
    nimboost::wave::util::retrieve_macrodefinition(node,
        BOOST_WAVE_MACRO_DEFINITION_ID, macrodefinition, pos, false);

    if (has_parameters) {
#if BOOST_WAVE_SUPPORT_VARIADICS_PLACEMARKERS != 0
        if (nimboost::wave::need_variadics(ctx.get_language())) {
...
token_sequence_type expanded;
    ...
    // replace all remaining (== undefined) identifiers with an integer literal '0'
        ...
        {
            string_type outstr(nimboost::wave::util::impl::as_string(toexpand));
            outstr += "(" + nimboost::wave::util::impl::as_string(expanded) + ")";
        ...
        catch (nimboost::wave::preprocess_exception const& e) {
...
namespace impl {
    ...
    bool retrieve_line_info (IteratorT first, IteratorT const &last,
        ...
        nimboost::wave::preprocess_exception::error_code& error)
    {
        using namespace nimboost::wave;
        ...
        // re-extract line number with spirit to diagnose overflow
            using namespace nimboost::spirit::classic;
    ...
    // preprocess the body of this #line message
            ...
            line, file_name, error))
        {
            typename ContextT::string_type msg(
                nimboost::wave::util::impl::as_string(expanded));
...
// set new line number/filename only if ok
    if (!file_name.empty()) {    // reuse current file name
        using nimboost::wave::util::impl::unescape_lit;
...
// message
    ...
    // report the corresponding error
        BOOST_WAVE_STRINGTYPE msg(nimboost::wave::util::impl::as_string(expanded));
...
const_tree_iterator_t last = make_ref_transform_iterator(end, get_value);
    ...
    if (first != last) {
        if (T_IDENTIFIER == token_id(*first) &&
            nimboost::wave::need_c99(ctx.get_language()) &&
...
// returned to the caller.
    if (nimboost::wave::need_emit_pragma_directives(ctx.get_language())) {
        pending_queue.splice(pending_queue.begin(), expanded);
    ...
    token_sequence_type const &pragma_body, token_sequence_type &result)
{
    ...
    return nimboost::wave::util::interpret_pragma(
...
//  pp_iterator
//
//      The nimboost::wave::pp_iterator template is the iterator, through which
...
template <typename ContextT>
class pp_iterator
:   public nimboost::spirit::classic::multi_pass<
        nimboost::wave::impl::pp_iterator_functor<ContextT>,
        nimboost::wave::util::functor_input
    >
{
public:
    typedef nimboost::wave::impl::pp_iterator_functor<ContextT> input_policy_type;

private:
    typedef
        nimboost::spirit::classic::multi_pass<input_policy_type, nimboost::wave::util::functor_input>
        base_type;
    typedef pp_iterator<ContextT> self_type;
    typedef nimboost::wave::util::functor_input functor_input_type;

...
private:
    ...
    nimboost::shared_ptr<defined_macros_type> defined_macros; // global symbol table
...
//  T_IDENTIFIER token, try to replace this as a macro etc.
    ...
    // ignore placeholder tokens
        ...
        // try to replace this identifier as a macro
            ...
            else if (nimboost::wave::need_variadics(ctx.get_language()) &&
    ...
    token_id base_id = BASE_TOKEN(token_id(*cit));
        ...
        bool is_ellipsis = false;
            ...
            if (IS_EXTCATEGORY((*cit), ExtParameterTokenType)) {
                BOOST_ASSERT(nimboost::wave::need_variadics(ctx.get_language()));
...
#if BOOST_WAVE_SUPPORT_VARIADICS_PLACEMARKERS != 0
                if (is_ellipsis && nimboost::wave::need_variadics(ctx.get_language())) {
...
{
    using namespace nimboost::wave;
    using namespace nimboost::wave::grammars;

ContainerT result;
IteratorT start = first;
nimboost::spirit::classic::parse_info<IteratorT> hit =
...
// re-tokenize the newly generated string
    ...
    nimboost::wave::language_support lang =
        nimboost::wave::enable_prefer_pp_numbers(ctx.get_language());
    lang = nimboost::wave::enable_single_line(lang);
...
position_type pos("<built-in>");
...
#if BOOST_WAVE_SUPPORT_VARIADICS_PLACEMARKERS != 0
    if (nimboost::wave::need_c99(ctx.get_language())) {
...
#endif
    {
        ...
        if (nimboost::wave::need_cpp0x(ctx.get_language())) {
...
template<typename ContextT>
struct version<nimboost::wave::util::macromap<ContextT> >
{
    typedef nimboost::wave::util::macromap<ContextT> target_type;

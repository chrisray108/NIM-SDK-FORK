...
struct bidirectional_map
{
    ...
    typedef nimboost::multi_index::multi_index_container<
        value_type,
        nimboost::multi_index::indexed_by<
            nimboost::multi_index::ordered_unique<
                nimboost::multi_index::tag<from>, 
                nimboost::multi_index::member_offset<value_type, FromType, from_offset> 
            >,
            nimboost::multi_index::ordered_non_unique<
                nimboost::multi_index::tag<to>, 
                nimboost::multi_index::member_offset<value_type, ToType, to_offset> 
            >
        >
    > type;
  ...
  typedef nimboost::multi_index::multi_index_container<
      ...
      nimboost::multi_index::indexed_by<
          nimboost::multi_index::ordered_unique<
              nimboost::multi_index::tag<from>,
              nimboost::multi_index::member<value_type, FromType, &value_type::first> 
          >,
          nimboost::multi_index::ordered_non_unique<
              nimboost::multi_index::tag<to>,  
              nimboost::multi_index::member<value_type, ToType, &value_type::second> 
          >
      >
  > type;
...
{
private:
    typedef std::list<std::pair<nimboost::filesystem::path, std::string> > 
        include_list_type;
...
public:
    ...
    nimboost::filesystem::path get_current_directory() const 
        { return current_dir; }
...
private:
    ...
    nimboost::filesystem::path current_dir;
    nimboost::filesystem::path current_rel_dir;

#if BOOST_WAVE_SUPPORT_PRAGMA_ONCE != 0
public:
    bool has_pragma_once(std::string const &filename)
    {
        using nimboost::multi_index::get;
...
private:
    ...
    {
        using namespace nimboost::serialization;
#if BOOST_WAVE_SUPPORT_PRAGMA_ONCE != 0
...
inline bool
as_relative_to(nimboost::filesystem::path const& path, 
    nimboost::filesystem::path const& base, nimboost::filesystem::path& result) 
{
    if (path.has_root_path()) {
        ...
        if (base.has_root_path()) {
            ...
            typedef nimboost::filesystem::path::const_iterator path_iterator;
...
///////////////////////////////////////////////////////////////////////////////
//  Serialization support for nimboost::filesystem::path
template<class Archive>
inline void save (Archive & ar, nimboost::filesystem::path const& p, 
...
template<class Archive>
inline void load (Archive & ar, nimboost::filesystem::path &p,
    const unsigned int /* file_version */)
{
    using namespace nimboost::serialization;
    std::string path_str;
...
template<class Archive>
inline void serialize (Archive & ar, nimboost::filesystem::path &p,
    const unsigned int file_version)
{
    nimboost::serialization::split_free(ar, p, file_version);
}
...
inline void save (Archive & ar,
    const typename nimboost::wave::util::bidirectional_map<
        std::string, std::string
    >::type &t,
    const unsigned int /* file_version */)
{
    nimboost::serialization::stl::save_collection<
        Archive, 
        typename nimboost::wave::util::bidirectional_map<
            std::string, std::string
        >::type
    >(ar, t);
}
...
inline void load (Archive & ar,
    typename nimboost::wave::util::bidirectional_map<std::string, std::string>::type &t,
    const unsigned int /* file_version */)
{
    typedef typename nimboost::wave::util::bidirectional_map<
            std::string, std::string
        >::type map_type;
    nimboost::serialization::stl::load_collection<
        Archive, map_type,
        nimboost::serialization::stl::archive_input_unique<Archive, map_type>,
        nimboost::serialization::stl::no_reserve_imp<map_type>
    >(ar, t);
}
...
///////////////////////////////////////////////////////////////////////////////
}}  // namespace nimboost::serialization

BOOST_CLASS_VERSION(nimboost::wave::util::include_paths, 
    nimboost::wave::util::include_paths::version);

...
#include <boost/utility/enable_if.hpp> // for nimboost::disable_if

namespace nimboost { namespace algorithm {

/// \fn clamp ( T const& val, 
///               typename nimboost::mpl::identity<T>::type const& lo, 
///               typename nimboost::mpl::identity<T>::type const& hi, Pred p )
...
/// \fn clamp ( T const& val, 
///               typename nimboost::mpl::identity<T>::type const& lo, 
///               typename nimboost::mpl::identity<T>::type const& hi )
/// \return the value "val" brought into the range [ lo, hi ].
...
///       typename std::iterator_traits<typename nimboost::range_iterator<const Range>::type>::value_type lo,
///       typename std::iterator_traits<typename nimboost::range_iterator<const Range>::type>::value_type hi )
/// \return clamp the sequence of values [first, last) into [ lo, hi ]
/// 
...
///
  ...
  typename nimboost::disable_if_c<nimboost::is_same<Range, OutputIterator>::value, OutputIterator>::type
  clamp_range ( const Range &r, OutputIterator out,
    typename std::iterator_traits<typename nimboost::range_iterator<const Range>::type>::value_type lo, 
    typename std::iterator_traits<typename nimboost::range_iterator<const Range>::type>::value_type hi )
  {
    return clamp_range ( nimboost::begin ( r ), nimboost::end ( r ), out, lo, hi );
  } 
...
//  In that case, the user will get the two iterator version.
  ...
  clamp_range ( const Range &r, OutputIterator out,
    ...
    Pred p )
  {
    return clamp_range ( nimboost::begin ( r ), nimboost::end ( r ), out, lo, hi, p );
  } 

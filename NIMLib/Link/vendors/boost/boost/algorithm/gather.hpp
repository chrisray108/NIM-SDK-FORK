...
#include <boost/bind.hpp>           // for nimboost::bind
#include <boost/range/begin.hpp>    // for nimboost::begin(range)
#include <boost/range/end.hpp>      // for nimboost::end(range)
...
//  while the second call partitions the rest of the sequence.
    return std::make_pair (
        std::stable_partition ( first, pivot, !nimboost::bind<bool> ( pred, _1 )),
        std::stable_partition ( pivot, last,   nimboost::bind<bool> ( pred, _1 )));
}
...
template <
    ...
    typename Pred>                  // Pred models UnaryPredicate
std::pair<
    typename nimboost::range_iterator<const BidirectionalRange>::type,
    typename nimboost::range_iterator<const BidirectionalRange>::type>
gather (
    const BidirectionalRange &range,
    typename nimboost::range_iterator<const BidirectionalRange>::type pivot,
    Pred pred )
{
    return nimboost::algorithm::gather ( nimboost::begin ( range ), nimboost::end ( range ), pivot, pred );
}

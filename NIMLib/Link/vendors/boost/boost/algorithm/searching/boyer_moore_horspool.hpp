...
*/
    ...
    public:
        ...
        corpusIter operator () ( corpusIter corpus_first, corpusIter corpus_last ) const {
            BOOST_STATIC_ASSERT (( nimboost::is_same<
        ...
        typename nimboost::range_iterator<Range>::type operator () ( Range &r ) const {
            return (*this) (nimboost::begin(r), nimboost::end(r));
            }
...
///
    ...
    {
        typedef typename nimboost::range_iterator<const PatternRange>::type pattern_iterator;
        boyer_moore_horspool<pattern_iterator> bmh ( nimboost::begin(pattern), nimboost::end (pattern));
        return bmh ( corpus_first, corpus_last );
    }
    
    template <typename patIter, typename CorpusRange>
    typename nimboost::lazy_disable_if_c<
        nimboost::is_same<CorpusRange, patIter>::value, typename nimboost::range_iterator<CorpusRange> >
    ::type
    boyer_moore_horspool_search ( CorpusRange &corpus, patIter pat_first, patIter pat_last )
    {
        boyer_moore_horspool<patIter> bmh ( pat_first, pat_last );
        return bm (nimboost::begin (corpus), nimboost::end (corpus));
    }
    
    template <typename PatternRange, typename CorpusRange>
    typename nimboost::range_iterator<CorpusRange>::type
    boyer_moore_horspool_search ( CorpusRange &corpus, const PatternRange &pattern )
    {
        typedef typename nimboost::range_iterator<const PatternRange>::type pattern_iterator;
        boyer_moore_horspool<pattern_iterator> bmh ( nimboost::begin(pattern), nimboost::end (pattern));
        return bmh (nimboost::begin (corpus), nimboost::end (corpus));
    }


    //  Creator functions -- take a pattern range, return an object
    template <typename Range>
    nimboost::algorithm::boyer_moore_horspool<typename nimboost::range_iterator<const Range>::type>
    make_boyer_moore_horspool ( const Range &r ) {
        return nimboost::algorithm::boyer_moore_horspool
            <typename nimboost::range_iterator<const Range>::type> (nimboost::begin(r), nimboost::end(r));
        }
    
    template <typename Range>
    nimboost::algorithm::boyer_moore_horspool<typename nimboost::range_iterator<Range>::type>
    make_boyer_moore_horspool ( Range &r ) {
        return nimboost::algorithm::boyer_moore_horspool
            <typename nimboost::range_iterator<Range>::type> (nimboost::begin(r), nimboost::end(r));
        }

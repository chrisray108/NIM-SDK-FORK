...
inline void serialize(Archive& ar,nimboost::units::unit<Dim,System>&,const unsigned int /*version*/)
{ }

/// Boost Serialization library support for quantities.
template<class Archive,class Unit,class Y>
inline void serialize(Archive& ar,nimboost::units::quantity<Unit,Y>& q,const unsigned int /*version*/)
{
    ar & nimboost::serialization::make_nvp("value", units::quantity_cast<Y&>(q));
}
...
template<class T, bool C = nimboost::is_arithmetic<T>::value>

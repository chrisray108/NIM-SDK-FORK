...
namespace nimboost
{
  ...
  {
    inline nimboost::recursive_mutex& terminal_mutex()
    {
      static nimboost::recursive_mutex mtx;
      return mtx;
    }
...
#define BOOST_THREAD_LOG \
  { \
    nimboost::lock_guard<nimboost::recursive_mutex> _lk_(nimboost::thread_detail::terminal_mutex()); \
    std::cout << nimboost::this_thread::get_id() << " - "<<__FILE__<<"["<<__LINE__<<"] " <<std::dec
#else
...
#define BOOST_THREAD_LOG if (true) {} else nimboost::thread_detail::dummy_stream
#define BOOST_THREAD_END_LOG nimboost::thread_detail::dummy_stream

...
namespace nimboost
{
   ...
   *
   * thread std/nimboost::thread destructor terminates the program if the thread is not joinable.
   * Having a wrapper that can join the thread before destroying it seems a natural need.
   *
   * Example:
   *
   *     nimboost::strict_scoped_thread<> t((nimboost::thread(F)));
   *
   */
  ...
  public:
    ...
    strict_scoped_thread(BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(A1) a1, BOOST_THREAD_FWD_REF(A2) a2, BOOST_THREAD_FWD_REF(A3) a3) :
      t_(nimboost::forward<F>(f), nimboost::forward<A1>(a1), nimboost::forward<A2>(a2), nimboost::forward<A3>(a3)) {}
#endif
   ...
   * Example:
   *
   *     nimboost::scoped_thread<> t((nimboost::thread(F)));
   *     t.interrupt();
   *
   */
  template <class CallableThread = join_if_joinable>

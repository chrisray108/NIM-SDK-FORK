...
namespace nimboost
{
  ...
  private:
    ...
    {
      elem = nimboost::move(data_[out_]);
      out_ = inc(out_);
      notify_not_full_if_needed(lk);
    }
    inline nimboost::shared_ptr<value_type> ptr_pull(unique_lock<mutex>& lk)
    {
      shared_ptr<value_type> res = make_shared<value_type>(nimboost::move(data_[out_]));
      out_ = inc(out_);
      notify_not_full_if_needed(lk);
      return res;
    }
...
//  }
  ...
  template <typename ValueType>
  nimboost::shared_ptr<ValueType> sync_bounded_queue<ValueType>::try_pull()
  {
    try
    {
      unique_lock<mutex> lk(mtx_);
  ...
  template <typename ValueType>
  nimboost::shared_ptr<ValueType> sync_bounded_queue<ValueType>::ptr_pull()
  {
    try
    {
      unique_lock<mutex> lk(mtx_);
  ...
  bool sync_bounded_queue<ValueType>::try_push(BOOST_THREAD_RV_REF(ValueType) elem, unique_lock<mutex>& lk)
  {
    ...
    push_at(nimboost::move(elem), in_p_1, lk);
    return true;
  }

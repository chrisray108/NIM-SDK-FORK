...
namespace nimboost
{
    ...
    public:
        ...
        void lock_shared()
        {
            ...
            BOOST_VERIFY(timed_lock_shared(::nimboost::detail::get_system_time_sentinel()));
#else
        ...
        bool timed_lock_shared(nimboost::system_time const& wait_until)
        {
            for(;;)
            {
                state_data old_state=state;
                for(;;)
                {
                    ...
                    if(new_state.exclusive || new_state.exclusive_waiting_blocked)
                    {
                        ...
                        {
                            nimboost::throw_exception(nimboost::lock_error());
                        }
                    }
                    else
                    {
                ...
                unsigned long const res=detail::win32::WaitForSingleObject(semaphores[unlock_sem],::nimboost::detail::get_milliseconds_until(wait_until));
        ...
        {
#if defined BOOST_THREAD_USES_DATETIME
            BOOST_VERIFY(timed_lock(::nimboost::detail::get_system_time_sentinel()));
#else
...
#if defined BOOST_THREAD_USES_DATETIME
        bool timed_lock(nimboost::system_time const& wait_until)
        {
            for(;;)
            {
                state_data old_state=state;
                ...
                #endif
                unsigned long const wait_res=detail::win32::WaitForMultipleObjects(2,semaphores,wait_all,::nimboost::detail::get_milliseconds_until(wait_until));

...
namespace nimboost
{
  ...
  private:
    ...
    nimboost::container::deque<ValueType> data_;
    bool closed_;
    ...
    {
      elem = nimboost::move(data_.front());
      data_.pop_front();
    }
    inline nimboost::shared_ptr<value_type> ptr_pull(unique_lock<mutex>& )
    {
      shared_ptr<value_type> res = make_shared<value_type>(nimboost::move(data_.front()));
      data_.pop_front();
      return res;
    }
...
//      iterator_t first = nimboost::begin(range);
//      iterator_t end = nimboost::end(range);
//      for (iterator_t cur = first; cur != end; ++cur)
//      {
//        data_.push(nimboost::move(*cur));;
//      }
//      notify_not_empty_if_needed(lk);
//    }
//    catch (...)
...
//  }
  ...
  template <typename ValueType>
  nimboost::shared_ptr<ValueType> sync_queue<ValueType>::try_pull()
  {
    try
    {
      unique_lock<mutex> lk(mtx_);
  ...
  template <typename ValueType>
  nimboost::shared_ptr<ValueType> sync_queue<ValueType>::ptr_pull()
  {
    try
    {
      unique_lock<mutex> lk(mtx_);
  ...
  {
    throw_if_closed(lk);
    push(nimboost::forward<ValueType>(elem), lk);
    return true;
  }
  ...
  {
    sbq.push(nimboost::forward<ValueType>(elem));
    return sbq;
  }

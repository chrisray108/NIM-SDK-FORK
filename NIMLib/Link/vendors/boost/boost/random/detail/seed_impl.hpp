...
struct seed_type
{
    typedef typename nimboost::mpl::if_<nimboost::is_integral<T>,
        T,
        nimboost::uint32_t
    >::type type;
};
...
void generate_from_real(Engine& eng, Iter begin, Iter end)
{
    ...
    nimboost::uint_least32_t saved_bits = 0;
    ...
    while(true) {
        ...
        if(Bits < 32 && available_bits < 32 - remaining_bits) {
            saved_bits |= nimboost::uint_least32_t(val) << remaining_bits;
            remaining_bits += Bits;
        } else {
            ...
            if(Bits < 32 || remaining_bits != 0) {
                nimboost::uint_least32_t divisor =
                    (nimboost::uint_least32_t(1) << (32 - remaining_bits));
                nimboost::uint_least32_t extra_bits = nimboost::uint_least32_t(fmod(val, mult32)) & (divisor - 1);
                val = val / divisor;
            ...
            if(Bits >= 32) {
                for(; available_bits >= 32; available_bits -= 32) {
                    nimboost::uint_least32_t word = nimboost::uint_least32_t(fmod(val, mult32));
                    val /= mult32;
                    *begin++ = word;
                    if(begin == end) return;
                }
            ...
            saved_bits = static_cast<nimboost::uint_least32_t>(val);
        }
    }
}
...
void generate_from_int(Engine& eng, Iter begin, Iter end)
{
    ...
    typedef typename nimboost::make_unsigned<IntType>::type unsigned_type;
    int remaining_bits = 0;
    nimboost::uint_least32_t saved_bits = 0;
    unsigned_type range = nimboost::random::detail::subtract<IntType>()((eng.max)(), (eng.min)());
    ...
    while(true) {
        unsigned_type val;
        do {
            val = nimboost::random::detail::subtract<IntType>()(eng(), (eng.min)());
        } while(limit != range && val > limit);
        val &= mask;
        int available_bits = bits;
        if(available_bits == 32) {
            *begin++ = static_cast<nimboost::uint_least32_t>(val) & 0xFFFFFFFFu;
            if(begin == end) return;
        } else if(available_bits % 32 == 0) {
            for(int i = 0; i < available_bits / 32; ++i) {
                nimboost::uint_least32_t word = nimboost::uint_least32_t(val) & 0xFFFFFFFFu;
            ...
            if(bits < 32 || remaining_bits != 0) {
                nimboost::uint_least32_t extra_bits = nimboost::uint_least32_t(val) & ((nimboost::uint_least32_t(1) << (32 - remaining_bits)) - 1);
...
template<class Engine, class Iter>
void generate_impl(Engine& eng, Iter first, Iter last, nimboost::mpl::true_)
{
    return detail::generate_from_int(eng, first, last);
}

template<class Engine, class Iter>
void generate_impl(Engine& eng, Iter first, Iter last, nimboost::mpl::false_)
{
...
{
    return detail::generate_impl(eng, first, last, nimboost::is_integral<typename Engine::result_type>());
}
...
{
    static const int log = ::nimboost::mpl::if_c<(m == 0),
        ::nimboost::mpl::int_<(::std::numeric_limits<IntType>::digits)>,
        ::nimboost::static_log2<m> >::type::value;
    static const int k =
        (log + ((~(static_cast<IntType>(2) << (log - 1)) & m)? 32 : 31)) / 32;
    ::nimboost::uint_least32_t array[log / 32 + 4];
...
{
    nimboost::uint_least32_t storage[((w+31)/32) * n];
    ...
    for(std::size_t j = 0; j < n; j++) {
        ...
        x[j] = val & ::nimboost::low_bits_mask_t<w>::sig_bits;
    }
}

template<int w, std::size_t n, class SeedSeq, class IntType>
inline void seed_array_int_impl(SeedSeq& seq, IntType (&x)[n], nimboost::mpl::true_)
{
    typedef typename nimboost::make_unsigned<IntType>::type unsigned_array[n];
    seed_array_int_impl<w>(seq, reinterpret_cast<unsigned_array&>(x));
}

template<int w, std::size_t n, class SeedSeq, class IntType>
inline void seed_array_int_impl(SeedSeq& seq, IntType (&x)[n], nimboost::mpl::false_)
{
...
{
    seed_array_int_impl<w>(seq, x, nimboost::is_signed<IntType>());
}
...
inline void fill_array_int_impl(Iter& first, Iter last, IntType (&x)[n], nimboost::mpl::true_)
{
    typedef typename nimboost::make_unsigned<IntType>::type unsigned_array[n];
...
template<int w, std::size_t n, class Iter, class IntType>
inline void fill_array_int_impl(Iter& first, Iter last, IntType (&x)[n], nimboost::mpl::false_)
{
...
{
    fill_array_int_impl<w>(first, last, x, nimboost::is_signed<IntType>());
}

template<int w, std::size_t n, class RealType>
void seed_array_real_impl(const nimboost::uint_least32_t* storage, RealType (&x)[n])
{
    nimboost::uint_least32_t mask = ~((~nimboost::uint_least32_t(0)) << (w%32));

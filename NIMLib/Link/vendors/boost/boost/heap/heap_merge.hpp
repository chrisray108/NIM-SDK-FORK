...
struct heap_merge_emulate
{
    ...
    typedef typename nimboost::mpl::if_c<Heap1::has_reserve,
                                      reserver,
...
struct heap_merge_same
{
    ...
    typedef typename nimboost::mpl::if_c<is_mergable,
...
{
    BOOST_CONCEPT_ASSERT((nimboost::heap::PriorityQueue<Heap1>));
    BOOST_CONCEPT_ASSERT((nimboost::heap::PriorityQueue<Heap2>));

    // if this assertion is triggered, the value_compare types are incompatible
    BOOST_STATIC_ASSERT((nimboost::is_same<typename Heap1::value_compare, typename Heap2::value_compare>::value));

    const bool same_heaps = nimboost::is_same<Heap1, Heap2>::value;

    typedef typename nimboost::mpl::if_c<same_heaps,

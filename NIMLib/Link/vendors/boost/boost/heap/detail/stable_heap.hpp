...
template <typename T,
          ...
          typename StabilityCounterType = nimboost::uintmax_t,
          bool stable = false
         >
struct heap_base:
#ifndef BOOST_MSVC
    ...
    size_holder<constant_time_size, size_t>
{
    ...
    internal_type make_node(T const & val)
    {
        ...
        if (counter_ == (std::numeric_limits<stability_counter_type>::max)())
            BOOST_THROW_EXCEPTION(std::runtime_error("nimboost::heap counter overflow"));
...
class stable_heap_iterator:
    public nimboost::iterator_adaptor<stable_heap_iterator<T, ContainerIterator, Extractor>,
                                   ContainerIterator,
                                   T const,
                                   nimboost::random_access_traversal_tag>
{
    typedef nimboost::iterator_adaptor<stable_heap_iterator,
                                    ContainerIterator,
                                    T const,
                                    nimboost::random_access_traversal_tag> super_t;
...
struct make_heap_base
{
    ...
    typedef typename parameter::binding<Parspec, tag::stability_counter_type, nimboost::uintmax_t >::type stability_counter_type;
